...
| `D` `H` `S` `J` `F` | 진단로그 / 하드웨어반응 / 스트레스 / 할일 알림 데모 / 집중모드 | **`StickMateDevTools` 게이트가 열려야 동작**. ★ 2026-09-08 정정(coder-ui 실측) — `J`는 **할일 알림 연출(TodoReminder) 데모일 뿐 [오늘 할일] 팝오버를 열지 않는다**(`ItemCatalog.cs:871`: "사용자 진입점이 없다"). 그 팝오버를 여는 전역 단축키는 **없다** — 실제 진입 경로는 캐릭터 우클릭/톱니 → 부채꼴 메뉴 ③[오늘 할일] 클릭뿐이라, 자동화로 열려면 좌표 클릭 주입이 필요하다(예시: `scratchpad/mousehold.swift`, 아직 스킬에 편입 안 됨 — 필요하면 이 자리에 추가할 것) |
