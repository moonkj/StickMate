...
# macOS ANSI 가상 키코드. 앱이 쓰는 동작키 16종만 담는다(GlobalKey 열거형과 1:1).
keycode_for() {
  case "$(printf '%s' "$1" | tr '[:lower:]' '[:upper:]')" in
    ...
    ",") echo 43;;  # kVK_ANSI_Comma (2026-09-01 소은 발견: 설정창 진입 단축키 ⌃⌥⌘, 가 표에 없었음)
    *) return 1;;
  esac
}
...
cmd_key() {
  ...
  if [ -f "$LOG_FILE" ]; then
    ...
    local pat="\[앱제어\]|\[활쏘기\]|\[격파|\[그라피티\]|\[창도둑\]|\[창 도둑\]|\[윈도우크래시\]|\[하드웨어|\[스트레스|\[가출\]|\[투두\]|\[집중|\[정보창\]|\[성장\]|\[기록\]"
