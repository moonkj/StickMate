...
> ## ★★★ 2026-09-05 상태 고지 — **이 카탈로그는 전량 폐기됐다(이력 보존)**
> 사용자 확정 2026-09-03 ***"소리시스템을 전체빼줘"***. **이 표의 어떤 키도 만들지 않는다.**
> 조달(D3)도 발생하지 않는다 — 저장소에 오디오 파일은 여전히 **0개**이고 그대로 둔다.
> 살아남은 것은 §0의 **카탈로그 규칙 5개**(특히 1번 *"소리는 새 이벤트를 만들지 않는다"*)뿐이고,
> 그 원리는 `design/sound/DANCE_SOUND_VERDICT.md`에서 계속 쓰인다.

> ## ★★ 2026-09-05 R6 추가 — 이 카탈로그에 **없는 표면**이 심사됐다
> 부채꼴 메뉴(`design/sound/FAN_MENU_SOUND_VERDICT.md`). **이 문서의 18키 어디에도 없던 표면**이고,
> 심사 결과 사건 10개 중 5개가 자격을 통과했다 — **그런데도 만들지 않는다**(채널이 닫혀 있다).
> ★ **§0 규칙 1(«소리는 새 이벤트를 만들지 않는다»)이 여기서 값을 했다**:
> 부채꼴의 사건 10개는 전부 **프로덕션이 이미 갈라 놓은 상수**에서 나왔다
> (`ExpandTotalSeconds` / `HoverSeconds` / `PressFlashSeconds` / `CollapseUserSeconds` /
> `CollapseDragSeconds` / `CollapseAutoSeconds` / `QuitConfirmSeconds`).
> **소리가 이벤트를 발명했다면 「접힘」을 한 개로 셌을 것이고, 자율 갈래가 조용히 섞였을 것이다.**
