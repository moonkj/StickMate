...
- `cards12.py` — `Resources/Items/*.asset`의 **카드 아이콘 12장** 정원/보조색 감사(12장 중 6장 위반).
...
- `scale060.py` — 실루엣 코너 붕괴 2건(`BeanieCuff` / `BeretBody`) 수정안 + 부작용 24항목 점검
  + **"LOD를 넣기 전에 좌표를 고쳐야 하는 이유"**(수정 없이 LOD만 넣으면 0.7120에서 그대로 막힌다).
  ★ `items.py`는 **프로덕션 거울**이라 손대지 않았다 — 사본에 패치를 얹어 잰다.
  coder가 좌표를 넣은 뒤 `items.py`를 갱신하고 `scale060.py`를 지우면 된다.
- `sheet060.py` → `design/equipment/scale060-hats.png` (현행/수정안 × 배율 0.60/0.75, 실제 획 두께).
