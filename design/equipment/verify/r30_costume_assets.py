...
def cyber():
    out = _from_r28(R28.cyber())
    # ★ R31 수정(design-equipment, 2026-09-08) — `filled` 는 면이 아니라 **2.2배 재묘사**라
    #   내부가 번짐 반폭(1.10 W_P = 0.03729 H)보다 두꺼우면 «가운데 뚫린 굵은 테두리»가 된다.
    #   R28 8각(0.18 x 0.12)은 rho_in 0.0600 H = 상한의 161% -> 구멍 반경 1.37pt @0.75.
    #   두께를 0.068 H 로 낮춘 **6각(끝을 깎은 납작한 커넥터 패드)**으로 바꿔 rho_in 0.0340.
    #   윗변을 y=0.098 로 올려 `Mast` 밑끝(0.54,0.09)이 판 «안»으로 박히므로 d=0(접합)이 유지된다.
    #   재현·검산: design/equipment/verify/r31_fillsolid.py
    for s in out:
        if s.name == "BasePad":
            s.pts = [(0.4400, 0.0640), (0.4700, 0.0300), (0.5900, 0.0300),
...
def miner():
    ...
    # ★ R31 수정 — 같은 이유(위 cyber 주석). R28 쐐기(뿌리폭 0.12)는 rho_in 0.0499 H = 상한의 134%
    #   -> 구멍 반경 0.76pt @0.75. 뿌리폭 0.12 -> **0.078**, 끝폭 0.05 -> **0.0356** 으로 좁히고
    #   축을 (0.4380,0.7220)->(0.5230,0.5770) 으로 옮겨 RockFace/Vein2/Crack1 여유까지 함께 벌었다.
    #   테이퍼 tau = 0.458 (<= 0.55) 이라 「광부」 잎 판정은 그대로다.
    for s in out:
        if s.name == "Vein1":
            s.pts = [(0.4716, 0.7417), (0.5384, 0.5860), (0.5076, 0.5680), (0.4044, 0.7023)]
    return out
