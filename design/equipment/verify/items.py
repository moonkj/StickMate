...
# ============================================================================
def cap():
    """0 야구모자. ★ 2026-09-03 R12 이식 1단계(HEAD) — 인계본 `clothhat` 관을 σ=1.461로 받았다.
    관 반폭 0.94 -> 1.02 R · 꼭대기 1.24 -> 1.22 R(인계본 1.2248), 그리고 곧은 띠(0.46 R)가 새로 선다.
    띠 톤은 그늘(2)이다 — 이 모자의 보조색은 챙이고 규칙 3-2는 아이템당 보조색 하나를 강제한다.
    챙은 인계본 좌표를 폐기하고 우리 현행 HatBrim을 그대로 쓴다(처방 #2, A 성분)."""
    crown = [(-1.02,-0.22), (-1.02, 0.36), (-0.72, 1.04), (0.00, 1.22),
             ( 0.72, 1.02), ( 1.02, 0.34), ( 1.02,-0.06), (0.60, 0.04), (-0.40, 0.06)]
    brim  = [(-0.40, 0.06), (0.60, 0.04), (1.36,-0.02), (1.92,-0.34),
             ( 1.22,-0.54), (0.62,-0.26), (-0.06,-0.12)]
    band  = [(-1.02,-0.02), (1.02,-0.02), (1.02, 0.44), (-1.02, 0.44)]
    return [Shape("HatCrown", crown, filled=True),
            Shape("HatBrim",  brim,  filled=True, tone=1),
            Shape("HatBand",  band,  filled=True, tone=2)]

def beanie():
    """1 털모자. ★ 2026-09-03 R12 이식 1단계 — 인계본 `furhat`을 세로 아핀 y'=0.85y-0.155로 받았다.
    부피가 머리 아래가 아니라 **옆**으로 간다: 넓은 털 단(반폭 1.05)이 y=-0.06~0.52 R,
    그 위에 좁은 관(반폭 0.57)이 y=0.56~1.44 R. 폼폼은 우리 것 그대로(꼭대기 1.72 R 불변).
    2026-09-02 처방은 유지 — 관과 단은 여전히 **한 채움**이고 접힌 자리는 그늘색 낱선 하나다.
    단 밑변을 곧게 편 것이 인계본과의 유일한 봉투 차이다(커버선은 x에 무관한 한 값이라야 한다)."""
    crown = [(-1.05,-0.06), ( 1.05,-0.06), ( 1.05, 0.52), ( 0.57, 0.56),
             ( 0.50, 1.06), ( 0.00, 1.44), (-0.50, 1.06), (-0.57, 0.56),
             (-1.05, 0.52)]
    fold  = [(-0.57, 0.56), (0.57, 0.56)]          # 관의 두 발을 그대로 받는다(규칙 4-a)
...
def fedora():
    """2 중절모. ★ 2026-09-03 R12 이식 1단계 — 인계본 `fedora` 전체 -0.35 R.
    챙이 볼록렌즈(중심 +0.19 · 반폭 1.29 · 두께 0.46 R)가 되고 관은 좁고 높아진다(반폭 0.61 · 꼭대기 1.38).
    크리스(눌린 자국)가 **채움의 윤곽선 노치**로 돌아왔다 — 관이 높아져 노치 두 변이 1.38획이 됐다.
    띠는 인계본 좌표를 폐기하고 곧은 띠 0.46 R로 다시 세운다(§5-4-1)."""
    brim  = [(-1.10, 0.15), (-0.61, 0.30), (0.19, 0.36), (0.61, 0.34),
             ( 1.48, 0.15), ( 0.99,-0.01), (0.19,-0.10), (-0.61,-0.01)]
    crown = [(-0.61, 0.30), (-0.61, 0.86), (-0.30, 1.38), (0.00, 1.01),
             ( 0.30, 1.38), ( 0.61, 0.86), ( 0.61, 0.34)]
    band  = [(-0.61, 0.30), (0.61, 0.34), (0.61, 0.80), (-0.61, 0.76)]
    return [Shape("FedoraBrim",  brim,  filled=True),
            Shape("FedoraCrown", crown, filled=True),
            Shape("FedoraBand",  band,  filled=True, tone=1)]

def crown_hat():
    """3 왕관. 얹는 물건이라 밑이 뚫린다(HatCoverLocalY = +inf). 봉우리는 점으로 수렴.
    ★ 2026-09-03 R12 이식 1단계 — 세로 아핀 y'=0.9164y-0.2025(우리 봉투 [-0.10,+1.52]가 정한 계수).
    몸이 테 위로 올라앉고(밑변 0.02 -> 0.27), 테는 인계본 19점을 폐기하고 곧은 띠(반폭 1.01)가 된다 —
    그 한 처방이 몽당변 2건과 규칙 1-C 미달을 동시에 닫는다."""
    body = [(-0.94, 0.27), (-0.84, 1.36), (-0.42, 0.82), (0.00, 1.52),
            ( 0.42, 0.82), ( 0.84, 1.36), ( 0.94, 0.27)]
    rim  = [(-1.01,-0.10), (1.01,-0.10), (1.01, 0.36), (-1.01, 0.36)]
    return [Shape("CrownBody", body, filled=True),
            Shape("CrownRim",  rim,  filled=True, tone=1)]

def beret():
    """4 베레모. 뒤로 처진 비대칭 덩어리. 밑변이 곧 테(보조색 선)."""
    body = [(-1.46,-0.10), (-1.02, 0.62), (-0.20, 1.06), (0.62, 0.90),
            ( 0.98, 0.54), ( 0.92, 0.02), (-0.30,-0.02)]
    # ★ 윗변 왼끝만 수직 압출이 아니다 — 몸통 왼쪽 변(-1.46,-0.10)->(-1.02,0.62)이 기울어 있어
    #   그대로 올리면 띠가 몸통 밖으로 나간다. 그 꼭짓점만 몸통 변 위로 물린다(x = -1.1788887R).
    rim  = [(0.92, 0.02), (-0.30,-0.02), (-1.46,-0.10),
            (-1.1788887, 0.36), (-0.30, 0.44), (0.92, 0.48)]
    return [Shape("BeretBody", body, filled=True),
            Shape("BeretRim",  rim,  filled=True, tone=1)]
...
def straw():
    ...
    band  = [(-0.86, 0.10), (0.86, 0.08), (0.86, 0.54), (-0.86, 0.56)]
    return [Shape("StrawBrim",  brim,  filled=True),
            Shape("StrawCrown", crown, filled=True),
            Shape("StrawBand",  band,  filled=True, tone=1)]
