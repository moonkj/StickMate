...
# ══════════════════════════════════════════════════════════════════════════════
# ★ 남는 머리 비율 — 2026-09-02 사용자 신고 2건
#     "털모자 착용시 거의 머리 전체를 가림"
#     "지금 모자도 과하게 머리를 덮는거 같아 ㅁ자 창때문에 이것도 대부분의 머리를 가림"
#   ★★ 이 검사가 없어서 회귀가 나갔다. 밑단을 −0.52 → −0.64로 내린 라운드는 **부작용 24항목을
#      전부 검산하고 통과**했다 — 목록에 "남는 머리"가 없었기 때문이다. 그래서 여기 넣는다.
#   하한과 그 근거는 headroom.py 한 곳에만 있다(숫자를 두 곳에 적지 않는다).
# ══════════════════════════════════════════════════════════════════════════════
import headroom as _hr
print()
print("╔══ 남는 머리 비율 (하한: 두께 %.2f획 / 면적 %.0f%%  — headroom.py) ══╗"
      % (_hr.HEADROOM_THICKNESS_FLOOR_W, _hr.HEADROOM_AREA_FLOOR * 100))
_hrfail = 0
for _s in _hr.HEADROOM_GATE_SCALES:
    _w = _hr.stroke_in_R(_s)
    _tag = " ← 사용자 실제 저장 배율" if abs(_s - 0.60) < 1e-9 else " ← 출하 기본"
    print("  [배율 %.2f  W=%.4fR]%s" % (_s, _w, _tag))
    for _n, _sh in items.HEAD.items():
        _m = _hr.measure(_sh, _w)
        _th = _m['depth'] * 2.0 / _w
        _ok = _th >= _hr.HEADROOM_THICKNESS_FLOOR_W and _m['area'] >= _hr.HEADROOM_AREA_FLOOR
        if not _ok: _hrfail += 1
        print("    %s %-6s 두께 %5.2f획  면적 %5.1f%%  외곽호 %5.1f°  잉크밑단 %+.3fR"
              % ("OK " if _ok else "✗  ", _n, _th, _m['area'] * 100, _m['arc'], _m['ink_bottom']))
if _hrfail:
    fail += _hrfail
    print("  ✗ 남는 머리 하한 위반 %d건 — 모자가 머리를 지운다(사용자 신고 그 자리다)." % _hrfail)
print("╚═══════════════════════════════════════════════════════╝")
...
print()
print("★ 총 위반 %d건 (규칙 30종 + 남는 머리)" % fail)
