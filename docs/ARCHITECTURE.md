...
> 작성: 팀 리더(아키텍트) · 최종 갱신: **2026-09-02 13:50**(`game-architect` 드리프트 정정 라운드)
>
> ★ **2026-09-02 정정 4건** — 이 문서가 실재 코드와 갈라져 있던 자리다. 문서를 믿지 말고 코드를 믿는다.
> | # | 이 문서가 적고 있던 것 | 실재 |
> |---|---|---|
> | 1 | DLC 이행 **단계 A**를 "할 일"로 | **끝났다.** `Core/AccessoryDefSO.cs` + `Resources/Items/*.asset` **42개** |
> | 2 | DLC 이행 **단계 B**를 통째로 "할 일"로 | **절반만 끝났다.** 카탈로그는 에셋을 읽지만(`ItemCatalog.cs:346` `Resources.LoadAll<AccessoryDefSO>`) **몸에 그리는 `AccessoryShapeBuilder.cs`는 여전히 2,874줄 · `case` 48개**다 |
> | 3 | 미니보스 레이드 "**사용자 확인 필요**"(4곳) | **확인이 끝났다 — 사용자가 2026-08-31 제외를 확정**(AskUserQuestion, CLAUDE.md). 다시 묻지 않는다 |
> | 4 | 세이브 "**v7 필드 예약**"을 착수 항목으로 | **지나갔다.** 다음은 v10이다 |
>
> ★ **2026-09-02 오후 — 위 4번의 후속 정정 + 순서 정본 안내**
> - 4번의 *"`CurrentVersion = 9`"* 는 **오전 실측이고 지금은 다르다.** 소스는 **10**
>   (`CharacterSaveStore.cs:109`)인데 **커밋되지 않았고**(`git show 4a5a4de:` / `7ab0468:` 모두 9)
>   **어떤 디스크에도 안 쓰였다**(사용자 세이브 `"version": 9`, mtime 갱신 중).
>   → **v10은 아직 소비되지 않은 번호다. 다음 플레이어 빌드가 그 문을 닫는다.** §5-4 A-4-b / §5-5 L-1.
> - ★ **착수 순서의 정본은 이 문서가 아니라 `docs/GAME_ARCHITECTURE_REVIEW.md` §7 확정판**이다
>   (한 장 요약 §7-7). 이 문서 §5-4는 **기술 실현성 백로그**이지 순서가 아니다.
>
> ★ **2026-09-02 오후 2차 — 사용자 확정 6건이 이 문서의 A/B/C 배치를 바꿨다**:
> DLC와 영어화가 **C(장기)에서 1.0으로 올라왔고**, 모바일이 **1.0에서 나갔다**.
> 그리고 「이벤트」 탭이 **폐지**됐다. 각 항목에 확정 표시를 달았다.
...
3. 유저 실제 파일/아이콘/창은 절대 변경 안 함 — 전부 읽기 전용 열거 + 시각적 복사본
   - ★ 승인된 예외 1건: **Windows 작업표시줄 자동 숨김**(실행 중에만 해제, 종료 시 원복,
     ...
     `docs/TASKBAR_REVEAL.md`. macOS Dock은 포함되지 않는다(같은 문서 3절에 판단 근거).
...
| ~~미니보스 레이드~~ | **제외 확정 — 착수하지 않는다**(사용자 2026-08-31 AskUserQuestion). 라이벌 서브시스템도 2026-08-30에 삭제됨 | — | — | **닫힘** |
...
- **창문-던전 파밍(큼/장기)**: 성장 인프라(`CharacterProgressionModel` 레벨/XP, `ItemCatalog`, 세이브 v6)는
  ...
  5-2-4가 선행.
- **~~미니보스 레이드~~ — ★ 제외 확정(2026-08-31 사용자, AskUserQuestion). 이 항목은 닫혔다.**
  근거는 그대로 유효하다: 2인 이상 액터가 필요한데 **라이벌 서브시스템은 2026-08-30에 사용자
  지시로 전체 삭제됐다**(커밋 `b6755f4`). 보스 레이드는 그 인프라를 다시 세우는 일이다.
  ★ **이 문서가 "확인 필요"로 적어 둔 탓에 이미 한 번 되살아난 적이 있다** — 확정된 제외를
  미결로 적어 두면 다음 기획서가 그것을 열린 문으로 읽는다. 다시 열려면 사용자 지시가 필요하다.
...
> ★ **2026-09-02 저녁 — 이 스키마에 필드 3개가 추가로 확정됐다**(세 경로 획득 설계).
> 근거·검산·되돌릴 수 없는 정도는 **`docs/GAME_ARCHITECTURE_REVIEW.md` §8**(특히 §8-1 · §8-5-a).
>
> | 필드 | 타입 | 왜 지금인가 |
> |---|---|---|
> | `cohortId` | string (`"base"` / `packId`) | **등급이 파생값이라 DLC가 붙으면 기본 42종의 등급·가격·유예가 통째로 이동한다.** 슬롯 6→12에서 전설(330동전)이 **희귀(70)**가 되고 슬롯 동전 합이 **650 → 230(−64.6%)**. 지금은 팩이 0개라 코호트=슬롯이 **항등**이라 값이 하나도 안 바뀐다 |
> | `setId` | string | 세트를 `rank`에서 파생하면 팩+기본 혼합이 **가짜 세트**로 판정된다. 세트 코드가 아직 없어 **지금 비용 0** |
> | `entitlementId` | string (비면 개별 현금 경로 없음) | 개별 유료 구매의 유일한 스팀 형태가 **아이템 1종 = DLC appid 1개**다(마이크로트랜잭션은 판매자 서버를 요구 — 1차 출처 §8-9). **DLC 팩 아이템은 이 필드를 비운다** = *"DLC 아이템은 DLC로만"*(사용자 확정)의 문법적 보장 |
>
> ★ **가격 필드를 만들지 마라.** 스팀 *Pricing* 문서: 가격은 **파트너 사이트에서 37통화·4지역군으로** 설정된다. 달러 값이 우리 코드에 있으면 지역·환율·할인이 즉시 갈라진다.
| 필드 | 타입 | 설명 |
|---|---|---|
...
| `displayName` / `description` | ~~로컬라이즈 키~~ → **실제로는 한글 원문** | ★ 2026-09-02 실측 정정. `Resources/Items/*.asset` **42개**에 `\uXXXX` 이스케이프로 **한글 84건**이 들어 있다(`displayName` 42 + `description` 42). 키가 아니다. ★ **`grep '[가-힣]' *.asset`은 0건을 답한다** — 이스케이프라서다. 지역화 감사는 **반드시 디코드 후에** 세야 한다(TEAM.md 거짓통과 #4와 같은 형태) |
...
| 단계 | 내용 | 난이도 | 회귀 잠금 |
|---|---|---|---|
| **A** ✅ **완료** | `AccessoryDefSO` 스키마 정의 + 기존 종을 .asset으로 **값 동일하게** 생성 | 중간 | 실측: `Core/AccessoryDefSO.cs` 존재, `Resources/Items/*.asset` **42개**(계획의 32개보다 많다 — 머리카락/FX/펫이 합류했다) |
| **B-1** ✅ **완료** | **카탈로그**가 에셋을 읽게 전환 | 중간 | 실측: `ItemCatalog.cs:346` `Resources.LoadAll<AccessoryDefSO>`. `new Row(...)` 28줄은 제거됨(`ItemCatalog.cs:304` 주석) |
| **B-2** ❌ **미착수 — 원칙 4의 실제 구멍** | **몸에 그리는 도형**을 `case`에서 에셋 데이터로 전환 | 중간 | ★ 실측: `Interaction/AccessoryShapeBuilder.cs` **2,874줄 · `case` 48개** → ★ **2026-09-02 저녁 재실측 2,600줄 · `case` 43개**(B-2 파일럿이 줄였다. 앞의 값은 낡았다). ★ **DLC 36종을 지금 구조로 얹으면 여기에 `case`가 36개 더 붙는다** — B-2는 「선택적 정리」가 아니라 **DLC의 하드 선결**이다(`GAME_ARCHITECTURE_REVIEW.md` §8-8-d). 에셋의 `icon` 필드는 **인벤토리 아이콘**만 담는다 — 몸에 걸치는 형상은 여전히 코드다. **같은 아이템에 진실이 둘**(§1-D) |
| **C** | 팩 매니페스트 + Addressables 도입 + 6종 팩 제작 | 큼 | 팩 미설치/부분설치/구버전 팩 3종 로드 테스트 |
- **원칙 4를 실제로 지키기 시작하는 시점은 단계 B가 끝나는 순간이다.**
  ★ **2026-09-02 실측: 그 순간은 아직 오지 않았다.** B-1(카탈로그)만 끝났고 B-2(조형)가 남아서,
  **신규 장비는 지금도 `AccessoryShapeBuilder.cs`를 열어 `case`를 더해야 추가된다.**
  즉 원칙 4는 **모션·이펙트에는 성립하고 장비에는 성립하지 않는다** — 원칙이 장비만 비껴가 있다.
...
> ## ★ 2026-09-02 오후 — 이 절(5-4)은 **기술 실현성 백로그**이고, **착수 순서가 아니다**
>
> 리더 방향 교정(*"큰부분부터 구현하고 정밀화"*) 이후, **실제 착수 순서의 정본은
> `docs/GAME_ARCHITECTURE_REVIEW.md` §7**이다(한 장 요약은 §7-7).
> 아래 A/B/C/D는 그 순서에 **재료를 대는 목록**으로 읽는다.
>
> **그리고 아래 표는 이미 두 번 낡았다** — 그게 이 저장소가 반복해 온 사고라서 상태를 표에 박는다.
> 항목 옆의 ✅/❌는 2026-09-02 오후에 **코드를 열어** 확인한 것이다(문서 주장 아님).
...
| 4 | 세이브 원자적 쓰기 | 5-1-4 | 작음 | ✅ **착지.** `WriteAtomically`가 임시파일 `Flush(true)` → 교체 직전 버전 확인 → 재시도 사다리 → **실패 시 직전 세대 대피 후** 덮어쓰기. 사용자 디스크에 `stickmate_character.prev.json` 실재 |
| **4-b** | **v10 필드 묶음 확정** | 5-1-10 | 작음 | ❌ **미착수 · 시간에 민감.** ★ **2026-09-02 오후 2차 — 필드 하나가 개명돼야 한다**: 내가 제안한 `ownedItemIds`가 `docs/security/SECURITY_MODEL.md` §3-2의 감사 금지 토큰(`owned`)과 정면 충돌한다. → **`purchasedItemIds`로 개명**(B층 = 동전 구매만). C층(유료 소유)은 **세이브에 넣지 않는다**. 근거·3층 분리: REVIEW §7-11. **지금은 필드가 없어 개명 비용 0, 출하 뒤엔 마이그레이션이다.** `CurrentVersion = 10`은 **작업 트리에만** 있다(커밋 `4a5a4de`/`7ab0468` 모두 9). 사용자 세이브는 여전히 `"version": 9`. → **v10은 아직 소비되지 않은 번호이고, 다음 플레이어 빌드 한 번이 그 문을 닫는다.** 근거·후보 필드·유일한 함정(`itemReachedAtSeconds` 기산점): **REVIEW §7-1** |
...
| **9** ★신규 | **화면 공유·발표 중 자동 회피 (S1 판정 2등급 분할)** | `docs/SCREEN_SHARE_DETECTION.md` 4절 | **작음** | ⚠️ **절반 착지 · 출시 블로커.** `MILESTONES.md` A절 실측 "비침해 70% — 등급 2분할의 **플랫폼 절반만** 완료, **소비자 8파일 미착수**". 남은 것은 소비자 배선이다. 새 네이티브 **0줄** · 새 권한 **0개** · 새 P/Invoke **0개**. 클릭 방해 **100%** 제거. **이 백로그 전체에서 비용 대비 효과가 가장 크다** (REVIEW §7-2-b 순서 1번) |
| **10** ★신규 | **등급 리본 + 스탯 4줄 (표면만)** | `design/systems/ECONOMY_SPEC.md` 2·3절 | 작음~중간 | ❌ 미착수. ★ **저장 필드 0개** — 등급은 요구 레벨 순위에서, 스탯은 레벨+착용에서 **파생**한다. 그래서 **v10 결정을 안 기다린다.** 등급 색 4단은 `design/art/PALETTE_SPEC.md` §4에서 확정(`#9C978C`/`#BCAC8B`/`#DBBD7F`/`#F9CB70`). ★ 단 **리본 요소는 우리 카드에 아직 없다**(`BuildCard` = Surface+Outline+Thumb+Name+Meta+Action) |
| **12** ★신규 | **설정창 5 → 4탭 (「이벤트」 폐지 반영)** | B-17 | **작음** | ❌ 미착수. 리더 판정은 났고 **코드는 아직 5탭**이다. 비용 = 정의 4곳 + 테스트 1파일. ★ **세이브 안전**(탭은 저장되지 않는다). ★ **11번과 같은 파일**(`SettingsWindow.cs`)이므로 **한 라운드로 묶어라** |
| **11** ★신규 | **설정창 「접근성·성능」 탭 — 저전력 30/45/60 세그먼트 하나** | `SettingsWindow.PlaceholderLabel(Tab.Accessibility)`가 이미 약속함 | 작음 | ❌ 미착수. `Platform/FramePacing.cs`가 이미 등급 체계를 갖고 있어 **새 인프라 0**. 빈 3탭 중 **의존성 없이 지금 채울 수 있는 유일한 탭** (「데이터」는 재화 뒤 / 「이벤트」는 사용자 결정 대기 — REVIEW §7-2-c, §7-3-c) |

### B. 다음 (2~4주) — 콘텐츠 1차
9. `IdleActivityDirector` 공용 스케줄러 + 저위험 유휴 4종(그림자 인형극 / 커서 쓰다듬기 / 아이콘 닦기 / 숨바꼭질) — 중간
10. **DLC 스키마 단계 B의 남은 절반(B-2)** — ★ **A는 끝났고 B는 절반 끝났다**(위 정정표 1·2).
    남은 것은 **`AccessoryShapeBuilder`의 `case` 48개를 에셋 `shape` 데이터로 내리는 일** 하나다.
    `hidesHair`는 이미 스키마에 있어 Major 4는 해결됐고, `stateBinding`/`unlockRule`/`packId`/
    `pointColorTargets`/`facingAsymmetric` **5개 필드는 아직 `AccessoryDefSO`에 없다** — 중간
    ★ **2026-09-02 오후 재실측**: `AccessoryShapeBuilder.cs` **2,874줄 · `case` 48개 · `switch` 8개**
    (`default:`는 1개 → **9개**로 늘었다 = S1 안전망 착지). `AccessoryDefSO` 공개 필드 8개
    (`itemId`/`slot`/`itemIndex`/`displayName`/`description`/`requiredLevel`/`hidesHair`/`icon`)
    — **`shape` 없음**이 확정. ★ **착수 시점은 사용자 결정 D-1(원칙 4 개방성)에 갈린다**:
    봉인이면 "DLC 6팩 조형 직전"이면 되고(1.0 안 막음), 개방이면 **임포트 검증기가 함께 필요**해져
    범위가 커진다(**규모 미확인** — 세려면 `internal const` 190개를 「자」/「좌표」로 전수 분류해야 함).
    상세 두 갈래: `docs/GAME_ARCHITECTURE_REVIEW.md` §7-5
...
15. ★신규 **재화 → 상점** — `ECONOMY_SPEC` 4절이 수치를 전부 확정했고 프로덕션은 **0줄**이다
    (양성 대조 붙여 재측정: `coinBalance`/`Purchase`/`Currency` 0파일 vs `EquipmentSlot` 18파일).
    간극 = **저장 필드 7 + 지급 훅 2 + 화면 3**, 새 튜닝 상수 0개.
    지급 훅은 이미 단일 지점으로 존재한다 — `Interaction/FocusWatchDirector.CompleteSession()` /
    `CharacterStatsModel.ArcheryBullseyes` 기록 지점. **선행: A-4-b(v10 확정)** — 중간
16. ★신규 **설정창 「데이터」 탭** — MVP는 「저장 파일 위치」 한 줄. ★ **「초기화」는 재화 뒤**다:
    지금 만들면 지우는 것이 XP/레벨(저절로 복구)이고, 재화 이후엔 **다시 못 버는 것**이 된다.
    성질이 바뀌기 전에 설계된 확인 절차가 그대로 남는 것이 이 저장소가 겪은 사고의 형태다
    (REVIEW §7-2-c) — 작음
17. ~~**설정창 「이벤트」 탭**~~ — ★ **폐지 확정**(리더 판정 2026-09-02, 사용자 위임).
    설정창이 **5 → 4탭**이 된다. 근거: 그 탭이 약속한 *"자동 발동 개별 토글"*의 대상
    `*Chance` **7개가 전부 `= 0f`**이고 툴팁 7개가 *"2026-08-29 기본 OFF … 총평 '제대로 동작하는게
    하나도 없음'"*을 달고 있다 — **스위치를 만드는 것은 품질 문제를 설정으로 떠넘기는 것**이다.
    ★ **코드는 아직 5탭이다**: `SettingsWindow.cs:176` `enum Tab`에 `Event = 2`가 그대로 있고
    `:178 TabCount = 5` · `:180 TabNames` 5개 · `:1360 PlaceholderLabel(Tab.Event)`가 남아 있다.
    **되돌리기 비용 실측 = 정의 4곳 + `Tab.Event`를 이름으로 참조하는 테스트 1파일**
    (`Tests/PlayMode/SettingsWindowReturnPathTests.cs` — enum 멤버가 사라지면 **컴파일이 깨진다**).
    ★ **안전 확인 완료**: `SettingsWindow.Tab`은 **어디에도 저장되지 않는다**(마지막 탭 기억 없음).
    즉 enum 값을 지워도 **세이브가 밀리지 않는다** — `EquipmentSlot`이 겪은 그 사고가 여기엔 없다

### C. 장기 로드맵 (분기 단위 · 외부 선결 조건 있음)
18. **DLC 단계 C** — Addressables + 팩 매니페스트 + 6종 팩 제작 (큼).
    ★ **1.0 포함 확정**(사용자 2026-09-02) — 따라서 **이 절이 아니라 1.0 항목이다.**
    ★ **봉인 확정으로 차단 해제**: 제3자 임포트 검증기가 불필요해져 착수 가능.
    ★ 트랙 분해(조형 설계 T1 / B-2 T2 / 매니페스트 T3 / 엔타이틀먼트 T4)와
    **가장 긴 시계가 기술이 아니라 조형 설계라는 판단**: `GAME_ARCHITECTURE_REVIEW.md` §7-9.
    ★ 선행 **플러그인 레지스트리 신설** — 2026-09-02 오후 재확인: `MotionPluginSO`/`EffectPluginSO`는
    **프로덕션 소비자 0곳 · `.asset` 인스턴스 0개**(참조는 주석과 테스트뿐). 즉 "구현"이 아니라
    **인프라 신설**이다. ★ 그리고 사용자 결정 **D-4(오디오)**가 안 닫히면 매니페스트의 `sounds`
    필드를 넣을지가 안 정해진다 — 나중에 넣으면 `packVersion`/`minAppSchemaVersion` 재발행이다
19. 자동 업데이트 (유통 채널 확정 후) — 중간
20. 옵트인 크래시 리포트 (스토어 출시 후) — 중간
21. Steam Cloud Auto-Cloud (appid 확보 후) — 작음, 대기 중
22. 스크린리더 호환 (UniWindowController 창 소유권 선결) — 중간
23. 인질극(`IWindowChromeGeometryService` 신설 필요) — 중간
24. 스케치북 무기 / 창문-던전 파밍 — 큼
25. ~~미니보스 레이드~~ — **제외 확정(2026-08-31 사용자). 로드맵에서 닫혔다.**
26. Steam P2P 친구 방문 — 장기, 오프라인 원칙 체계 확립 후
27. **모바일 스크린샷 백드롭 모드 (iPad/iPhone)** — ★ **1.0에서 제외 확정**(사용자 2026-09-02: 추후).
    4플랫폼 목표의 남은 한 축이고 **착수 0**.
    실측(`Tests/EditMode/PlatformParityAuditTests.cs` · `dev-platform`): `Platform/`는 65파일 21,572줄이고
    iOS에서 **통째로 빠지는 것이 9,204줄(42.7%)**. 모바일 코드는
    `Mobile/ScreenshotBackdropPlatformService.cs` **110줄 1개**뿐이며, 그 설정 API 3종
    (`SetBackdropScreenshot`/`AddUserDefinedFoothold`/`IsConfigured`)은 **프로덕션 호출부 0건** —
    유일한 참조는 `StickmanAgent.CreatePlatformService()`의 `new` 한 줄이다.
    **컴파일 갭은 아니다**(iOS 크로스 컴파일 errors=0, 양성 대조 2건 통과).
    ★ **이번 라운드의 모바일 결정 1건**: 발판 좌표 + 백드롭 참조는 **저장돼야 하므로**
    A-4-b(v10)에 **필드 자리만 예약**한다(선언 2줄, 값은 비움 = JsonUtility가 null).
    안 하면 모바일 라운드가 v11을 확정한다.
    ★ **리더 판정 1건은 1.0 밖으로 나가 급하지 않아졌다**(모바일 라운드가 열릴 때 함께 판정):
    `docs/UX_FLOW.md` 351행(*"발판 미지정 → 화면 최하단 고정 발판으로
    즉시 대체"*) vs `CreatePlatformService()`가 모바일만 `FallbackPlatformWindowService`로
    **안 감싸는 것**(주석: *"빈 결과는 버그가 아니라 의도된 신호"*). 바닥 안전망이 그 래퍼 안에 있어
    **지금 설계대로면 모바일에 안전망이 없다.** 둘 중 하나가 틀렸다
28. **영어화(R-2)** — ★ **1.0 포함 확정**(사용자 2026-09-02). 따라서 **이 절(C. 장기 로드맵)이 아니라
    1.0 필수 항목이다** — 위치는 `GAME_ARCHITECTURE_REVIEW.md` §7-2-b의 순서 2·3·13번.
    `product-strategy` `ROADMAP.md` §3-2가 **정식 출시 조건**으로 올린 항목이고
    **내 1차 순서에 없었다**(그쪽 §4-2 지적이 맞다 — 내 렌즈는 "기술 막힘"이라 매출 막힘이 안 보인다).
    ★ 그러나 **문자열 테이블만으로는 부족하다**: `Dialogue/DialogueKind.cs:82,98`의
    `PerGlyphSeconds`가 `text.Length`(UTF-16 코드 유닛)를 곱하는데 **한글에서만 글자=음절**이다.
    검산 — `헉... 높다`(3음절/7자) 0.805초 요구 vs `Whoa, that's high`(3음절/17자) **1.555초** 요구
    (**1.93배**, 상한은 `MaxSeconds/MinSeconds = 3.55배`). 그리고 이 값은 **발화 자격 게이트**라
    (*"말할 시간이 없으면 애초에 말하지 않는다"*) **영어만 침묵한다.**
    → 번역보다 **단위 수정이 먼저**다. 착수 여부는 사용자 결정 **D-5(영어 출시를 1.0에 넣는가)**.
    ★ **번역 분모가 세 문서에서 다르다(최대 약 15배)** — 최신 실측을 쓴다:
    **`docs/localization/verify/ship.out.txt`(15:29) = 출하 표면 `.cs` 424 + `.asset` 84 ≈ 508건.**
    (`tier.out.txt` 15:25는 586, `ROADMAP.md` §3-3은 7,968 — 7,968은 **부채가 아니라 리터럴 총량**이고
    툴팁·로그·주석을 포함한다. 일정 근거로 쓰면 크게 과대다.)
    ★ 부채 계산 주의: `product-strategy`의 한글 리터럴 **7,968건은 `.cs`만** 센 값이다.
    `Resources/Items/*.asset` **42개에 `\uXXXX` 이스케이프로 한글 문자열 84개**(`displayName` 42 +
    `description` 42, 코드포인트 669)가 더 있고 `grep '[가-힣]' *.asset`은 **0건**을 답한다
    (양성 대조: `equip_head_cap.asset` → `"천모자"` = 천모자 ✔)
...
4. ~~**[삭제 결정 충돌] 미니보스 레이드**~~ — **닫힘. 사용자가 2026-08-31에 제외를 확정했다**(AskUserQuestion).
...
### ★ 2026-09-02 오후 추가 — 순서를 막고 있는 결정 (정본: `GAME_ARCHITECTURE_REVIEW.md` §7-6)

**리더가 지금 내려야 하는 것 1건 (시간에 민감)**

> ★ **2026-09-02 오후 2차 — 이 판정은 「채택됐지만 코드에 없다」.**
> 리더가 되돌리기를 채택했고 `docs/MILESTONES.md`가 "✅ 해결됨"으로 적었으나,
> **`CharacterSaveStore.cs:109`는 여전히 `CurrentVersion = 10`이다**(mtime 15:09 실측).
> 문서가 닫아 놓은 미이행 항목이라 **아무도 다시 보지 않는다** — 아래 권고는 그대로 유효하다.
> 되돌릴 곳 3군데와 대안 2가지: `GAME_ARCHITECTURE_REVIEW.md` §7-6-a.

> **L-1 — v10을 지금 소비할 것인가. 권고: 소비하지 않는다.**
> `CurrentVersion = 10`은 **작업 트리에만** 있고(커밋 `4a5a4de`/`7ab0468` 모두 9), 사용자 세이브는
> 여전히 `"version": 9`다. 그 트리로 **플레이어를 한 번 빌드해 실행하면** 모니터 필드만 든 v10이
> 디스크에 앉고, 재화·스탯·모바일은 **v11·v12·v13**이 된다 = 하위 호환 테스트 3배.
> **문을 닫는 것은 커밋이 아니라 빌드다.** 그리고 이 개발 머신이 곧 사용자 머신이다.
>
> 함께 못박을 것 — **v10 라운드 체크리스트**:
> *"새 필드마다 「이 키가 없을 때의 값이 정확한 사실인가」를 한 줄로 적는다. 못 적으면
> 그 필드가 버전을 강제하는 필드다."* 후보 8종 중 그 줄을 못 적는 것은
> **`itemReachedAtSeconds` 하나뿐**이다(0으로 채우면 `companionSeconds` 21.4시간짜리 기존
> 사용자에게 **일반·희귀가 첫 실행에서 한꺼번에 자동 해금**되어 상점이 텅 빈 채로 열린다 —
> 처방과 하위 호환 테스트 내용은 REVIEW §7-1-c).

**★ 2026-09-02 사용자 확정 — 6건 중 4건이 닫혔다**

| # | 결정 | 결과 | 따라오는 것 |
|---|---|---|---|
| ~~D-1~~ | 원칙 4 개방성 | **✅ 봉인(1st-party 전용)** | 제3자 임포트 검증기 **불필요** → **1.0을 안 막는다.** B-2는 **그래도 한다**(결함 범주 소멸) |
| ~~D-5~~ | 영어 출시를 1.0에 | **✅ 넣는다** | `PerGlyphSeconds` **단위 수정이 선행**(안 고치면 서술 대사가 영어에서 침묵) |
| ~~MS 스토어~~ | 1.0에 넣는가 | **✅ 제외. 스팀 단일** | MSIX 불필요 · **코드서명 「필수」→「선택」**(백신 오탐 완화 가치는 남는다) |
| ~~모바일~~ | 1.0에 넣는가 | **✅ 추후** | 바닥 안전망 판정도 1.0 밖. ★ **단 v10 필드 예약 2줄은 지금** |
| ~~DLC~~ | 1.0에 넣는가 | **✅ 포함** | ★ **분모가 늘었다.** B-2가 「해도 좋은 것」에서 **선행 조건**이 됐다 |
| ~~D-6~~ | 「이벤트」 탭 | **✅ 리더 판정 — 만들지 않는다** | 설정창 5 → 4탭. ★ **코드는 아직 5탭이다** |

**남은 사용자 결정 3건** — 상세는 REVIEW §7-6.

| # | 결정 | 막히는 것 | 등급 |
|---|---|---|---|
| **D-4** | 오디오를 켤 것인가 | ★ **DLC 편입으로 급해졌다.** 팩 매니페스트의 `sounds` 필드가 여기 매달리고, **매니페스트는 세이브와 같은 종류의 되돌릴 수 없는 계약**이다(팩이 `packVersion`을 들고 다닌다). 나중에 넣으면 **출하된 팩 6개 전부 재발행**. **순서 8번(팩 매니페스트) 시작 전에 답이 있어야 한다.** ★ 실측: 오디오 프로덕션 파일 **0개**(히트는 테스트 1파일뿐) = 켜면 **인프라 신설** | **P1 ⬆** |
| D-2 | 유료 본편 vs F2P | 6팩의 품질 등급(F2P면 유일 매출 경로의 첫인상). ★ MS 스토어 제외로 **영향 축소** | P2 |
| D-3 | 매출원 vs 인지도 자산 | D-2의 상위 질문 + 가격 전체 | P2 |
