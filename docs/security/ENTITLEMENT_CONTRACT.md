...
### ★ E-6-c 시행 규칙 — **감사가 무엇을 세는가** (2026-09-07 개정, `security`)

초판 감사는 §E-6-c를 **「프로덕션 전체의 public 테스트 강제값 개수 ≤ 1」**이라는 대리 지표로 구현했다.
**그 대리 지표는 규범과 어긋났고, 어긋남이 양방향으로 사고를 냈다.**

| | 무슨 일이 있었나 |
|---|---|
| **과탐** | 밧줄등반 라운드가 QA 스위치 2종(`RopeClimbQaOverride`)을 신설하자 3건이 되어 감사가 빨개졌다. 그 라운드는 이를 *"무관한 기존결함"*으로 보고했고 **리더 실측으로 그 보고가 틀렸음이 확인됐다**(원인 파일이 정확히 그 라운드가 만든 것이었다). 그러나 **감사 쪽도 옳지 않았다** — 이 두 스위치는 §E-6-c가 겨눈 대상이 아니다 |
| **미탐** | 초판 탐지기는 줄에 `(`가 있어야만 셌다. 그래서 **자기 가족 안에서도** 괄호 없는 공개 속성 `AmbientCalendarClock.WallClockOverrideForTesting`을 통째로 못 봤다. 실측하면 그 가족의 public 수는 3이 아니라 **4**였다 — **상한만 3으로 올렸다면 이 미탐을 그대로 안고 갔다** |

**개정 규칙 — 상한을 올리지 않는다. 계기를 규범에 맞춘다.**

> **E-6-c-1 (본론, 0 허용):** **C층에 도달하는** public 테스트 강제값은 **0건**이어야 한다.
>   「도달」은 세 방향으로 판정한다 —
>   **D0** 강제값이 C층 파일 안에 있다 / **D1** C층 파일이 그 강제값의 선언 타입을 참조한다(§E-6-b의 뜻) /
>   **D2** 그 파일이 C층 정책 타입을 참조한다.
> **E-6-c-2 (습관, 집합 등호):** 좁은 가족(이름에 `Test`와 `Override`가 함께)의 public 목록은
>   **개수 상한이 아니라 집합 등호**로 잠근다. 개수는 **맞바꿈**(무해한 것을 빼고 위험한 것을 넣기)도
>   **사라짐**(부재 단언이 조용히 초록)도 못 본다.
> **E-6-c-3 (무엇을 「강제값」으로 세는가):** **쓰기만 센다.**
>   메서드 · **공개 setter가 있는** 속성 · 가변 필드는 세고,
>   `const` · 식 본문 getter(`=>`) · `private set` 속성은 **세지 않는다.**
>   ★ `public const`는 테스트가 프로덕션 상수를 숫자로 베끼지 않게 하는 **권장** 형태다(CLAUDE.md 규칙).
>   그것을 강제값으로 세면 감사가 올바른 처방을 벌주게 되고, 그런 감사는 몇 번 만에 꺼진다.

**실측 (2026-09-07, 프로덕션 258파일 — 오프라인 실행. 러너 확인은 별도)**

| 항목 | 값 |
|---|---:|
| C층 파일 | 3 (`PackEntitlement.cs` · `StickPackManifestSO.cs` · `SteamPackEntitlementSource.cs`) |
| 테스트 강제값 선언 | 63 → 64 (그중 public 45 → **46**) ※ |
| **C층 도달 위반** | **0** |
| 좁은 가족 public | 4 (`RopeClimbQaOverride` ×2 · `StickMateDevTools` · `AmbientCalendarClock`) |

※ **넓은 가족은 계속 는다** — 이 개정 도중에도 다른 라운드가 `CrispText.ResetInstanceCountersForTest`를
추가해 63 → 64로 움직였다. 그래서 감사는 이 숫자를 **상한으로 잠그지 않는다**(그러면 남의 라운드를
벌주는 감사가 된다). 이 숫자가 하는 일은 «스캐너가 눈이 멀었는가»를 재는 **바닥값 20**의 근거뿐이다.
**좁은 가족(4건)만 집합 등호로 잠근다** — 이쪽은 실제로 잘 늘지 않는다.

**밧줄등반 QA 스위치 2종의 도달 범위(코드 추적)**: `ChanceOverride` → 배회 AI 로프 추첨 확률,
`TestWallEnabled` → 발판 목록에 합성 벽 1개. **두 종점 어디에서도 XP·동전·아이템 보유·엔타이틀먼트로
가는 화살표가 없다**(밧줄 등반은 어떤 재화도 지급하지 않는다 — XP 지급처는 패시브·활쏘기·집중모드 셋뿐).
⇒ **C층 강제값이 아니다. `internal` 전환은 불필요하고, PlayMode가 `InternalsVisibleTo` 대상이 아닌
구조를 깨는 대가만 남는다.**

★ **판정이 뒤집히는 조건 2개**(적어 두지 않으면 다음 사람이 모른다):
(가) 밧줄 등반이 XP·동전·아이템 해금 중 하나라도 지급하게 되는 날,
(나) 이 프로세스가 **제3자 어셈블리를 적재**하게 되는 날(원칙 4 플러그인 통로가 **코드까지** 열리는 경우).
오늘 (나)는 성립하지 않는다 — 프로덕션에 `Assembly.Load` 계열 **0건**이고 `Activator.CreateInstance`
3건은 전부 문서화된 셸 COM 조회다(양성 대조: 같은 패턴이 그 3건을 실제로 찾아냈다).
...
| `Tests/EditMode/UnlockSwitchScopeAuditTests.cs` | #2. 선언 파일 밖 참조 **파일 집합 등호** + 멤버는 `UnlockAll`만 + **C층 도달 강제값 0건**(§E-6-c-1) + **좁은 가족 public 집합 등호**(§E-6-c-2). ★ 2026-09-07 개정 전에는 마지막 항목이 «public 개수 ≤ 1» 래칫이었다 — 과탐 1건·미탐 1건을 동시에 냈고 그 기록은 §E-6-c 시행 규칙에 있다 |
...
## E-13. ★ 신규(2026-09-03) — **소모성 유료 아이템은 이 계약과 원리적으로 충돌한다**

`SECURITY_MODEL.md` **T절**(시계 조작 방어)의 후속. 기존 E-1~E-12는 한 줄도 고치지 않았다.

배경: 사용자 확정으로 **유휴 수급 / 오프라인 보상 / 회복제(하루 1개 무료 + 나머지 실화폐 판매)**가
설계 중이다. **그 마지막 항목이 C층 계약을 정면으로 건드린다.**

### E-13-1. 왜 충돌하는가 — E-4(실행 간 캐시 금지)의 직접 귀결

E-4의 논거는 *"C층을 세이브에서 뺀 목적은 방어 추가가 아니라 **표적 제거**"*였다.

| | **영구 해금**(durable add-on) | **소모품**(consumable) |
|---|---|---|
| 남은 개수를 어딘가에 써야 하나 | **아니오** | **예 — 필연적으로** |
| 그 저장소 | 없음(매 실행 스토어 조회 + 메모리) | **평문 세이브**(`stickmate_character.json`) |
| E-4 준수 | ✅ | ❌ **표적이 되돌아온다** |
| E-8-c(세이브 → 엔타이틀먼트 화살표 없음) | ✅ | ❌ **잔량이 곧 그 화살표다** |
| 오프라인 동작 | E-1~E-3으로 무해화 완료 | 스토어 재고 조회 필요 — **미확인** |
| MS 스토어 | §5-2에서 durable이 **정확히 우리 모양**이라고 확인 | 재검토 필요 |

★ **핵심**: 소모품 잔량은 **평문 한 줄이고 상한이 없다.** 위조하면 유료 SKU가 **무한**이 된다.
E-8이 B층 위조를 「그냥 둔다」고 판정할 수 있었던 이유는 *"위조해도 잃는 것은 자기 경험뿐"*이었는데,
**소모성 유료 아이템은 그 전제를 깬다.**

### E-13-2. 권고 — 파는 것을 「개수」가 아니라 **「요율」**로 바꾼다

> **E-13-a (권고, 판정은 리더·`product-strategy`):** 회복제를 개당 팔지 말고
> **「매일 회복제 +N개」 같은 영구 해금**으로 판다.
> 그러면 팔리는 것이 **소모품이 아니라 요율**이고, E-1~E-5 · §3-1 C층 · §5-2가
> **한 줄도 안 바뀌고 그대로 적용된다.** 새 계약 조항이 필요 없다.
>
> **E-13-b (필수, 어느 형태를 택하든):** 유료로 산 요율/개수는 **`purchasedItemIds`(B층)에 들어가지 않는다.**
> E-8-c 재확인 — 세이브에서 엔타이틀먼트로 가는 화살표가 코드에 있으면 그것이 곧 크랙이다.
>
> **E-13-c (필수):** 무료 1개/일은 **C층이 아니다.** T-5의 단일 일일 예산 래칫에 태운다.
> 별도 카운터를 만들면 시계 조작 상금이 곱해진다(`SECURITY_MODEL` T-D-3).

### E-13-3. 미확인 (E-12에 추가)

9. **Steam Inventory Service(소모품 API)의 오프라인 동작 미확인.** 스팀 실기가 없다.
   E-13-1의 판정은 *"미확인이라 위험하다"*이지 *"불가능하다"*가 아니다.
   소모품을 팔기로 하면 **실기 확인이 선행돼야 한다.**

### E-13-4. ★ 2026-09-03 개정 — **사용자가 재고형을 선택했다. E-13-a는 폐기, E-13-1의 표는 유지**

사용자 확정: *"회복제를 하루에 한개 무료제공 **하루 넘어가면 없어짐**. 유료로 회복제 구매할수 있고
하루에 재화 1500개 획득 회복제 사용시 한도 500추가 하루에 총 1000개 추가가능 즉 하루에 회복제사용은 2개까지만"*

- **E-13-a(요율 영구해금 권고)는 폐기한다.** 사용자에게 위험을 고지한 뒤 내려진 결정이다.
- **E-13-1의 대조표는 유지한다** — 소모품이 durable보다 구조적으로 취약하다는 사실은 결정과 무관하게 참이다.
- ★ **다만 사용자 결정의 한 조각이 그 취약성을 대부분 상쇄한다: 「하루 넘어가면 없어짐」.**
  이월이 없으면 **누적 재고 필드가 존재하지 않고**, 그러면 E-4가 지키려던 「표적 제거」가
  소모품에서도 성립한다. → **`SECURITY_MODEL` T-14-5-a.**

> **E-13-d (필수, T-14-5-a와 같은 조항):** **유료로 산 회복제도 당일 소멸이어야 한다.**
> 유료분만 이월되면 `paidPotionStock`이 평문 세이브에 앉고 **`9999`가 유료 SKU 무한이 된다.**
> **E-13-e (권고):** 미소비 재고는 **스토어가 보관**한다(우리 파일에 안 쓴다). 오프라인 동작은 **미확인**.
> **E-13-f (필수·순서):** 소비는 **① 우리 상한 +500 → ② 스토어 소비 통지**. 반대로 하면
> 중간 실패 시 **유저가 돈 낸 것을 잃는다**(§4-1-4 오탐 비대칭). ①②의 실패는 개발자 손해 500동전이고
> `potionsUsedToday ≤ 2` clamp로 **하루 2,500에서 유한하게 멈춘다.**
> **E-13-g:** 오프라인이면 **유료 구매만 막고 무료 1개는 정상 동작**. 문구는 **E-2-a**를 따른다
> (*"안 샀습니다"* ✗ / *"지금 확인할 수 없습니다"* ✓).

**피해 상한(정량, `SECURITY_MODEL` T-14-7)**: 동전 소모처가 4,550으로 유한하므로
**시계 조작으로 잃는 최대 매출 = 사용자 1인당 유료 회복제 약 3개.**
★ **단 「동전으로 사는 반복 구매품」이 생기면 이 상한이 무한이 된다**(T-14-7-a / T-D-12).
