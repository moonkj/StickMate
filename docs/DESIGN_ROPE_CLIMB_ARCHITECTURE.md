...
| `Ascend` | **공중**(밧줄에 매달려 오르는 중, 발판에서 완전히 이탈) | **즉시 Fall**(★ 2026-09-08 정정: `TryGetFootholdTopWorldYCoveringX` — 핸들 존재 **+ 밧줄이 걸린 앵커 x를 아직 덮는가**) | 아래 정정 문단 참고 |

> ★★ **2026-09-08 정정(debugger)** — 이 칸은 원래 «`ParkourClimbState.Tick()`과 100% 동일 규칙,
> `TryGetFootholdTopWorldY` 재확인 실패 시»였다. **그 «100% 동일»이 실제 결함을 만들었다.**
> 사용자 신고: *"줄타고 올라가다가 창을 치워도 계속 줄타고 올라감 -> 떨어져야함"*.
> `TryGetFootholdTopWorldY`는 «핸들이 목록에 있는가»만 묻는데 **창 핸들은 창을 옮겨도 바뀌지 않는다**
> (실측: macOS `kCGWindowNumber`는 700pt 이동에도 불변, 닫아야 사라짐 / Windows는 `HWND`로 동일).
> 그래서 «닫기»만 취소되고 «치우기»는 취소되지 않았다 — **바로 아래 문단이 이미 «닫히거나 이동해
> 갈고리가 헛돎»이라고 옳게 적어 둔 그 사건이 구현에서 절반만 성립하고 있었다.**
> 손 등반은 손이 창틀을 잡고 있어 *일부러* 움직이는 창을 따라가므로 규칙이 같을 수 없다.
...
`Enter()`/`Tick()` 각각에서 재확인하는 **지점**은 `ParkourClimbState.Tick()`의 대응 지점과 동일한
자리에 두면 된다 — **단, 실패 시 분기는 페이즈로 나누고**(`_phase == Throw ? Idle/Walk : Fall`),
★ 2026-09-08부터 **묻는 질문 자체도 다르다**: `TryGetFootholdTopWorldY`(핸들만)가 아니라
`TryGetFootholdTopWorldYCoveringX(wallHandle, 앵커x, ...)`다. 이유는 위 정정 문단.
...
## 11. 2026-09-08 실측 — 시뮬레이션(§9-3)이 빗나간 이유와 `ropeClimbChance` 0.20 → 0.85

### 11-1. 사용자 신고

> "여전히 줄던져서 벽타는 행동은 안하는데"

§9-4-A는 0.20을 넣으면 **3분 이내 관측확률 ~94%**라고 예고했다. 사용자는 그 예고를 만족하지 못했다.

### 11-2. 두 갈래 실험으로 원인을 갈랐다

배포 빌드(macOS, 2026-09-08 07:05 재빌드)에 `STICKMATE_QA_ROPE_CLIMB_CHANCE=1.0`을 걸어
**확률을 방정식에서 제거하고 지형만** 쟀다. 결과:

| 관측 | 값 |
|---|---|
| 밧줄등반 결정 → Throw → Ascend → 완료 | **4단계 전부 정상** (실기 캡처로 육안 확인) |
| 벽 탐색 성공(=기회) 빈도 | 515초에 6회 = **평균 86초에 1회** |

즉 **트리거 체인도 상태기계도 렌더러도 고장나 있지 않았다.** 확률을 1.0으로 줘도 86초가 천장이다.
0.20을 곱하면 체감 주기가 **7분대**가 된다 — 사용자가 "안 한다"고 느낀 것이 정확했다.

### 11-3. 왜 시뮬레이션은 63초를 예측했나 — 전제가 지형을 지웠다

§9-3 본문이 스스로 이렇게 적어 두었다:

> "벽은 한쪽 경계에 고정 배치, 반대쪽은 평범한 반전 경계로 가정 — 즉 9-1의 지형 문제는
> **'해결됐다'고 가정한 이상적 케이스**다"

그 가정 아래에서는 **모든 다리(leg)가 곧 기회**다. 실제 데스크톱은 그렇지 않다. 측정 당시 지형:

| 발판 | 딛는 면(Dock, OS y=907) 대비 높이 | 로프 대역 (1.94 ~ 11.26유닛) |
|---|---:|---|
| 왼쪽 Finder (OS x 0~410, 상단 y=677) | **5.62유닛** | ✅ 화면 통틀어 유일한 유효 벽 |
| 오른쪽 Finder (OS x 976~1512, 상단 y=292) | 15.03유닛 | ❌ 상한 11.26 초과 |
| Dock ↔ 안전망 턱 | 1.64유닛 | ❌ 파쿠르 상한 1.94 미만(손으로 오름) |

유효한 벽이 **한 개**뿐이고 그것도 Dock **왼쪽 끝**에 있는데, 캐릭터는 대부분의 시간을 오른쪽에서
Dock↔안전망 왕복(뛰어내리기↔되올라가기)으로 보낸다. 그래서 기회 자체가 86초에 한 번이다.

**교훈: §9-3 표는 "지형이 있다면"의 상한이지 체감 주기가 아니다.** 두 수를 같은 것으로 읽은 것이
2차 라운드의 오류였고, 표 자체는 틀리지 않았다(전제를 명시해 두었다) — 전제를 지운 채 인용한 쪽이
틀렸다.

### 11-4. 사용자가 지정한 목표와 산식

> "창이 저렇게 올라갈수 있는 조건이면 2분에 한번정도의 빈도로는 올라가야함"

목표는 **유효한 벽이 도달 범위에 있을 때 120초에 1회**다("~조건이면"이 그 전제다).

측정은 **두 단**으로 했다(모델로 정하지 않는다 — 그게 2차의 실패였다).

| 측정 | 조건 | 결과 | 95% 신뢰구간 |
|---|---|---:|---|
| ① | `STICKMATE_QA_ROPE_CLIMB_CHANCE=1.0` (확률 제거, 지형만) | 515초 / 6회 = **86초에 1회** | 48 ~ 430초 |
| ② | 배포 기본값 `ropeClimbChance=0.60` (오버라이드 없음) | 1201초 / 6회 = **200초에 1회** | 111 ~ 1002초 |

②에서 단순 반비례로 외삽하면 목표 120초에는 `c = 1.00`이 필요하다. 그러나 그건 **추첨을 없애는
것**이라 창이 빽빽한 데스크톱에서는 경계마다 밧줄을 던지게 된다. **`c = 0.85`** 는 외삽 141초로
"2분 정도"에 들면서 15%는 그냥 지나쳐 그 도배를 막는다. 값이 `stepUpChance`와 같은 것은 우연이
아니라 **같은 뜻**이어서다 — "기회가 오면 대체로 잡지만 늘 그렇지는 않다".

★ **측정 정밀도의 한계를 숨기지 않는다.** 6회 표본의 신뢰구간은 위 표대로 넓고, 두 점으로
`T(c) = t_e/c + t_c`를 연립하면 `t_e = 145.5초, t_c = −59.5초`로 **`t_c`가 음수**가 나온다 — 모델이
데이터와 맞지 않는다는 뜻이다(측정 사이에 창 배치가 달라졌을 가능성이 가장 크다. ①과 ② 사이에
20분이 있었다). 그래서 0.85는 "정밀하게 푼 해"가 아니라 **목표 구간에 드는 가장 방어 가능한
선택**이다. 더 조이려면 **창 배치를 고정한 채 30분 이상** 재측정할 것.

### 11-5. 범위 밖으로 남긴 것 — 대역 상한

오른쪽 Finder(15.03유닛 = 8.17H)는 `ropeClimbMaxHeights`(6.6H)를 넘어 탈락했다. 이걸 8.5H 정도로
올리면 캐릭터가 **대부분의 시간을 보내는 오른쪽 경계**가 통째로 기회가 되어 빈도가 크게 오른다.
다만 그건 "얼마나 자주"가 아니라 **"무엇을 오를 수 있는가"**를 바꾸는 별개의 디자인 결정이고
(8.17H는 0.88H/s로 9.3초짜리 등반이라 연출 길이도 함께 바뀐다), 이번 지시의 범위가 아니라
**의도적으로 손대지 않았다.** 필요하면 `design-systems`/`design-motion` 라운드로 올린다.

### 11-6. 플랫폼 영향

**Windows 영향: 없음.** 변경은 `StickConfig`의 직렬화 수치 1개와 배포 에셋 1줄뿐이고, 두 파일 모두
플랫폼 중립이다(`#if UNITY_STANDALONE_*` 분기 없음). 소비자
`AutoWanderController`/`StickmanBlackboard`/`GroundSensor` 역시 플랫폼 분기가 없다. 다만 **기회
빈도는 창 배치에 종속**되므로 Windows에서의 체감 주기는 그쪽 지형으로 별도 측정해야 한다 —
이 라운드가 잰 86초는 macOS 실측이다.

---

## 12. 2026-09-08 (2차) — 높이 상한을 「신장 배수」에서 「화면 비례」로

### 12-1. 사용자 지적

> "6.6배는 너무 작은거 같아 화면자체가 캐릭터보다 많이 큰데 6.6배면 너무 작은거 같은데
> **화면 해상도에 따라 계산해서 나눠야할거같은데**"

### 12-2. 실측이 지적을 뒷받침했다

배포 기본 배율(macOS 1512×982pt) 기준:

| 항목 | 월드유닛 | 신장 배수 |
|---|---:|---:|
| 화면 높이 | 23.99 | 13.0 H |
| 발밑(Dock) → 화면 클램프 상단 = **실제 도달범위** | 22.16 | **12.1 H** |
| 옛 상한 `ropeClimbMaxHeights` 6.6H | 12.14 | 6.6 H → **도달범위의 55%** |
| 오른쪽 Finder 창 | 15.03 | 8.17 H → **탈락 중이었다** |

★ **해상도 의존이 잘못된 방향으로 걸려 있었다.** 고정 배수는 화면이 커질수록(= 도달범위가 더 많은
H를 담을수록) 커버율이 **떨어진다**. 같은 캐릭터 배율로 4K 모니터에 올리면 6.6H는 더 작은 조각이
된다. 11절이 "지형이 원인"이라고 밝힌 그 지형 문제의 **절반은 이 상한이 만들고 있었다** —
11-5에서 범위 밖으로 미뤄 뒀던 항목이 여기서 해소된다.

### 12-3. 새 공식

```
실질 상한 = min( 도달범위 × ropeClimbMaxScreenFraction ,  ropeClimbMaxHeights × H )
              └─ "화면 어디까지"                          └─ "몇 초까지"
```

- **`ropeClimbMaxScreenFraction` = 0.90** (신설, 주 상한). 도달범위의 90%. 남긴 10%는 화면 최상단에
  딱 붙은 창을 목표로 삼아 매달린 채 화면 밖으로 밀려나는 것을 막는 여유다 — 화면 클램프가 이미
  상단을 자르지만 **그 경계에 정확히 닿는 목표는 프레임 간 판정이 갈릴 수 있고**, 그것이 이 저장소가
  겪은 "진입 직후 목표 벽 소실" 사고의 형태다(§9-4-D `rearLimitX` 문단과 같은 논지).
- **`ropeClimbMaxHeights` = 6.6 → 13.6** (역할 변경: 주 상한 → **소요시간 천장**).
  새 속도 1.36 H/s에서 13.6H = **10.0초**다. 배포 기준 화면에서는 화면 비례 쪽(10.9H)이 먼저 걸려
  binding 되지 않는다 — **훨씬 큰 모니터에서만 발동하는 안전핀**이다.
- **`ropeClimbSpeedHeightsPerSecond` = 0.88 → 1.36** (사용자 승인). 상한이 6.6H → 10.9H로 오르면서
  옛 속도로는 그 등반이 **12.4초**가 된다. 상주 앱에서 캐릭터가 그만큼 다른 행동을 못 하는 것은
  너무 길다는 판단이고, 1.36 H/s면 10.9H가 **8.0초**다.

| | 이전 | 이후 |
|---|---:|---:|
| 실효 상한(배포 기준 화면) | 6.6 H (도달범위 55%) | **10.9 H (90%)** |
| 오른쪽 Finder 8.17H | ❌ 탈락 | **✅ 후보** |
| 최대 등반 소요 | 7.5초 | **8.0초** |

높이는 65% 늘었는데 소요시간은 거의 그대로다.

### 12-4. 정직하게 남기는 손실 — 파쿠르 경계 연속성이 깨진다

옛 0.88은 임의 값이 아니라 **경계 연속성에서 역산한 값**이었다(§8-2):
`stepUpMaxHeights(1.0551) / parkourClimbDuration(1.20초) = 0.88 H/s`. 파쿠르가 자기 상한을 오르는
실효 속도와 같게 두면, 파쿠르 상한 **바로 위**에서 시작하는 첫 밧줄 등반이 하한 경계에서 갑자기
빨라지지 않는다.

**1.36으로 올리면 그 연속성이 깨진다** — 이제 경계에서 밧줄이 파쿠르보다 약 **1.55배** 빠르다.
"등반이 너무 오래 끌지 않는다"를 그 연속성보다 우선한 결과다.

★ **되돌릴 근거가 생기면 속도를 되돌리는 대신 `ropeClimbMaxScreenFraction`을 낮추는 쪽을 먼저
검토할 것** — 그러면 연속성과 소요시간을 동시에 살릴 수 있다(상한이 낮아지면 옛 속도로도 등반이
짧게 끝난다). 속도와 `ropeClimbMaxHeights`는 **짝**이다(둘의 몫이 초 단위 천장이므로 한쪽만 바꾸면
천장의 의미가 조용히 달라진다).

### 12-5. Windows에 대한 가설 — 같은 뿌리일 가능성

사용자 신고: *"윈도우에서는 밧줄 동작안함"*. 소스 조사로 확인한 사실:

1. **`Win32WindowService`는 `IDockMetricsService`를 구현하지 않는다**(macOS 전용). Windows에는
   작업표시줄 발판이 없어 캐릭터가 **화면 최하단 안전망**에서 걷는다. 그러면 같은 창이라도
   **발밑에서 더 높게** 측정되므로 옛 6.6H 상한을 넘기기가 macOS보다 쉽다.
2. 밧줄 경로(`RopeClimbState` · `GroundSensor` · `StickmanBlackboard` · `AutoWanderController` ·
   `RopeClimbRenderer` · `FallbackPlatformWindowService`)에 **플랫폼 분기가 0건**이다 — 로직은
   양쪽이 같고, 갈리는 것은 **입력(발판 목록)**뿐이다.

⇒ 12-3의 화면 비례 상한은 도달범위 기준이므로 **발밑이 더 아래여도 커버율이 유지된다**. Windows
증상이 이것으로 해소될 가능성이 높다.

★ **다만 확증하지 못했다.** `[발판리포트]`는 `Platform/MacOS/MacOverlayStateEnforcer.cs`에만 있고
**Windows에는 대응물이 없다** — 원격으로 "그 화면에 어떤 발판이 있었는가"를 볼 수단이 없다.
리더가 계기 추가를 제안했으나 사용자가 *"지금은 두고 밧줄부터 확인"*으로 판단해 **이월**한다.
다음 Windows 신고가 또 올라오면 **이 계기부터 넣고 시작할 것** — 이 라운드의 진단이 소스 추론에
머문 이유가 정확히 이것이다.
