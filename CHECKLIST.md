...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [x] coder 착지 완료 — 색 배분 목표 근접(최대 오차 1.9%p, W68%→20.4%로 정정),
        머리제거 일반화(채도 급락 자동탐지), 검산은 전부 남의 계기로 재측정(design-
        equipment `bitmap_anchor_r1.py` 7항 전부 통과, design-character `M()`을
        파일에서 직접 떼어 실행 — 평탄면 3, 원본은 8). EditMode 2,954건 실패 0(회귀
        2건은 기능 문제 아님 — P0 관문이 P1 데이터 등장으로 자연 진화, 그중 1건은
        "선언 키 유무"가 아니라 "값"으로 판정 방식 자체를 고쳐야 했던 부수 발견 —
        나머지 41종도 같은 함정 가능성 있어 qa-regression 확인 요청 등재). xcheck
        osx/win 5/5 errors=0. 실기 캡처 완료(저해상도 디스플레이가 이 머신에 없어
        Retina만 확인, 정직하게 보고됨).
    - [x] ★ 리더 지시 정정(coder가 실측으로 반박, 수용) — "머리 경계 아크 위/아래로
          자르라"는 지시대로 하면 밴드가 통째로 머리 뒤로 사라짐(실측 확인). 대신
          design-equipment 규격서 §4-4의 "내용 기준"(안쪽 면이 실제로 노출되는
          덩어리만 뒤판)을 자동 판정으로 구현 — 겹침 최소 0.367R(요구 ≥0.20R).
    - [x] 리더 육안 확인 — 실기 캡처 직접 열람, ★ **카드/상세 패널이 여전히 옛 벡터
          아이콘이고 착용만 새 비트맵**임을 확인(coder도 자체 소견에서 지적함 —
          이번 라운드 범위 밖이라 안 건드렸으나 "부위별로 화법이 다르다"는 오늘
          하루 종일의 핵심 불만이 형태만 바뀌어 재발). **마저 닫기로 판단** —
          카드용 파생본(같은 화풍, 별도 해상도)을 굽고 크라운의 `cardIconOverride`도
          같은 라운드에서 배선하는 후속 작업 배정.
  - [x] coder 마무리 완료 — 카드 파생본 배선(옛 벡터 카드 프레임 그대로 재사용,
        제자리 교체). 배경은 지시(#15181E 불투명)와 다르게 **알파 보존**으로
        감(카드가 앉는 판 색이 보유/착용중/잠김/상세 4개 상태마다 달라, 불투명이면
        셋에서 네모 타일이 됨 — 실측 대조 후 판단, `--card-bg` 인자로 되돌릴 길은
        열어둠). 커밋 `7a442fe`.
    - [x] ★ "머리사이즈와 왕관사이즈가 안맞음" 원인 규명 완료 — **배선 버그
          아니었음.** 스케일 계산 자체는 정확했다(실측 1.125배, 스펙 계산
          1.1188배로 0.5% 일치). **진짜 원인**: 옛 벡터 왕관은 색 본체가 머리
          지름과 정확히 1.0009배였는데, 잉크색 윤곽선이 검은 머리와 시각적으로
          붙어버려서 실루엣(1.1326배)보다 작아 보이는 착시가 있었다. 새 비트맵은
          윤곽선이 없어 실루엣=색 본체라 그 착시가 사라지고 진짜 배율(1.125배)이
          그대로 드러난 것 — **목표치 자체가 옛 착시를 반영 안 하고 있었다.**
          잉크 반폭을 머리 실제 시각반경(1.17193R) 기준으로 재계산해 교정,
          재실측 1.000배로 정확히 일치.
    - [x] EditMode 2,956건 실패 0(교정 전후 2회), xcheck osx/win 각 5/5
          errors=0(2회), PackCardBitmapIconTests 6→9건(뮤테이션 검증). Windows
          영향 없음(데이터+주석뿐).
    - [x] 실기 확인 — 카드·상세·착용·슬롯줄 **4표면 일치** 확인(같은 프레임
          캡처). ★ **5번째 표면(정보창 초상화 PREVIEW)은 아직 옛 벡터** —
          `CharacterPortraitStage.DrawAccessories`에 비트맵 갈래가 없어 같은
          창 안에서 초상화와 카드가 다른 왕관을 그림. 다른 렌더러라 이번
          라운드 범위 밖, 후속 배정 필요.
    - [ ] 후속 배정(급하지 않음) — design-equipment: 사양서(`pack_bitmap_authoring_spec_v1.md`
          §5-2)의 왕관 목표치가 이번 교정으로 낡음, 갱신 필요. "색본체÷머리지름
          =1.00"을 HEAD 6종 공통 판정식으로 승격할지 판단(나머지 5종도 같은
          착시 함정 가능). coder: 정보창 초상화 렌더러에 비트맵 갈래 추가.
  - [x] G1 최종 판정 수령 — **"왕관 디자인은 기존이 낫다.."** 기술적으로는 전부
        통과(화법 조건·크기·4표면 일치)했지만 **미감 판정에서 기각**. 리더가
        되돌릴지 물었더니 **"일단 중단하고 기다려줘"** — 세 선택지(전체 중단/
        다른 아이템 재시도/보류) 중 어느 것도 그대로 고르지 않고 **지금은 아무
        조치도 취하지 말고 대기**를 택함.
  - [x] ★★★ **전체 중단 — 다음 사용자 지시까지 이 스레드의 모든 후속 작업
        정지.** 왕관은 **되돌리지 않고 현재(비트맵) 상태로 그대로 둠**(사용자가
        "되돌려라"가 아니라 "중단하고 기다려라"라고 했으므로, 지시 없이 되돌리는
        것도 하지 않음). 미해결로 남은 것: 초상화 표면 1건(벡터), design-equipment
        사양서 낡음, coder의 정보창 렌더러 배정 — **전부 사용자가 재개 지시할 때까지
        보류.** 엔진(P0 부착 시스템)은 42종 전부 필드가 비어 있어 다른 41종엔
        영향 없음.
