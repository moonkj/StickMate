...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [ ] game-architect 배정 — 이 대규모 전환의 착수 순서·리스크·"이전 것 보존" 계약을
        설계. 특히 SHOULDERS 계열 흔들림 손실 문제(다관절 분할 스프라이트가 필요한지, 단순
        정지 이미지로 갈지)와, 파일럿 규모(전체 42종 한 번에 vs DLC 12종으로 먼저
        검증 후 확장)를 판정할 것.
  - [ ] coder 배정 — 뼈대 부착 렌더링 컴포넌트 신설(병행 착수 가능, 최종 그림 없이도
        구조는 지금 있는 DLC 비트맵으로 우선 검증). **기존 벡터 `wornShapes`/`icon`은
        절대 안 지운다** — 새 필드는 `cardIconOverride`와 같은 널러블 오버라이드
        패턴으로 추가해, 아이템별로 또는 전체적으로 언제든 벡터로 되돌릴 수 있게 한다.
  - [ ] 사용자 배정 — HEAD/EYES/NECK/SHOULDERS/HAIR 30종 + DLC 12종 = 42종을 "평면·
        투명배경" 프롬프트로 재생성(리더가 정확한 한글 아이템명·설명 목록을 정리해 전달
        예정).
