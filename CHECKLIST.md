...
- [ ] **밧줄등반 중 창 소실 시 낙하 안 함** (2026-09-08) — "줄타고 올라가다가 창을 치워도
      계속 줄타고 올라감 -> 떨어져야함". 리더가 직접 코드 확인: `RopeClimbState.cs`
      클래스 문서와 `TickAscend()`(약 406행)에 "목표 벽 소실 시 즉시 Fall" 로직이 **이미
      존재하는 것처럼 보이는데도** 사용자가 재현 결함을 신고 — 원인 불명확이라 debugger에
      가설 기반 규명 배정함(착수함, 아래 진행 로그).
...
- [ ] **"병렬로 전부 진행"** (2026-09-08) — 남아있던 모든 대기 항목을 한 번에 병렬 착수하라는
  ...
  - [x] **Track 7(coder-systems) 완료 — 단 착지는 안 함(리더 결재 대기 후 착지 라운드
      ...
      - [ ] **착지 라운드 필요(다음 배정)**: 파킹된 에셋을 실제로 트리에 넣고 위 판정
            반영해 26건(코호트 한정 재정의, 기계적) + 5건(실결함 — Patched Hood 실루엣이
            베레모와 10%만 다름[하한 15%]/Eyes 아이콘 넘침[조형 축소 필요]/팔레트 게이트
            모집단 재정의/보조색 교체/번역 래칫) 총 15개 테스트 파일 수정.
