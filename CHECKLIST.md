...
- [ ] **광부·대마법사 팩도 사이버펑크처럼 실구매 배선** (2026-09-08) — "Dlc팩 다른것들도
      있잖아". 코스튬 매니페스트(`sourceKind: Pack`)와 채움 조형 수정은 3종(사이버/광부/
      대마법사) 전부 이미 완료돼 있으나, **실제 구매 가능하게 만드는 배선(팩 아이템 6종+
      `StickPackManifestSO`+회귀 게이트)은 사이버펑크만** 됐음(그게 최초 팩이라 파일럿으로
      먼저 뚫었음, game-architect §18-3 계획대로). 이제 인프라(`BaseCohortScope.cs`,
      팔레트 게이트 모집단 좁힘, 번역 래칫 분리)가 갖춰졌으니 광부·대마법사는 "에셋만
      추가"로 훨씬 싸게 될 것으로 예상 — 착수함(아래 진행 로그).
