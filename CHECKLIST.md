...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [ ] coder 배정(P0, 그림 0장으로 지금 착수 가능) — 필드+병렬표 신설(E1) /
        `_sprites` 5곳 배선+flipX(E2, 원칙2 위험 해소 포함) / 알파박스 굽기+임포트
        프리셋(E3) / 절차적 교정 스프라이트+테스트 6건(E4) / `PlatformParityAuditTests`
        항목 추가(E5) / `AccessoryDefSO.cs:667-674` 주석 갱신(같은 커밋에서). E1은
        42종 전부 빈 칸이라 착지 직후 회귀 0건이 정상 — 변화가 있으면 E1이 틀린 것.
  - [ ] P1 파일럿(왕관 1종, 그림 필요) — 저작 규격서 확정 후 착수. 게이트:
        G0(교정 스프라이트 수치 일치) → **G1(왕관 1종, 배율 0.35/0.60/0.75/1.00 ×
