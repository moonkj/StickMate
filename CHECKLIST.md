...
- [ ] **DLC 12종 착용 품질 — R4도 불만족, 보류 중** (2026-09-09) — R4 캡처 전송 후 사용자
      판정: **"전체적으로 전부 다 마음에 안듬"**. R2·R3·R4 세 차례 재작업에도 만족선을
      못 넘김 — 구체적으로 무엇이 문제인지는 아직 안 받음. "일단 윈도우용 릴리즈
      업로드해줘"로 이 사안은 잠시 보류하고 릴리즈 게시부터 진행하라는 지시. **다음에
      사용자가 다시 꺼내면**: 지금까지 시도한 방향(비트맵 색맞춤 R3 → 조형 정제 R4)이
      전부 "벡터를 다듬어 비트맵/중절모에 가깝게"였다는 공통점이 있음 — 다른 축(예:
      실제로 비트맵을 몸에 붙이는 방법, 처음에 리스크 설명 후 사용자가 보류했던 옵션)을
      다시 검토할 필요가 있는지 원점에서 물어볼 것.
