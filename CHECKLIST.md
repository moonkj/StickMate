...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [ ] P1 파일럿(왕관 1종, 그림 필요) — design-character 스타일 성립 판정 대기 중
        (진행 중, design-art의 "윤곽선으로는 해결 불가" 제약 전달 완료). 게이트:
        G0(교정 스프라이트 수치 일치, 완료) → **G1(왕관 1종, 배율 0.35/0.60/0.75/1.00 ×
        저해상도 1080p + Retina, 카드·초상·몸 3표면, 사용자 눈으로 판정 — 배율 0.35
        필수)** → G2(HEAD 6) → G3(18종+망토 재확인) → G4(HAIR→PET/FX 카드→DLC).
