...
- [ ] **장비 디자인 고도화** (2026-09-08) — "각 장비들의 디자인 고도화가 필요함 게임
      디자이너 채용해서 고도화진행해줘". 방금 캡처한 DLC 12종 스크린샷을 보고 나온
      지시로 추정(단순한 도형으로 보임) — 정확한 범위(DLC 12종만인지, 기존 42종
      포함 전체인지)는 문맥상 DLC 신규분 위주로 해석. `design-equipment`가 이미 이
      역할(장비·소품 조형 전담)로 팀에 있어 신규 채용 대신 그쪽에 배정(팀원 재확인
      메모: 장비 디자인은 Fable 모델 사용). 착수함(아래 진행 로그).
