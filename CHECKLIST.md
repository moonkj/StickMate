...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [ ] 다음 — 4갈래(game-architect/coder/design-equipment/design-art) + design-character
        전부 완료. **저작 규격서 최종본 종합 + 왕관 파일럿 프롬프트 작성**(리더),
        사용자에게 전달 후 그림 생성 시작. 게이트: G0(완료) → **G1(왕관 1종, 배율
        0.35/0.60/0.75/1.00 × 저해상도 1080p + Retina, 카드·초상·몸 3표면, 사용자
        눈으로 판정 — 배율 0.35 필수)** → G2(HEAD 6) → G3(18종+망토 재확인) →
        G4(HAIR→PET/FX 카드→DLC).
