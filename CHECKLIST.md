...
- [x] **밧줄등반 목표 선정 랜덤화 + 배회 거리 화면비례 수정**
  ...
  - [ ] ★ **Windows 릴리즈 게시(zip/태그/GitHub Releases 업로드)는 이 세션에서 막혀 있다** — 이
        환경에 `gh` CLI가 설치돼 있지 않고(`brew`도 없음), 저장된 인증정보 파일을 직접 열람해
        우회하는 것은 자격증명 접근이라 판단해 시도하지 않았다(에이전트 권한 분류기가 실제로
        차단함). 과거 `windows-preview-2026090*` 릴리즈들은 `gh`가 있던 다른 실행 환경에서
        게시된 것으로 보인다. **사용자 확인 필요**: (가) 사용자 쪽에서 `gh auth login` 후 이
        머신에 `gh`를 설치해 주면 이어서 진행 / (나) 로컬 zip만 만들어 두면 사용자가 직접
        업로드 / (다) 당분간 릴리즈 게시는 보류하고 로컬 커밋·빌드까지만 유지. 아래 신규 2건
        (종이비행기 궤도 + 대사 4건 교체)도 같은 문제라 함께 보류 중.
