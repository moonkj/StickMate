...
- [ ] **DLC 12종 착용 품질을 중절모 수준으로 재작업(R4)** (2026-09-09) — R3 캡처를 본 사용자
  ...
  - [x] 리더 조사 완료 — `equip_head_fedora.asset` 실측: `wornShapes` **0개**(기본 42종 중
        NECK 6종 제외 전부 그렇다, `AccessoryShapeBuilder.cs:1469,1652` 문서 확인). 즉 카드용
        `icon`(3조각·26점, 남색 몸통 2조각+주황 밴드 1조각) **하나만 있고 그게 카드와 착용
        모습에 동시에 쓰인다** — 두 표현이 애초에 하나라서 구조적으로 100% 일치한다. 반면
        오늘 만든 팩 12종은 카드=비트맵(별도 그림)·착용=`wornShapes`(별도 벡터, 평균 7.4조각·
        72점, R2→R3에서 계속 늘려온 방향)로 **처음부터 두 벌**이라 아무리 다듬어도 구조적으로
        "다른 두 그림"이다. ★ 가설: R2→R3가 비트맵을 흉내내려 조각을 계속 늘린 것(3.17→6.5→
        7.4)이 오히려 중절모의 "적은 조각·확실한 실루엣" 미덕과 반대 방향이었을 수 있다
        (대마법사 4종 동시 착용 캡처가 초록 덩어리로 뭉쳐 보였던 것이 그 증상일 가능성).
  - [ ] design-equipment 배정(R4) — 중절모를 조형 미덕의 기준으로 삼아 팩 12종 착용 벡터를
        재작업. 비트맵 색 맞추기보다 **적은 조각으로 확실하게 읽히는 실루엣**을 우선.
