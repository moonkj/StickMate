...
- [ ] **DLC 12종 렌더링 아이콘 제작(참고 이미지 스타일)** (2026-09-08) — 사용자가 그라데이션·
  ...
  - [x] 12종 전부 제작 완료 — 팩별 팔레트(사이버 `#009682`/광부 `#C96F00`/대마법사
        `#00992E`, 기존 확정 색상값 그대로 재사용) + 재사용 가능한 그라데이션·글로우·
        보석·아웃라인 코드 키트(`design/equipment/pack_render_icons/icon_kit.py`)로
        512×512 PNG 12장 제작. 이 게임 사상 최초의 래스터 이미지 에셋(기존 100% 벡터
        선 도형). 몽타주로 전체 확인 후 사용자에게 전송 완료.
        `Assets/_Project/Resources/Items/Icons/`에 배치.
