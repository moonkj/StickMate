...
- [ ] **★★ 장비 전체를 비트맵 스프라이트 부착 방식으로 전환(대규모, 진행 중)** (2026-09-09) —
  ...
  - [ ] coder 배정(마무리, 진행 중) — 크라운 카드용 파생본 굽기(같은 화풍·팔레트,
        해상도만 카드 소비처에 맞게 별도) + `cardIconOverride` 배선 + 재캡처로
        카드·상세·착용 3표면 전부 새 화풍으로 일치하는지 확인.
    - [x] ★ 사용자 실기 확인 후 새 신고 — **"머리사이즈와 왕관사이즈가 안맞음"**.
          리더가 캡처를 직접 잘라서 육안 동의(크라운 폭이 머리보다 눈에 띄게
          넓어 보임). 그런데 사양서 숫자(잉크박스 폭 2.622R vs 머리 실제지름
          2.344R)로는 **12%밖에 안 차이나야 하는데 실측은 그보다 훨씬 커 보임**
          — 스펙 계산과 실제 렌더링 사이 괴리 의심. 진행 중인 coder 라운드에
          같이 배정(같은 왕관 에셋 작업 중이라 라운드 통합) — 잉크박스가 아니라
          캔버스 전체 rect가 실수로 스케일 기준에 쓰이고 있는지 `CharacterAccessoryRenderer.cs`
          배선 추적, 아니면 실기 픽셀 실측으로 K/머리반경 상수 자체를 재검증하도록
          지시.
