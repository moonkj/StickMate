...
- [ ] **DLC 12종 착용 모습을 카드 비트맵에 더 가깝게 재설계** (2026-09-08) — 맥 실행 확인 중
  ...
  - [ ] coder-systems 배정 — R3 12종 착지 + F-2(underBack) + M-2(아케인 색상각 150°,
        동기화 4곳) + EditMode 검증 + macOS 실기 재캡처(카드-착용 나란히 비교)로 확정.
  - [ ] 후속 배정(급하지 않음) — F-3: 팔레트 게이트가 CIE76을 써서 이 색 영역에서
        CIEDE2000 대비 ΔE를 약 2.4배 부풀림(design-art 배정 권고).
