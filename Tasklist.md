...
## 2026-08-31 — 구석 호버 패널(크기 다이얼 + 미리보기 카드) + 런타임 크기 변경 **[Coder]**

리더 배정 3단계 전부 구현. **커밋 없음**(리더 통합용).

| 항목 | 상태 | 산출물 |
|---|---|---|
| ① `parkourMantleInset` 유도값 전환(다이얼 선행조건 5항) | **완료** | `Core/DockGeometry.ResolveParkourMantleInset`, `States/StickmanBlackboard.ParkourMantleInsetWorld`, `States/ParkourClimbState`, `Core/StickConfig.parkourMantleInsetDerived` |
| ② 캐릭터 크기 런타임 적용(원자 5단계) | **완료** | `Core/StickmanAgent.ApplyCharacterScale()` + 선행 결함 3건 동반 수정 |
| ③ 구석 호버 패널 + 크기 다이얼 + 미리보기 카드 | **완료** | `Interaction/CornerHoverPanel.cs`, `Interaction/SizeDialWidget.cs`, `Interaction/UiChrome` 유리 프리미티브 3종 |

### 실행 숫자 (실제 배치 실행)

| 실행 | 결과 | 로그 |
|---|---|---|
| 컴파일 | `error CS` 0 / `warning CS` 0 | `Logs/coder_final_compile.log` |
| EditMode | **147 / 147** (기준선 143 → +4) | `Logs/coder_em_final.xml` |
| PlayMode | **270 / 270** (기준선 253 → +17) | `Logs/coder_pm_final3.xml` |
| 배포 빌드 | `Succeeded, 에러 0 / 경고 0` (103MB) | `Logs/coder_build.log` |
| 증거 PNG | COLLAPSED / EXPANDED / 다이얼 1.60× 조작 후 | `Logs/evidence_20260831_coder_corner/` |

---

### ① `parkourMantleInset` — 고정 상수 → 유도값 (배율 상한 1.125 제거)

`ResolveEdgeStopDistance` / `ResolveStepUpMaxHeight`와 **정확히 같은 형태**다: 설정값은 하한이고
물리 실측이 더 큰 값을 요구하면 실측이 이긴다.

```
맨틀 인셋 = max(설정값 0.60, 경계 판정 거리 + 여유)
여유      = max(0.10, 물리 반폭 × 0.25)
```

**여유를 절대값 하나로 두지 않은 이유** — 덮어야 하는 세 가지 중 둘이 배율에 비례한다:
- 유휴 "주위 살피기" 머리 이동 = 신장 × 0.035 = **0.199 × 물리 반폭**
- 30fps 한 프레임 보행 이동 = `ResolveWalkSpeed`/30 = **0.208 × 물리 반폭**
- 좌표 왕복 오차 0.02 + 명확한 여유 0.05 = **절대값**(0.10이 덮는다)

비율 항 0.25는 위 두 값 중 큰 것(0.208)보다 20% 여유다.

**실측 결과(전 구간 여유)** — `Logs/coder_pm_final3.xml`의 `[SCALE-RUNTIME]` 로그:

| 배율 | 물리 반폭 | 경계 판정 | 맨틀 인셋 | 여유 | 누가 이겼나 |
|---|---|---|---|---|---|
| 0.35 | 0.140 | 0.300 | 0.600 | 0.300 | 설정값 |
| **0.75(배포)** | 0.300 | 0.400 | **0.600** | 0.200 | 설정값 — **화면 거동 무변경** |
| 1.00 | 0.400 | 0.500 | 0.600 | 0.100 | 설정값(경계) |
| 1.50 | 0.600 | 0.700 | 0.850 | 0.150 | **유도** |
| 2.00 | 0.800 | 0.900 | 1.100 | 0.200 | **유도** |

→ 유도가 실제로 이기기 시작하는 배율은 **0.875**부터다. 배포 배율에서는 한 픽셀도 바뀌지 않는다.

#### 막혔던 지점(네거티브 컨트롤)을 어떻게 풀었나 — 이월 사유 그대로 해결

`EdgeHopDownTests`의 살아 있는 네거티브 컨트롤이 `parkourMantleInset = 0.25`를 **강제**해 옛 회귀를
재현하는데, 유도를 걸면 0.25가 조용히 0.505로 되올라가 **그 컨트롤이 소리 없이 죽는다**(이 프로젝트가
R3-M1에서 이미 겪은 "값의 정확성과 소비자가 그 값을 읽는지는 별개" 유형).

해결: `StickConfig.parkourMantleInsetDerived`(기본 true)라는 **네거티브 컨트롤 전용 스위치**를 신설했다.
`postClimbDescendCooldown = 0`과 정확히 같은 성격이며 **선례가 있는 방식**이다. 그리고
**컨트롤이 또 조용히 죽지 않도록 컨트롤의 컨트롤을 붙였다**:

- 신규 `EdgeHopDownTests.NegativeControl_LegacyMantleInsetIsActuallyRestoredOnlyWhenDerivationIsOff`
  — 같은 소비 경로(`bb.ParkourMantleInsetWorld`)로 "유도 켜면 0.25가 안 남고, 끄면 정확히 0.25가 남는다"를 단언.
- 신규 `WanderEdgeConfigInvariantTests.맨틀_인셋_유도가_켜져_있어야_한다`
  — 배포 에셋에서 스위치가 켜져 있는지 잠근다(`postClimbDescendCooldown > 0` 테스트와 같은 성격).
- 교체 `DockGeometryInvariantTests.맨틀_인셋이_모든_배율에서_경계_밴드를_넘어서야_한다`(8배율)
  + 신규 `네거티브컨트롤_유도를_끄면_큰_배율에서_맨틀_인셋이_경계에_먹힌다`
  (옛 "천장 1.125 기록" 테스트를 대체 — 이제 기록이 아니라 **단언**이다).

---

### ② 런타임 크기 변경 — `StickmanAgent.ApplyCharacterScale()` (한 프레임 원자 5단계)

디버거가 확정한 순서 그대로: `root.localScale = v / 구워진 배율` → `config.characterScale = v` →
`metrics.Remeasure()` → 전 LineRenderer 두께 재대입(2.0pt 하한) → 시각 반폭 즉시 재측정.
**`Rigidbody2D.mass`는 일부러 건드리지 않았다**(안 따라가는 덕분에 랙돌 임계가 배율 불변).

#### ★★ 선행 결함 3건 — "이미 해소됨"이라던 배정 전제와 **실제가 달랐다**(리더 확인 요망)

배정문은 선행조건 5개 중 4개가 오늘 이미 해소됐다고 했으나, 코드를 읽어 보니
**액세서리 이중 스케일 Blocker와 LineRenderer 두께 Major는 손대지 않은 상태**였다
(`grep -rn lossyScale Assets/_Project/Scripts` = 5줄, 전부 옛 위치 그대로). 그래서 이번 라운드에 함께 고쳤다.

| # | 결함 | 수정 | 실측 증거 |
|---|---|---|---|
| **Blocker** | 액세서리 이중 스케일(s²) — 컨테이너가 루트의 자식인데 좌표는 이미 월드 배율이 곱해진 `StickmanMetrics`에서 온다 | 컨테이너 `localScale = 1/루트배율`로 **상쇄**(디버거가 제안한 두 안 중 (b)) | 배율 2.00에서 컨테이너 월드 스케일 **1.00000** / 상쇄를 끄면 모자 꼭대기가 **2.664배**(≈ 루트 스케일 2.6667)로 떠오름 |
| **Major** | LineRenderer 두께가 Transform 스케일을 안 따라감 | 배율 적용 시 `startWidth/endWidth` 재대입 + 화면상 2.0pt 하한 | 배율 0.35→2.00에서 두께가 실제로 커지고, 어느 배율에서도 하한 아래로 안 내려감 |
| **Major** | `ResolveWalkSpeed()`가 런타임 `lossyScale` 무시 | `config.characterScale`을 같은 프레임에 함께 대입 | 전 배율에서 `walkSpeed × 배율`과 정확히 일치 |
| **Minor** | 시각 반폭 0.25초 주기 → 화면 가장자리 순간이동 | 배율 적용 시 타이머 리셋 + 즉시 1회 재측정 | 전 배율에서 발바닥 Y 변화 **0.0000** |

**왜 (b) 상쇄를 택했나**: (a) 모든 치수를 `lossyScale`로 나누는 방식은 나눠야 할 지점이 4개 프로퍼티 +
`StrokeWidth` + 32종 도형 빌더에 흩어져 있어 빠뜨리기 쉽고, **빠뜨린 곳은 배율 1.0에서 여전히 정상으로
보인다**(가장 나쁜 실패 유형). 게다가 LineRenderer 두께는 스케일을 안 따라가므로 (a)로 두께까지 나누면
오히려 두 번 틀린다. (b)는 컨테이너 안이 월드 스케일 1이라 **좌표와 두께가 같은 단위 하나**로 통일되고,
`AccessoryShapeBuilder`(32종 전체)를 **한 줄도 고치지 않는다**.

**부수 정정**: `ResolveBodyOffsetY()`가 루트-로컬(`_headTransform.localPosition.y`)과
월드(`_metrics.HeadCenterLocalY`)를 섞어 빼고 있었다. 루트 스케일이 1이던 시절에만 우연히 맞던 식이다.

#### 단일 소스화
`MinStrokeScreenPoints`(2.0pt)를 `SceneBootstrapper`(Editor 전용)에서 `StickConfig`로 올렸다 —
이제 프리팹을 굽는 쪽과 런타임 다이얼 쪽이 **같은 상수**를 읽는다(Editor 어셈블리 상수는 런타임에서
참조할 수 없어 값을 두 곳에 적으면 "구운 두께 ≠ 다이얼로 바꾼 두께"가 조용히 생긴다).

---

### ③ 구석 호버 패널 (34-4 ~ 34-6)

- **진입**: `ICursorPositionService` 좌표 폴링만으로 판정(콜라이더 0개). `InfoGearIconWidget`과 같은 경로.
  감지 영역 140×120pt, 원점은 Dock 실측(`IDockMetricsService` + `DockGeometry.DockThicknessPoints`) 위 12pt,
  없으면 핫코너를 피해 (24, 24).
- **상태 머신**: HIDDEN →(180ms)→ PEEK →(+260ms)→ COLLAPSED ↔(드래그)↔ EXPANDED. 이탈 320/400ms,
  값 변경 직후 800ms 접힘 유예, 드래그 중 타이머 정지.
- **다이얼**: 34칸(0.05 간격) / 264° 스윕(12시 96° 비움) / 눈금당 헤일로+코어 2겹 + 링 블룸 1겹 /
  `θ_grabOffset`으로 "잡는 순간 값이 순간이동"하지 않게 / 랩어라운드 금지 / 짧은 클릭은 그 눈금으로 점프.
- **미리보기 카드**: `CharacterPortraitStage`를 **10200**에 따로 세운다(`Create`에 `stageWorldX` 선택 인자 추가,
  기존 호출부 무수정). 액자는 `CharacterInfoWindow.PortraitContentSize`를 **정확히** 재사용.
  좌우 화살표/페이지 점은 34-6-4대로 **미채택**.
- **비침해**: `IsSuspended`면 연출 없이 그 프레임에 캔버스+차단막 회수, 복귀 후 1초 억제, 자동 재개방 없음.
  ESC → 즉시 숨김 + **3초 억제**. 880 정보창이 열려 있는 동안 감지 영역 비활성(초상화 카메라 2대 동시가동 방지).
- **탈출구 ③**(구석 패널 영구 끄기): 기어 부채꼴이 아니라 **캐릭터 우클릭 메뉴**에 넣었다(아래 설계 이탈 2번).

#### 구현 중 실행으로 잡은 결함 3건 (전부 캡처/테스트가 먼저 발견)

1. **다이얼 그림과 히트 판정이 서로 다른 자리에 있었다** — `CenterScreen`(판정)만 갱신하고
   RectTransform은 부모 중심 앵커 기본값에 남아 있어, 첫 캡처에서 다이얼이 패널 한가운데에 그려져
   미리보기 카드와 겹쳤다. `SetCenterInParentPoints()`로 두 좌표를 같은 한 쌍의 수에서 파생시켰다.
2. **액자는 그려지는데 안이 텅 비었다** — `CharacterPortraitStage.SetRenderingEnabled()`가
   `on && _texture != null`로 카메라를 켜는데 호출부가 **RT를 만들기 전에** 그 함수를 부르면 카메라가
   꺼진 채 남고 아무도 다시 켜 주지 않는다. `TryEnsureTexture`에서 활성 상태를 다시 맞추도록 고쳤다
   (**교차 레이어** — 880 정보창도 같은 순서 의존에서 벗어난다).
3. **드래그 중 다이얼 값이 매 프레임 되돌려졌다** — "저장이 없으면 캐릭터의 현재 배율로 다이얼을
   맞춘다"를 매 프레임 돌게 짜서, 드래그 중(아직 실캐릭터에 적용 전이라 두 값이 다른 것이 정상)에
   값이 0.75에 붙박였다. 복원에 **끝**을 세 개 달았다(저장 발견 / 2초 유예 만료 / 사용자가 다이얼을 잡음).

---

### 34절 스펙에서 **의도적으로 벗어난 지점 3건** (리더 확인 요망)

1. **`SizeDialWidget`을 MonoBehaviour가 아닌 평범한 클래스로 만들었다.** 34-9 #5는 "신규 컴포넌트 2종"
   이라고 적었지만, 컴포넌트로 만들면 **프리팹 배선 지점이 하나 더** 늘어난다 — 33-9 #10이 경고했고
   실제로 Blocker B1로 터진 바로 그 지점이다. 다이얼은 호버 패널 없이 존재할 이유가 0이고 수명도
   완전히 같으므로 소유자가 직접 들고 있는 편이 위험이 적다. **파일은 문서대로 분리**했다.
2. **"구석 패널 끄기"를 기어 부채꼴이 아니라 캐릭터 우클릭 메뉴에 넣었다.** 부채꼴은 32절에서
   "원버튼 3개"로 확정된 구조라 4번째를 끼우면 그 절의 기하(각도/반지름)를 다시 유도해야 한다.
   반면 우클릭 메뉴에는 이미 **같은 성격의 영구 토글 3개**(잉크색/로데오/진단로그)가 모여 있다.
3. **호버 미지원 플랫폼에서 부품을 파괴하지 않고 감지만 끈다.** 처음에는 캔버스/촬영장을 파괴했는데,
   그러면 배치 모드에서 **증거 캡처도 회귀 테스트도 불가능**해진다(실제로 하네스가 빈 화면을 찍었다).
   캔버스는 비활성, 촬영장 카메라도 꺼짐이라 매 프레임 비용은 0이고, 880 정보창도 같은 방식으로
   촬영장을 상시 들고 있다.

### 검증 방법에서 벗어난 지점 1건 (리더 판단 요망)

**"실제 빌드로 스크린샷"을 그대로 하지 않았다.** 이 패널의 트리거는 **OS 전역 커서 좌표**이고 그 커서는
유저의 물리 마우스다 — 자동화가 커서를 움직이는 것은 유저 환경을 건드리는 일이라 이 프로젝트의
비침해 규약에 어긋난다. 대신 **같은 캔버스를 같은 코드로 만든 뒤** 카메라로 찍는 PlayMode 하네스
(`Tests/PlayMode/ZZCornerPanelCaptureHarness.cs`, 디버거의 `ZZDebuggerOverlapDiagnostic`과 같은 관례)를
썼다 — 그림을 만드는 경로는 100% 동일하고 트리거만 우회한다. 배포 빌드 자체는 별도로
`Succeeded, 에러 0 / 경고 0`을 확인했다. **이 하네스는 삭제 대상**이다(리더 판단).

증거 3장 (`Logs/evidence_20260831_coder_corner/`):
- `collapsed.png` — 264×148. 12시 96°를 비운 덕에 **접힌 상태에서도 눈금이 하나도 안 잘린다**(34-3-2의 의도 확인).
- `expanded.png` — 264×392. 카드 안에 모자/나비넥타이/망토/펫이 붙은 미니 피규어 + 캡션 `쉬는 중`.
- `dialed_up.png` — 다이얼을 실제로 드래그해 **1.60×**. 켜진 눈금이 늘고, 카드 속 피규어와
  **바탕화면의 실캐릭터가 함께 커졌으며, 모자가 머리에서 떠오르지 않았다**(s² 수정의 육안 확인).
...
| # | 영향 | 대상 | 성격 |
|---|---|---|---|
| 1 | **씬의 `CharacterPortraitStage`가 1개 → 2개**(880창 10000 / 호버 패널 10200) | `PortraitFallenFramingTests`(2) / `CharacterPortraitStageTests`(3) / `PortraitDragIndependenceTests`(1) 의 `ExactlyOne<>` 단언이 **깨졌다** | 34-6-3이 예고한 결과. 6건 전부 `PrimaryStage()`(= X=10000에 선 것 하나)로 교체 — 원래 지키려던 위험("한 자리에 둘이 겹쳐 카메라 하나가 피규어 둘을 찍는 것")을 **X 좌표별로** 다시 세웠으므로 검증력은 오히려 강해졌다 |
| 2 | `CharacterPortraitStage.TryEnsureTexture`가 **카메라 활성 상태를 다시 맞춘다** | 880 정보창도 함께 영향 | 순서 의존 제거(위 결함 2). 기존 동작은 변하지 않는다 |
| 3 | `CharacterAccessoryRenderer` 컨테이너에 `localScale` 상쇄 도입 + `ResolveBodyOffsetY` 단위 정정 | 액세서리 32종 전체 | 배율 1.0에서는 결과 동일(1/1 = 1). 배율이 1이 아닐 때만 달라진다 |
| 4 | `StickConfig` 신규 필드 2 + 상수 2 (`parkourMantleInsetDerived` / `MinStrokeScreenPoints` / `ReferencePointsPerWorldUnitApprox`) | 배포 에셋에 키 1줄 추가 | 순수 추가 |
| 5 | **저장 스키마 v5 → v6** (`characterScaleSaved` / `characterScale` / `cornerPanelEnabled`) | `Core/CharacterSaveStore.cs` | `cornerPanelEnabled`만은 **기본이 true**라 "없으면 기본값"이 저절로 성립하지 **않는다** → 버전 분기 + 회귀 테스트 `v5_파일을_읽어도_구석_패널이_꺼지지_않는다` 신설 |
| 6 | 우클릭 메뉴 행 17 → **18**(`구석 크기 패널: 켬/끔`) | `Interaction/AppControlDirector.cs` | 순수 추가 |
| 7 | `UiChrome` 신규 프리미티브 3종 + 공개 상수 2개(`EdgeFeatherPoints` / `CapsuleCapTexelsPublic`) | 순수 추가 | 34-9 #3 |
| 8 | 상주 비용 증가: 캔버스 1 + 초상화 카메라 1 + `Image` 약 80개(다이얼 눈금 68 + 유리 6겹 ×2 + 카드) | 전부 **생성 시 1회**만 그려지고 값이 바뀔 때만 색/길이 갱신. 카메라는 EXPANDED일 때만 돈다 | **perf-doc 확인 권장** |

### 남은 관찰 / 미착수 (리더 판단)

- **`FloorContactVisibilityTests`가 플레이키하다.** 전체 스위트 4회 중 1회 실패
  (`Getup` 상태에서 잉크가 화면 아래로 4.90pt, 허용 1pt). **격리 실행 2회 연속 통과**,
  같은 스위트 재실행도 **270/270 통과**. 실패 런의 로그를 보면 그 시점 배율은 0.75 정상이고
  낙하 구간 관측치가 런마다 다르다(`Fall −35.67pt` vs `−56.39pt`) — 물리 타이밍 의존으로 보인다.
  이 패널이 캔버스/카메라를 하나씩 더 얹어 프레임 시간이 달라진 것이 방아쇠일 수 있다.
  **가설 단계이므로 디버거에게 넘긴다**(추측으로 임계값을 올리지 않았다).
- **34-3-6의 "RAGDOLL 중 대기"를 연출로만 남겼다.** 물리적 근거가 없다는 것이 디버거 결론이므로,
  대기 중 중앙에 `곧 적용` 캡션을 띄우고 **최대 3초 뒤에는 상태와 무관하게 적용**한다.
- **미착수**: 34-4-4의 이징 곡선(`easeOutQuad`/`easeOutCubic`)을 `MoveTowards` 선형으로 근사했다.
  0.14~0.20초 구간이라 육안 차이가 크지 않다고 판단했고, 이 프로젝트에 이징 유틸이 없어 새로 만들지 않았다.
- **미착수**: 모바일 대체 진입(하단 스와이프 업, 34-9 #13)은 예약 그대로.

### 2026-08-31 — 리더 승인: 호버 패널 + 크기 다이얼 완성
1~3단계 전부 승인. 특히 배정문의 전제("선행조건 5개 중 4개 이미 해소")가 실제로는 틀렸음을 코더가
직접 grep으로 확인하고(액세서리 이중스케일/LineRenderer 두께 미수정 상태였음) 스스로 고친 것을
좋은 사례로 기록 — 배정 지시를 맹신하지 않고 검증한 태도.
승인 항목: parkourMantleInset 유도값 전환 + "컨트롤의 컨트롤"(`parkourMantleInsetDerived` 스위치로
기존 네거티브 컨트롤 보존), 액세서리 컨테이너 역스케일 상쇄, LineRenderer `startWidth` vs
`widthMultiplier` 단위 불일치 수정, 실행 중 발견한 3버그(다이얼 히트박스/포트레이트 카메라 꺼짐/
드래그 중 값 복원 경합) 수정, `PrimaryStage()` 재설계로 씬 2개 스테이지 대응, 저장 스키마 v6.
**설계 이탈 4건 전부 승인**: SizeDialWidget 비-MonoBehaviour화, 패널 끄기를 우클릭 메뉴로 이동,
호버 미지원 환경에서 파괴 대신 감지만 비활성화, **실제 빌드 스크린샷 대신 PlayMode 캡처 하네스
사용**(마우스를 실제로 움직이는 자동화는 비침해 원칙 위반이라는 판단 — 사용자의 실제 입력 환경을
테스트가 건드리면 안 된다는 이 프로젝트 원칙에 부합하는 좋은 판단).

---

## 2026-08-31 — `FloorContactVisibilityTests` 간헐 실패 원인 규명 **[Debugger]** — 과학적 토론 로그

**결론 한 줄: 이번 라운드의 회귀가 아니다(네거티브 컨트롤로 반증 완료). 진짜 원인은 GETUP이
포즈를 재구성할 때 발판(바닥)에 대한 제약이 전혀 없다는 기존 결함이고, 8pt 안전망 상수는 그 결함의
꼬리를 못 본 3표본짜리 실측으로 정해져 있었다. 임계값 1pt는 정당하며 올리면 안 된다.**

### 검증한 가설과 결과

| # | 가설 | 검증 방법(실측) | 결과 |
|---|---|---|---|
| H1 | **코더 가설** — 이번 라운드가 캔버스/카메라를 늘려 프레임 시간이 달라진 것이 방아쇠 | (a) `Time.deltaTime` 30만~50만 표본 분포 측정, (b) 라운드 변경분을 `git stash`로 **통째로 되돌린 뒤**(신규 파일 4쌍은 별도 격리) 동일 하네스 재실행 | **반증.** dt 평균 0.206ms(라운드) vs **0.203ms(라운드 전)**, p99 0.318 vs 0.306ms. 프레임은 물리 스텝(20ms)보다 **100배 빠르다** — 프레임 시간이 물리에 영향을 줄 여지가 구조적으로 없다. 깊은 침투 발생률도 라운드 40사이클 중 1회 vs **라운드 전 40사이클 중 2회**(라운드 전이 오히려 더 높다). 강제 스핀 스윕은 **양쪽 모두 31건 중 9건**이 1pt 초과로 완전히 동일. 게다가 호버 패널의 캔버스는 배치모드에서 비활성이고 초상화 카메라는 꺼져 있다(`Camera.allCamerasCount=1`) |
| H2 | **물리가 실행마다 비결정적이다**("낙하 관측치가 런마다 다르다") | 과거 로그 57런의 `state=Fall` 값 전수 집계 | **반증.** Fall 값은 연속 분포가 아니라 **−14.48 / −35.67 / −56.39 세 개의 이산값만** 나온다(현재 코드에선 두 개). 비결정 물리라면 연속으로 퍼진다. 실체는 **0.05초 샘플링 위상** — 공식 테스트는 `WaitForSeconds(0.05)`로 재는데 물리 스텝은 0.02초라 **2.5스텝당 1회만** 관측한다. 두 값의 차 20.72pt(0.506유닛)는 낙하 속도 약 10u/s 구간에서 정확히 **샘플 1주기**만큼의 이동이다. 코더가 비교한 두 수치(−35.67 / −56.39)는 애초에 **서로 다른 날짜의 다른 라운드 로그**끼리의 비교였다 |
| H3 | 샘플링 복권 — 깊은 침투는 매 런 일어나는데 0.05초 샘플이 가끔만 잡는다 | 같은 사이클에서 **매 프레임 관측**과 **0.05초 샘플 관측**을 동시에 기록 | **반증.** 침투가 일어난 사이클에서 두 값이 5.56 vs 5.51pt로 사실상 같다(침투 구간이 0.05초보다 길다). 즉 **사건 자체가 드문 것**이지 관측이 놓치는 것이 아니다 |
| H4 | 널브러진 정착 각도가 ±90°를 넘으면 잉크가 발판 아래로 내려간다 | 랙돌 진입 직후 각속도를 −600~600도/s로 **강제 산포**해 정착각-잉크 곡선을 결정론적으로 뽑음(31점) | **입증(부분).** GETUP 첫 프레임의 잉크: 84.1°→−0.66pt / 87.4°→+2.14 / 88.3°→+2.77 / 89.6°→+4.30 / 90.3°→+4.66 / 93.4°→+9.02 / 94.6°→+10.14 / 96.5°→+12.42pt — **단조 증가**, 약 1.2pt/도. 임계는 **|정착각| ≈ 87°** |
| H5 | 진입 각도만 막으면 충분하다(진입 스냅이 유일한 경로) | 사이클당 **GETUP 에피소드 수**를 세어 재랙돌 혼입을 배제한 뒤, 정상 정착각(83.8°) 사이클의 **에피소드 내 최악값** 측정 | **반증.** 에피소드 수는 전 사이클 **1**(재랙돌 없음)인데, 정착각 83.7~83.8°인 사이클에서도 첫 프레임은 안전(−1.8~−2.8pt)인 채 **보간 도중 최대 +7.70pt**까지 내려간다. 즉 침투 경로가 **진입 스냅**과 **보간 중 스윙** 두 개다 → 수정은 **매 프레임 제약**이어야 한다 |

### 근본 원인 (Major, 기존 결함 — 이번 라운드 무관)

`States/GetupState.cs` + `States/StickmanPoseAnimator.TickGetupPose` + `States/RagdollRig.TickGetupRoot`

GETUP은 (1) 루트 회전각을 "널브러진 각도 → 0"으로, (2) 팔다리 각도를 "널브러진 각도 → 중립"으로
직접 보간한다. 이때 팔다리는 이미 **Kinematic**이라 물리 충돌 해소가 없고, 루트 Y는 접지 안전망
(`StickmanBlackboard.TickGroundKeepingSafetyNet` → `SnapToGround`)이 **발판 상단에 못박는다**.

이 프로젝트의 규약은 "**루트 원점 = 발바닥**"인데, 누워 있는 동안 그 규약을 강제하면 기하학적으로
몸통·팔·모자가 발판 아래로 내려갈 수밖에 없다(누운 몸의 무게중심은 발바닥 높이의 수평선 근처에 있고,
회전각이 90°를 넘으면 반대편 파츠가 그 선 아래로 간다). **RAGDOLL에서는 콜라이더가 이걸 막고
(랙돌 구간 실측 잉크 최저는 발판 상단 아래 최대 4.6pt에서 멈춘다 — 화면 바닥 아래로는 한 번도 안 나갔다),
GETUP으로 넘어오는 순간 그 방어가 사라진다**
— 실측한 전이 점프는 정상 케이스 +5.7pt(위로), 깊은 케이스 **최대 −19.8pt(아래로)**.

실측 최악값: **화면 바닥 아래 12.5pt = 발판 상단 아래 20.5pt(≈0.5월드유닛)**, 최저 파츠는
`RightArmLower` / `LeftLegLower` / `CapeOutline`(망토).

### 왜 "임계값을 올린다"가 오답인가 (숫자로)

- 테스트의 `MaxInkBelowScreenPoints=1pt`는 임의 허용오차가 아니라 **획 두께(2.5pt)의 절반 미만 = 육안
  불가능**이라는 제품 불변식이다. 실측 최악 20.5pt를 덮으려면 `BottomSafetyNetInsetPoints`를 8 → **21pt**로
  올려야 하는데, 그러면 서 있을 때 발 잉크가 화면 바닥에서 19pt 뜬다 — 사용자가 **세 번 신고한 "떠 있다"**
  (당시 38pt)의 절반이다. 키가 약 70pt이므로 27%가 뜬다. **정면 재발.**
- 즉 오늘 Dock 라운드와 같은 처방이 맞다: 상수를 키우는 게 아니라 **근본 값을 유도값으로 바꾼다**.

### 8pt 상수 자체의 교정 결함 2건 (근거 추적 결과)

1. **3표본짜리 최댓값 추정.** 8pt의 근거인 `Logs/coder_floor_probe5.log`(2026-08-29 15:55)는
   "자유 관찰 45초 + **RAGDOLL 강제 3회**"로 `Getup 2.62pt`를 최악값으로 기록했다. 실제 GETUP 침투는
   약 3~4% 확률의 꼬리를 가지므로(내 실측: 자연 발생 80사이클 중 3회) 3표본이 이걸 볼 확률은 약 11%다.
   **못 본 게 정상이다.** 실제 최악은 그 기록의 약 8배(20.5pt).
2. **액세서리가 존재하기 전에 잰 값이다.** probe5는 08-29 **15:55**, 장비/액세서리 시스템 최초 커밋
   `1154629`는 같은 날 **23:34**다. 스윕 실측에서 망토(`CapeOutline`)가 단독으로 **8.92pt**를 기여한
   사이클이 있다(라운드 전 코드에서도 동일). 즉 8pt는 **모자도 망토도 없던 실루엣**으로 교정된 값이다.

### 실패율 (요청받은 반복 실행)

| 측정 | 표본 | 실패/침투(>1pt) |
|---|---|---|
| `FloorContactVisibilityTests` 격리 12회 반복(신규 실행) | 12런 | **0** (Getup 관측 −2.46~−6.26pt) |
| 과거 전체 스위트 로그 전수 집계(08-29~08-31) | 57런 | **2** (Getup 4.90pt 1건 / LandingCrouch 8.82pt 1건, 후자는 08-29로 이번 라운드보다 이틀 전) |
| 자연 발생 랙돌 사이클(하네스, 라운드 코드) | 40사이클 | **1** |
| 자연 발생 랙돌 사이클(하네스, **라운드 전** 코드) | 40사이클 | **2** |

→ 사이클당 침투 확률 **약 3~4%**. "4회 중 1회 실패"는 이 확률에서 충분히 나오는 값이고(4회 중
1회 이상 실패 확률 ≈ 13%), **격리 2회 통과는 아무것도 반증하지 못한다**(2회 통과 확률 ≈ 93%).

### 리더 판단 요청 — 수정 범위

이 결함은 **이번 라운드 범위 밖**이다(라운드 전 코드에서 동일하게, 오히려 더 자주 재현). 그래서
직접 고치지 않고 넘긴다. 수정한다면 코더에게 줄 요구사항은 이렇다.

- **필수: 매 프레임 제약**(H5로 진입 각도 클램프 단독은 반증됨).
- **형태(권고)**: GETUP 동안 루트 Y를 `발판상단Y + max(0, 발판상단Y − 재구성된 포즈의 최저 잉크Y)`만큼
  들어 올린다. progress→1이면 리프트가 **저절로 0**이 되므로 상수가 하나도 안 생긴다
  (`ResolveEdgeStopDistance`/`ResolveStepUpMaxHeight`/`ResolveParkourMantleInset`과 같은 유도값 패턴).
  최저점은 전 LineRenderer 정점이 아니라 FK 체인의 끝점 6개(양발/양손/정수리/모자챙끝)로 충분하다.
- **주의(배선 순서)**: `StickmanAgent.Update()`는 `machine.Tick()` → `TickGroundKeepingSafetyNet()` 순서라
  상태 안에서 올린 루트를 **안전망의 `SnapToGround`가 그 프레임에 도로 내린다**. 리프트는 안전망 이후에
  적용하거나, GETUP을 `IsGroundKeepingSelfManaged`로 옮겨 스스로 접지를 관리해야 한다.
- **테스트**: 임계값 1pt는 **그대로 둔다**. 대신 `ZZGetupAngleSweep`가 만든 결정론적 재현(랙돌 진입 후
  각속도 강제 산포)을 정식 회귀 테스트로 승격하면, 3~4% 확률에 기대지 않고 **매번** 이 결함을 잡는다.

### 남긴 임시 하네스 (검증 후 삭제 대상)

- `Tests/PlayMode/ZZGetupFloorDiagnostic.cs` — 랙돌→기상 사이클 반복, 매 프레임 vs 0.05초 샘플 대조,
  dt 분포. `STICKMATE_DIAG_NOPANEL=1`로 호버 패널만 끄는 대조군 지원.
- `Tests/PlayMode/ZZGetupAngleSweep.cs` — 정착각 강제 산포 스윕(액세서리 기여도 분리, 에피소드 계수).
- 로그: `Logs/dbg2_getup_A1·A2.xml`, `Logs/dbg2_sweep_A1·A3·A4.xml`, `Logs/dbg2_B_preround.xml`(네거티브
  컨트롤), `Logs/dbg2_floor_rep1~12.xml`.
- **작업 트리 원복 확인**: `git stash` 전후 `git diff` md5 동일(`f02f3011581cbef5a338ae83d49ad513`),
  변경 파일 수 동일. 커밋하지 않았다.

### 부수 관측 (별건, 리더 참고)

- **`LandingCrouch`도 같은 계열일 가능성**(가설, 미검증). 08-29 `Logs/dbg_floor1.xml`에서 LandingCrouch가
  8.82pt로 한 번 실패했는데(평시 −6.71pt), 그 상태도 GETUP과 같이 **상태가 포즈를 직접 세팅하고
  `TickPose`가 손대지 않는** 부류다. 검증 방법: 낙하 높이/착지 속도를 산포시켜 착지 자세의 잉크 최저점
  분포를 뽑는다(GETUP 스윕과 같은 방식).
- **공식 테스트의 샘플링 해상도**(0.05초 = 물리 2.5스텝)는 이 결함에는 문제가 안 됐지만
  (H3에서 확인), 1~2 스텝짜리 순간 관통을 재는 용도로는 원리적으로 부족하다. 필요해지면
  `WaitForFixedUpdate` 기반으로 바꾸는 것이 맞다.

### 2026-08-31 — 리더 승인: GETUP 바닥 관통 원인 확정 + 코더 배정
디버거 판정 승인: 이번 라운드 회귀 아님(git stash 대조로 코더 가설(프레임 시간) 반증 — 오히려
라운드 전 코드가 더 자주 재현). 근본 원인은 GETUP이 팔다리 Kinematic 보간 중 콜라이더 보호가 없는
상태에서 접지 안전망이 루트 Y를 발판 상단에 못박아 생기는 기존 기하 결함(3~4%/사이클, 8pt 상수가
3표본으로 교정돼 실제 최악치를 놓쳤던 것도 확인). "임계값을 올리면 발이 뜨는 회귀 재발"이라는
반박 논리 명확 — Dock 라운드와 같은 처방(유도값 전환)으로 코더에게 배정.

---

## 2026-08-31 — 사용자 신고 "창을 최대화하면 공은 창 위에 있고 캐릭터는 독 위에 있음" **[Debugger]** — 과학적 토론 로그

**심각도: Major** / `Assets/_Project/Scripts/Interaction/CharacterPetRenderer.cs:398`(수정 전 `ResolveGroundY`)
/ 부수 발견 `Assets/_Project/Scripts/Interaction/CharacterFxRenderer.cs:247,313`

### 재현 시나리오 (PlayMode로 결정론적 재현 성공)
입력: 발판 목록 = [최대화 창(핸들 1001, 화면 전체 폭, 상단 OS y = 화면 5%), Dock(핸들 −2)].
주인을 Dock 상단(월드 Y −10.167)에 세우고 `CurrentFootholdHandle = −2` 고착 → 작은 공 착용 → 1초 대기.
잘못된 결과(수정 전 코드 실측): **공 Y = 10.894**(최대화 창 상단 10.800 + 반지름), 주인 발밑 −10.073 →
**21.0유닛 어긋남**. 공의 x는 −0.938로 주인(0.000)을 정상적으로 따라가고 있었다.

### 가설과 검증 (지시받은 3개 방향 전부 검증, 2개 반증)
| # | 가설 | 결과 |
|---|---|---|
| H1 | `CharacterAccessoryRenderer` 모자 버그와 같은 계열 — 펫이 실시간 transform 대신 캐시/중립값을 쓴다 | **반증**. `TickBall`은 매 프레임 `bb.Body.position.x`를 읽는다(코드 경로 + 실측 공 x가 주인을 따라감). |
| H2 | 발판 전환 신호(창 최대화 → Dock 이동)를 펫이 못 받아 자기 좌표계에 멈춘다 | **반증**. 펫은 이벤트를 구독하지 않고 매 프레임 폴링한다. 멈춘 게 아니라 **매 프레임 성실하게 틀린 값**을 계산했다. |
| H3 | 구름 공식 `θ −= Δx/r`의 Δx 좌표계 혼용 / 텔레포트급 Δx 오해석 | **반증**. Δx는 펫 자신의 프레임 간 월드 x 변화이고 y와 무관하다. 스폰 프레임 보호도 이미 있다(R2 m4). |
| **H4** | **y만 다른 소스에서 온다** — `ResolveGroundY`가 `TryGetGroundSurfaceWorldY`(= 그 x에서 **가장 높은** 발판 상단)를 물어서, 화면을 덮는 창이 하나라도 있으면 어느 x에서든 답이 화면 꼭대기가 된다 | **확정**. 네거티브 컨트롤에서 옛 공식이 실제로 10.800(= 최대화 창 상단)을 답했다. |

**결론**: 텍스트-액션 desync도 좌표계 혼용도 아니다. 펫의 x는 주인의 실시간 위치, y는 "그 x에서 가장
높은 표면"이라는 **서로 다른 두 소스**에서 나왔고, 최대화 창이 그 둘을 21유닛 벌렸다.
펫이 물었어야 할 질문은 "이 x에서 가장 높은 면은?"이 아니라 "**주인이 지금 딛고 있는 면은?**"이며,
그 답은 발판 핸들(`StickmanBlackboard.CurrentFootholdHandle` → `TryGetFootholdTopWorldY`)에만 있다.

**이 프로젝트 3번째 동일 오용이다**: 드래그 순간이동(08-28, `GroundSensor.TryGetFloorWorldY` 문서),
구조 안전망 순간이동(08-29, `GroundSnapTeleportTests`), 그리고 이번. `TryGetGroundSurfaceWorldY`는
"커서가 지면 아래일 때 캐릭터를 가두지 않기" 전용이지 **표면을 고르는 용도가 아니다**.
...
### 검증
- 신규 `Tests/PlayMode/PetFollowsOwnerFootholdTests.cs` — P1(최대화 창 있어도 공은 주인 발밑),
  P1n(네거티브: 옛 공식이 실제로 화면 꼭대기를 답함, 격차 20.967유닛), P2(신고 시나리오 전이:
  창 위 → 최대화 → Dock, 공이 따라옴), P3(주인 공중이면 공은 마지막 발판에서 대기 = "공은 날지 않는다"
  규약 미파괴). **4/4 통과**(`Logs/dbg_pet_follow.xml`).
- **네거티브 컨트롤(수정 되돌리기)**: `ResolveGroundY` 본문만 수정 전 코드로 되돌려 재실행 →
  P1·P1n·P2 **실패**, 공 Y=10.894 vs 주인 −10.073으로 신고 증상 그대로 재현(`Logs/dbg_pet_negctrl.xml`).
  P3만 통과(무관한 불변식이므로 예상대로). 확인 후 즉시 원복.
- 회귀: `CharacterAppearanceLayerTests`(FX 발자국 재사용 테스트 포함) + `FloorContactVisibilityTests`
  + 신규 4건 = **20/20 통과**(`Logs/dbg_pet_regress.xml`). EditMode **147/147 통과**
  (`Logs/dbg_pet_edit.xml`). 컴파일 에러 0.
- 동시 실행 충돌: GETUP 코더의 배치 실행과 락을 다투어 순차 대기 후 실행. 수정 파일은
  `Interaction/` 2개 + 신규 테스트 1개로 코더 작업 영역(`States/`)과 겹치지 않는다. **커밋하지 않았다.**

### 리더 참고 — 같은 계열 잔여 후보(이번 라운드 범위 밖, 미수정)
- `States/ThrowTumbleState.TryPredictLandingSurfaceY:507` — `TryGetGroundSurfaceWorldY`로 "몸보다 아래의
  가장 높은 면"을 노리지만 실제로는 **전체에서 가장 높은 면**을 받고 위면 폴백한다. 최대화 창이 있으면
  착지 예측면이 Dock이 아니라 화면 최하단 안전망이 되어 정렬 타이밍이 어긋날 수 있다(Minor, 가설).
  검증 방법: 최대화 창 + Dock 배치에서 던지기 후 정렬 완료 시점과 실제 착지면을 비교.

### 2026-08-31 — 리더 승인: 펫-오너 발판 불일치(창 최대화 버그) 근본수정 완료
디버거가 지시받은 가설 3개(H1~H3)를 전부 코드로 반증하고 진짜 원인(H4: `TryGetGroundSurfaceWorldY`가
"그 x에서 가장 높은 면"을 찾는데 최대화 창이 있으면 그게 항상 화면 꼭대기가 됨 — "주인이 지금 딛고
있는 발판이 뭔지"를 물어야 하는데 "이 x에서 최고 높이가 뭔지"를 물은 것)를 확정, 직접 수정 완료.
**"이 프로젝트에서 같은 API 오용 3번째"**라는 지적 — 08-28 드래그 순간이동, 08-29 안전망 순간이동에
이은 반복 패턴. `CharacterPetRenderer.cs` 수정 + 동일 원인의 `CharacterFxRenderer.cs`(발자국/먼지가
같은 상황에서 화면 꼭대기에 찍히던 것, 사용자 미신고이지만 선제 수정) 승인. 네거티브 컨트롤로 정확히
신고 증상(21유닛 어긋남) 재현 확인 후 원복, 회귀 20/20·EditMode 147/147 통과.
**후속 후보(Minor, 미확정 가설)**: `ThrowTumbleState.TryPredictLandingSurfaceY`도 같은 API를 써서
최대화 창 상황에서 던지기 착지 예측이 어긋날 수 있음 — 다음 라운드 검증 후보로 기록.

---

## 2026-08-31 — GETUP 발판 관통 근본수정: 유도 리프트 + 결정론적 회귀 테스트 승격 **[Coder]**

**결론 한 줄: 상수는 하나도 늘지 않았다. GETUP 동안 "지금 이 포즈가 접지선 아래로 내려간 만큼"만
루트를 들어 올리고(progress→1이면 저절로 0), 3~4% 확률에 기대던 검증을 31점 결정론적 스윕으로
바꿔 매번 재현되게 만들었다.**
...
| 파일 | 변경 | 왜 |
|---|---|---|
| `Core/StickConfig.cs` + `Data/DefaultStickConfig.asset` | `getupFloorClearanceEnabled`(기본 ON) 1개 추가 | 네거티브 컨트롤 통로(`groundKeepingSafetyNetEnabled` / `parkourMantleInsetDerived`와 같은 관례). **수치 상수는 0개** |
| `States/StickmanPoseAnimator.cs` | `TryGetLowestBodyInkWorldY()` 신설 | 몸의 잉크 하한. 팔다리는 마디 2개짜리 꺾은선이라 **관절 3점**(부착점/무릎·팔꿈치/끝)이 극값을 지배하고, 몸통은 엉덩이~어깨라 부착점에 포함되며, 머리는 원이라 중심−반지름이 회전과 무관하게 항상 최저다 → 16점이면 하한이 **정확**하다 |
| `Core/ICharacterInkExtentProvider.cs` (신규) + `Interaction/CharacterAccessoryRenderer.cs` | 몸 바깥 잉크는 **부품이 스스로 신고**한다 | 실측에서 **망토가 단독 최저 파츠**로 나왔다(네거티브 컨트롤 최악값의 최저 파츠 = `CapeOutlineFill`). 소비자 쪽에 모자챙/망토 좌표를 다시 적으면 32종+DLC가 늘 때마다 그 목록이 낡는다(= "안전장치를 한 곳만 고치고 같은 패턴의 다른 경로에는 안 넣기" 재발) |
| `States/StickmanBlackboard.cs` | `TryComputeInkDropBelowRoot()` / `TickInkFloorClearance()` / `ReleaseInkFloorClearanceLift()` | 유도값 본체 |
| `Core/StickmanAgent.cs` | `Update()`에 **벗기기(상태 Tick 앞) / 얹기(안전망·TickPose 뒤)** 2줄 | 순서가 이 수정의 전부다(아래) |

리프트 = `max(0, 루트Y − 지금 포즈의 최저 잉크Y)`. **발판 Y도 화면 좌표도 입력이 아니다**(루트 상대
계산) — 그래서 접지 스냅 전에 재든 후에 재든 값이 같고, 물리 좌표(`Rigidbody2D.position`)와 렌더
좌표(`Transform.position`)를 섞지 않는다(이 프로젝트는 `autoSyncTransforms`가 꺼져 있다).

### 배정문 설계에서 **벗어난 지점 2건** (리더 확인 요망)

**1. 리프트를 `GetupState` 안이 아니라 `StickmanAgent.Update()` + 블랙보드에 뒀다 — 실측으로 강제된 변경.**
배정문은 "GETUP 상태 안에서 들어 올리되 안전망이 덮어쓰지 않게 하라"였다. 그대로 상태 안에 넣고
(상태가 `SenseGround`+`GroundedTick`을 직접 불러 안전망의 프레임 가드로 중복을 피하는 형태) 스윕을
돌렸더니 **31점 중 3점이 여전히 관통**했다: `spin=480 → 3.00pt / 540 → 2.00pt / 600 → 6.36pt`
(로그 `Logs/coder_getup_fix1.log`). 세 케이스 모두 **그 사이클의 최악값이 GETUP 첫 프레임 값과 정확히
같았다** — 원인은 명확하다: `ChangeState`는 `Exit/Enter`만 부르고 **새 상태의 `Tick`은 다음 프레임부터
돈다**. 즉 전이 프레임에는 상태 안의 어떤 코드도 실행되지 않는데, **가장 깊은 관통이 바로 그 프레임에서
난다**(랙돌 스프롤 자세 그대로 접지 안전망이 루트를 발판 상단에 못박는 프레임). 그래서
`TickPose`와 같은 자리 — "상태 ID만 보고 매 프레임 멱등 적용" — 로 옮겼다. 전이 프레임/강제 인터럽트/
외부 `ChangeState` 어느 경로로 들어와도 빠짐이 없다.
**부수 효과**: `IsGroundKeepingSelfManaged`를 건드릴 이유가 사라졌다(`DockSinkholeRegressionTests`의
T2가 잠근 "Dock 위 기상은 접지 유지를 받는다"가 그대로 유지된다) — 배정문이 제시한 두 선택지 중
어느 쪽도 필요 없었다.

**2. 리프트를 매 프레임 얹었다가 다음 프레임 맨 앞에서 **벗긴다**.**
얹은 채로 두면 접지 판정이 전부 틀어진다: `GroundSensor.Sense()`는 발이 발판 상단
±`groundSnapTolerance`(0.49유닛) 안일 때만 접지로 보고, `SnapToGround`는 이동 요구가
`groundSnapMaxDistanceWorld`(0.6유닛)를 넘으면 **발판을 놓고 Fall로 보낸다**. 실측 최악 리프트가
0.5유닛(20.5pt)이라 두 임계 모두 아슬아슬하게 걸린다 — "바닥을 안 뚫게 고쳤더니 기상 중에 갑자기
낙하한다"가 될 뻔한 지점이다. 벗겼다 얹으면 물리·센서·발판 판정은 **언제나 리프트 없는 진짜 접지
좌표**만 본다(리프트는 그 프레임의 그림에만 존재).

### 검증 (전부 실측, 로그 첨부)

| 항목 | 결과 |
|---|---|
| **정식 승격 회귀 테스트** `Tests/PlayMode/GetupFloorClearanceTests.cs` | 랙돌 진입 직후 각속도를 −600~600도/s로 강제 산포하는 **31점 결정론적 스윕**. 디버거의 임시 하네스 `ZZGetupAngleSweep`를 그대로 승격 |
| **수정 후** | **31/31 사이클 전부 정확히 −6.71pt**(화면 바닥 **위** 6.71pt = 발판 상단 아래 1.29pt = 획 반두께). 관측 최대 정착각 **97.7도**(디버거가 본 최악 96.5도보다 더 나쁜 각도까지 들어갔는데도 관통 0) |
| **네거티브 컨트롤**(`getupFloorClearanceEnabled=false`) | **31점 중 13점이 위반**, 최악 **+12.91pt**(정착각 94.6도, 최저 파츠 `CapeOutlineFill`). 디버거가 지목한 **두 경로가 모두 재현**됐다: 스윙 경로(정착각 83.8도에서 6.4~7.8pt) + 진입 스냅 경로(94.6도에서 12.91pt) |
| **부작용 없음 — 서 있는 발 위치 불변** | `FloorContactVisibilityTests`의 접지 최악간격 **7.24pt**. 과거 로그 실측치(`te_final_pm` 7.24 / `te_final_pm2` 7.24 / `te_pm2` 7.24 / `tt_play2` 7.37)와 **동일**. 리프트는 상태 ID가 Getup일 때만 계산되고, 마지막 Getup 프레임의 리프트도 다음 프레임 맨 앞에서 벗겨지므로 Idle로 새어 나가지 않는다 |
| 컴파일 | 에러 0 / 경고 0 |
| EditMode | **147/147 통과**(`Logs/coder_full_edit.xml`) |
| PlayMode 전체 | **276/276 통과**(`Logs/coder_full_play2.xml`, 730초). 같은 런의 `FloorContactVisibilityTests`가 `state=Getup 화면바닥아래최대=-6.71pt`를 기록 — **이번에 고친 그 플레이키 실패 지점이 이제 결정론적으로 안전 쪽**이다 |

전체 스위트 런에서도 스윕은 같은 결과를 냈다(수정 ON 최악 −6.71pt / 관측 최대 정착각 96.1도,
네거티브 컨트롤 최악 +12.24pt / 96.7도) — 각속도 산포가 만드는 정착각은 런마다 조금씩 달라지지만
**결론(수정 ON = 전 사이클 안전 / OFF = 다수 위반)은 3회 독립 실행에서 모두 동일**했다.

로그: `Logs/coder_getup_fix1.log`(설계 그대로 상태 안에 뒀을 때의 실패 3점 — 위 이탈 1의 증거),
`Logs/coder_getup_fix2.log`, `Logs/coder_getup_reg1.{log,xml}`(정식 테스트 2/2 통과),
`Logs/coder_full_edit.{log,xml}`.

### 교차 레이어 영향 로그 (즉시 보고)

| # | 영향 | 대상 | 성격 |
|---|---|---|---|
| 1 | `StickmanAgent.Update()`의 프레임 계약에 **2줄 추가**(상태 Tick 앞 벗기기 / 안전망·TickPose 뒤 얹기) | 상태 레이어 전체 | 순서가 곧 정합성이다. 이 두 줄의 위치를 옮기면 수정이 조용히 무력화된다(테스트 실패 메시지에 원인 후보 4개를 적어 뒀다) |
| 2 | **신규 인터페이스** `Core/ICharacterInkExtentProvider` — 캐릭터 루트에 잉크를 더 얹는 부품은 이걸 구현해야 바닥 보정에 포함된다 | 렌더 레이어(현재 구현체는 `CharacterAccessoryRenderer` 하나) | 앞으로 펫/이펙트/DLC가 몸 바깥에 잉크를 그리면 이 인터페이스를 구현해야 한다. 구현하지 않으면 그 부품만 기상 중 바닥을 뚫는다 |
| 3 | 액세서리 렌더러가 `TryGetLowestInkWorldY()`를 공개 | 액세서리 32종 전체 | 순수 추가(그리는 내용/타이밍 무변경). 알파 0(랙돌 중 숨김)이면 false를 돌려주므로 "숨은 망토가 몸을 띄우는" 경로가 없다 |
| 4 | `StickConfig` 신규 필드 1(`getupFloorClearanceEnabled`) + 배포 에셋 키 1줄 | 배포 에셋 | 순수 추가 |
| 5 | 임시 하네스 2개 삭제(`ZZGetupAngleSweep.cs`, `ZZGetupFloorDiagnostic.cs`) | 디버거 산출물 | 스윕은 `GetupFloorClearanceTests`로 승격돼 중복, 진단기는 조사 전용(리더가 삭제 허용) |

### ★ 별건 — 다른 라운드의 임시 하네스가 **PlayMode 배치 실행 전체를 크래시**시킨다 (리더 판단 요망)

전체 PlayMode 스위트를 이 프로젝트의 표준 명령(`-batchmode -nographics -runTests`)으로 돌리면
`Tests/PlayMode/ZZCornerPanelCaptureHarness.cs`(2026-08-31 호버 패널 라운드의 캡처 하네스)의
`Camera.Render()`에서 **에디터가 네이티브 크래시**해 결과 XML 자체가 생성되지 않는다
(스택: `CameraScripting::Render` ← `ZZCornerPanelCaptureHarness/<Capture>d__4:MoveNext`,
로그 `Logs/coder_full_play.log` 끝부분). `-nographics`에는 렌더 타깃이 없기 때문이다.
그 라운드의 증거 캡처는 그래픽을 켜고 돌렸을 것이므로 그때는 드러나지 않았다.
**영향**: 이 파일이 남아 있는 한 표준 배치 명령으로는 **누구도 전체 PlayMode를 완주할 수 없다**.
내 라운드 검증은 `-nographics`를 빼고 재실행해 진행했다. 리더가 이미 "삭제 대상(리더 판단)"으로
분류한 파일이라 내가 지우지 않았다 — **삭제 또는 `-nographics`에서 스킵하도록 가드 추가**를 권한다.

### 남은 관찰 / 다음 라운드 후보

- **`LandingCrouch` 동일 계열 가설(미검증, 디버거 제기)** — 이번 라운드에서는 손대지 않았다.
  다만 수정의 형태가 이미 그 검증을 쉽게 만들어 뒀다: `TickInkFloorClearance()`의 상태 조건에
  `LandingCrouch`를 한 줄 더하면 같은 보정이 그대로 적용되고, `GetupFloorClearanceTests`와 같은
  방식(낙하 높이 산포 → 착지 자세 잉크 최저점 분포)의 스윕이 그 필요성을 먼저 증명해야 한다.
  **지금 넣지 않은 이유**: 실측 근거가 08-29 로그 1건(8.82pt)뿐이고, 그 값은 디버거가 지목했듯
  "착지 첫 프레임 순간이동"(2026-08-29에 `MoveBodyToWorld` 단일 창구화로 이미 고친 결함)의 잔상일
  가능성이 있다. 근거 없이 상태를 늘리면 이 프로젝트가 반복해 겪은 "증상만 보고 범위를 넓히는" 실패다.
- 정식 테스트는 사이클당 약 2.3초 × 31점 × 2케이스라 **PlayMode 스위트가 약 3분 길어진다**.
  결정론을 사는 대가이며, 스핀 점수를 줄이면 고각(|정착각| ≥ 87도) 표본이 함께 줄어든다.

### 2026-08-31 — 리더 승인: GETUP 바닥 관통 근본수정 완료 + 임시 하네스 정리
두 설계 이탈 전부 승인. (1) 상태 전이 첫 프레임에는 새 상태의 Tick이 안 돈다는 걸 실측으로 발견해
로직을 GetupState 내부가 아니라 TickPose류의 "상태ID 기반 매프레임 멱등 적용" 자리로 옮긴 것 —
배정 설계를 맹신하지 않고 정확한 증거로 수정한 좋은 사례. (2) 보정값을 프레임 끝에 벗기지 않으면
접지 스냅 임계값과 충돌해 새 버그(기상 중 갑자기 낙하)가 될 뻔한 것을 미리 잡은 것도 승인.
`ICharacterInkExtentProvider` 신규 인터페이스(액세서리가 자기 잉크 범위를 스스로 신고 — 망토가
실제 최저 파츠로 확인됨)도 확장성 있는 설계로 승인.
검증: 31점 결정론적 스윕 전부 통과(97.7도 정착각까지), 네거티브 컨트롤 13/31 위반 정확 재현,
서있는 발 위치 불변(7.24pt, 과거 실측치와 동일), 컴파일 0/0, EditMode 147/147, PlayMode 276/276.
**임시 하네스 정리**: `ZZCornerPanelCaptureHarness.cs`가 `-nographics` 배치 실행에서 `Camera.Render()`
호출로 에디터를 네이티브 크래시시킨다는 걸 발견(그래픽 켠 채로 검증했던 라운드에서는 안 드러남) —
이미 삭제 대상으로 분류돼 있던 파일이라 리더가 직접 삭제 완료.
**후속 후보**: LandingCrouch 동일계열 가설은 증거(8.82pt 로그 1건, 이미 고친 순간이동 버그의 잔상일
수 있음)가 약해 스윕으로 필요성 증명 전까지는 보류 — 코더 판단 승인.

### 2026-08-31 — 리더 직접 조치: 앱 이름 확정 (StickMate)
사용자 지시("앱이름은 StickMate로 확정")로 `ProjectSettings/ProjectSettings.asset`의
`productName: StickMateSkeleton`(초기 스캐폴딩 잔재) → `StickMate`로 수정. 빌드 산출물 파일명
(`StickMate.app`/`StickMate.exe`)은 `BuildStandalone.cs`가 이미 고정 지정해 정상이었지만, 번들
내부 실행 바이너리(`Contents/MacOS/<productName>`)가 여전히 `StickMateSkeleton`이던 불일치를
해소. 다음 통합 재빌드 때 반영(현재 test-engineer가 Unity 사용 중이라 즉시 재빌드 안 함).

---

## 2026-08-31 — 3라운드 통합 **클린 전체 검증 + 최종 리뷰** **[Test Engineer]** — `(개선 R2)`

대상: (1) 구석 호버 패널 + 크기 다이얼(코더) (2) GETUP 바닥 관통 근본수정(코더) (3) 펫/이펙트 발판
불일치(디버거). **커밋하지 않았다.**

### ★ 1단계 — `-nographics` 표준 배치가 처음으로 완주했다

| 실행 | 결과 | 로그 |
|---|---|---|
| 컴파일 | `error CS` **0** / `warning CS` **0** | `Logs/te_r5_edit.log` |
| EditMode (`-nographics`) | **147 / 147** | `Logs/te_r5_edit.xml` |
| PlayMode (`-nographics`) | **275개 중 274 통과 / 0 실패 / 1 무시**, 718.8초, `PLAY_EXIT=0` | `Logs/te_r5_play.xml` |
| 최종 클린 재확인(임시파일 제거 후) | EditMode **147/147**, 컴파일 0/0 | `Logs/te_r5_edit_final.xml` |

`referenced script ... is missing!` **0건** / `NullReferenceException` **0건** / SIGSEGV·크래시 마커 **0건**.
→ **삭제된 `ZZCornerPanelCaptureHarness.cs`가 유일한 크래시 원인이었음이 확인됐다.**

**남은 크래시 원인 없음(감사 완료)**: 테스트/에디터 전체에서 `Camera.Render` / `ReadPixels` /
`ScreenCapture`를 전수 조사한 결과 나머지 1건(`AccessoryFillRenderingTests.Capture`)은 이미
`SystemInfo.graphicsDeviceType == Null` 가드가 있다.

#### 숫자 276 → 275의 정체 (산수가 정확히 떨어진다)
코더의 276(그래픽 켠 실행)과 내 275를 **테스트 이름 집합으로 diff**했다. 차이는 정확히 1개:
`ZZCornerPanelCaptureHarness.CaptureCornerPanel`(리더가 삭제한 그 파일). **조용히 사라진 테스트는 없다.**
- 무시 1건 = `PortraitTextureResolutionTests.PortraitTextureIsSupersampledAgainstPhysicalPixelsOnRetina`
  → 본인이 `Assert.Ignore("헤드리스(-nographics)에서는 오프스크린 카메라를 켜지 않습니다.")`.
  **기존 가드이고 이번 라운드와 무관**하다(코더는 `-nographics` 없이 돌려서 이게 실행됐다).

### 2단계 — 교차 영향

**① 크기 다이얼이 실제로 화면에 나오고 작동하는가 — 확인(증거 PNG 3장).**
비침해 규약대로 **마우스를 움직이지 않고** 코더와 같은 PlayMode 캡처 하네스 방식을 재사용했다.
증거 `Logs/evidence_20260831_te_corner/{te_collapsed,te_expanded,te_dialed}.png`.
제품과 같은 포인터 경로(`FeedPointerForTests` → `ProcessPointer`)로만 조작한 결과:
...
| 켜진 눈금 | 9 / 34 | **24 / 34** |
| 허브 표시 | "0.75×" | **"1.50×"** |
| 실캐릭터 루트 스케일 | 1.000 | **2.000** (= 1.50 / 구워진 0.75) |

표시 숫자 = 켜진 눈금 수 = 실제 배율이 **실행으로도** 일치했다(절대 불변 원칙 1).

> ★ 하네스 재작성 시 함정 2개를 실측으로 밟았다 — 후속 라운드 필독.
> (a) `-nographics` 가드(삭제된 하네스가 죽은 이유), (b) **`WaitForEndOfFrame` /
> `ScreenCapture`는 batchmode에서 영원히 돌아오지 않는다**(17분 무응답 실측 후 강제 종료).
> 캡처는 반드시 **오프스크린 카메라 + RenderTexture**로 해야 한다.

**② GETUP 수정 부작용 없음 — 확인.**
- 접지 최악간격 **7.24pt** — 과거 실측치(`te_final_pm` 7.24 / `te_final_pm2` 7.24 / `te_pm2` 7.24)와 **동일**.
- 상태별 화면바닥아래최대(음수 = 안전): Idle **−6.22** / Walk **−6.71** / Getup **−6.71** /
  LandingCrouch **−6.71** / Ragdoll **−7.05** / Fall **−56.39**. 서 있는 자세·걷기 발 위치 무변경.
- 31점 스윕: **위반 0점**, 최악 −6.71pt, 최대 정착각 96.3도.
- **네거티브 컨트롤이 살아 있다**: `getupFloorClearanceEnabled=false`면 **31점 중 10점 위반**,
  최악 **+11.97pt**(정착각 −96.0도, 최저파츠 `RightArmLower`). 수정이 무의미한 no-op이 아님을 증명.
- 부수: 이번 런에서 LandingCrouch가 −6.71pt(안전)로 나왔다 → 코더의 "증거 부족으로 보류" 판단을 지지.

**③ 펫/이펙트 — 확인 + 전수 감사.**
`PetFollowsOwnerFootholdTests` P1/P1n/P2/P3 전부 통과(P2가 창 위 → 최대화 → Dock 전이를 이미 덮는다).
추가로 **오용된 API의 모든 호출부를 전수 조사**했다: `TryGetGroundSurfaceWorldY`의 남은 제품 소비자는
정확히 **1곳**(`ThrowTumbleState.TryPredictLandingSurfaceY:507`)이며 디버거가 이미 후속 후보로 기록한
바로 그 지점이다. **숨은 4번째 오용 지점은 없다.**

**④ 저장 v6 마이그레이션 — 안전.**
`data.version >= FirstVersionWithCornerPanel(6) ? data.cornerPanelEnabled : true` 분기는 v1~v5를
**균일하게** 처리한다. `characterScaleSaved`는 없으면 false가 곧 정확한 사실이라 저절로 성립. (→ m4)

**⑤ 촬영장 2개 — 안전.**
`PrimaryStage()`가 "씬에 1개"를 **X 좌표별 1개**로 다시 세운 것은 원래 지키려던 위험을 더 정확히
잠근다. **두 카메라가 동시에 도는 경로가 없다**: 880 정보창이 열려 있으면 `DetectionArmed`가 false라
구석 패널이 즉시 접히고, 구석 촬영장 카메라는 EXPANDED에서만 켜진다. (→ m3)

**⑥ `ICharacterInkExtentProvider` — 망토 특별취급 아님, 일관 적용.**
`CharacterAccessoryRenderer.TryGetLowestInkWorldY()`는 컨테이너의 `_lines`/`_fills`를 **전부 순회**하므로
32종 액세서리(모자/안경/넥타이 포함)가 이미 균일하게 포함된다 — 망토는 실측에서 **최저였을 뿐**
코드가 특별 취급하지 않는다. 펫/FX는 캐릭터 루트 밑에 그리지 않으므로 미구현이 옳다. (→ m2)

### 3단계 — 최종 리뷰

#### 좋은 점
- **"왜"가 코드에 남아 있다.** 반증된 가설, 실패한 설계 시도, 밟은 함정을 지운 게 아니라 근거로 적어
  뒀다(`ReleaseInkFloorClearanceLift`의 두 임계값, `TickInkFloorClearance`의 "전이 프레임엔 Tick이 안 돈다").
- **네거티브 컨트롤이 전부 살아 있다.** 스윕 OFF에서 10/31 위반이 실제로 재현된다.
- **"컨트롤의 컨트롤"** — `parkourMantleInsetDerived`로 기존 네거티브 컨트롤이 조용히 죽는 것을 막고,
  그 스위치가 켜져 있는지까지 배포 에셋 테스트로 잠갔다.
- **소비자가 부품 목록을 들지 않게 방향을 뒤집은 것**(`ICharacterInkExtentProvider`)은 CLAUDE.md 원칙 4에 정확히 부합.
- **`ApplyCharacterScale` 상쇄안 (b) 선택 근거**("빠뜨린 곳이 배율 1.0에서는 정상으로 보인다")가 정확하다.
- 프레임 계약 2줄(벗기기/얹기)의 **위치가 곧 정합성**임을 테스트 실패 메시지에 원인 후보로 남겼다.
...
- **M1. 눈금이 하나도 없는 빈 구역을 클릭하면 캐릭터 크기가 끝값으로 튄다 — 실행으로 재현했다.**
  `SizeDialWidget.IsInRing()`은 **반지름만** 본다(각도를 보지 않는다). 그래서 12시 96° 빈 구역
  — 다이얼 허브와 미리보기 카드 사이의 **눈에 띄게 비어 있는 공간** — 도 조작 원환으로 잡히고,
  "짧은 클릭 = 그 눈금으로 점프"가 발동한다. 그런데 그 자리엔 눈금이 없어 `IndexForAngle`의
  `Clamp(-132, 132)`가 **가장 가까운 끝값**을 답한다.
  실측(`Logs/te_r5_wedge.log`): 펼친 패널에서 12시 r=60pt를 **한 번 클릭** →
  `1.50× → 0.35×`, 실캐릭터 루트 스케일 0.467 / `config.characterScale=0.350`까지 그대로 적용됐다.
  더 나쁜 점: θ가 정확히 ±180°라 **어느 끝값으로 갈지가 부동소수 부호로 갈린다**(0.35× 또는 2.00×).
  → 제안: `IsInRing`에 각도 조건(`|θ| ≤ SweepHalfDegrees`)을 더하거나, `IndexForAngle`이 빈 구역을
  **거부**(값 유지)하게 한다. 수정 후 "빈 구역 클릭은 값을 바꾸지 않는다" 회귀 테스트 필수.

- **M2. 상수 4개가 말없이 맞물려 있다 — 하나만 바뀌면 M1이 카드 위로 번진다.**
  다이얼 히트 원환의 꼭대기와 카드 바닥이 **정확히 접한다(둘 다 패널 바닥 168pt)**:
  `ExpandedHeightPoints(392) − Space3(12) − CardHeightPoints(212) == DialCenterFromBottomPoints(78) + HitOuterRadius(90)`.
  이 등식은 **어디에도 적혀 있지 않고 테스트도 없다**. 게다가 `CardHeightPoints = 212`는
  **다른 클래스의 값**(`CharacterInfoWindow.PortraitContentSize.y = 180`, + 여백 4 + 캡션 28)을
  주석으로만 연결한 채 숫자로 박아 뒀다 — 그쪽 액자 크기를 바꾸면 여기가 조용히 어긋난다.
  어긋나는 순간 다이얼 원환이 카드 아래를 먹고, **M1 때문에 그 클릭이 끝값 점프가 된다**.
  → 제안: `CardHeightPoints`를 `PortraitContentSize.y`에서 유도하고, 위 등식을 EditMode 불변식 테스트로 잠근다.

**Minor**

- **m1.** 같은 클래스에 `_stage`(`CharacterPortraitStage`)와 `_stage_`(호버 `Stage` 열거형)가 **끝의
  밑줄 하나로만** 구분되어 공존한다(`CornerHoverPanel.cs:177,180`). 읽는 사람이 매번 멈춘다.
  → `_portraitStage` / `_hoverStage` 권장.
- **m2.** `StickmanBlackboard._inkExtentProviders`가 **한 번 수집하고 영원히 캐싱**된다(무효화 경로 없음).
  런타임에 잉크 제공자를 붙이는 부품(= DLC 플러그인, CLAUDE.md 원칙 4의 바로 그 시나리오)은
  **조용히 제외**된다 — 이 인터페이스가 막으려던 실패 유형 그 자체다. 지금 구현체가 프리팹에 굽는
  1개뿐이라 현재는 무해하지만, 확장 지점에 함정이 남아 있다.
- **m3.** `PrimaryStage()` 헬퍼가 3개 테스트 파일에 **바이트 단위로 동일하게** 복제돼 있다
  (md5 3개 일치). X 좌표 규칙이 바뀌면 3곳을 고쳐야 한다. 또 이 단언은 "primary X에 1개"만 보고
  **secondary가 제자리에 있는지도, 전체 개수 상한도** 보지 않아 엉뚱한 X의 3번째 촬영장은 통과한다.
- **m4.** v6 마이그레이션 테스트가 **v5 하나만** 고정한다. 분기가 v1~v5 균일이라 논리적으로는
  안전하지만, `[TestCase(1)]~[TestCase(5)]` 스윕이면 근거가 실행으로 남는다.
- **m5.** 펼침 애니메이션 중 **그려지는 다이얼이 `PanelScreenRect` 밖으로 나간다**(증거
  `te_collapsed.png`: 패널 몸통 41.97pt인데 다이얼은 바닥 78pt 기준 고정). 차단막과 `BeginDrag`
  게이트가 둘 다 `PanelScreenRect`라 **클릭이 새지는 않지만**, "보이는데 안 눌리는" 픽셀이 생긴다.
- **m6.** (이번 라운드 산물 아님, 커밋 위생) 루트에 `mono_crash.a388a3d94.0.json`(삭제된 하네스가 낸
  크래시 덤프)이 **추적되지 않은 채** 남아 있고 `.gitignore`에 없다 → `git add -A`에 딸려 들어간다.
  삭제 + `.gitignore`에 `mono_crash.*.json` 추가 권장.
- **m7.** `ProjectSettings.asset`의 `productName`이 `StickMateSkeleton` → `StickMate`로 드리프트한다.
  **에디터를 열 때마다 결정론적으로 재발한다**(되돌린 뒤 배치 실행 1회로 즉시 재현). 저장소의
  `Assets/`·`ProjectSettings/` 어디에도 이 값을 쓰는 코드가 없고 빌드 프로파일 에셋도 없다 —
  원인 미확정이라 **되돌리지 않고 그대로 뒀다**(되돌려도 다음 실행에서 돌아온다).
  세 라운드와 무관한 변경이므로 **기능 커밋에 조용히 섞여 들어가면 안 된다**. 리더 판단 필요:
  (a) `StickMate`가 맞는 이름으로 보이고(`StickMateSkeleton`은 스캐폴드 잔재) 별도 커밋으로 분리하거나,
  (b) 원인을 별건으로 조사한다.
- **m8.** `CornerHoverPanel.EndDrag`에 빈 `if` 한 줄이 있다 —
  `if (committed && _dial.IsOnHub(cursor)) { /* 원환 안에서 시작했으므로 여기 오지 않는다 */ }`.
  주석은 유용하지만 본문 없는 분기는 삭제하고 주석만 남기는 편이 낫다.

### 결론 — **최종 완료 아님. 아키텍트로 반려 `(개선 R2)`**
1단계(크래시 해소·전체 완주)와 2단계(교차 영향 6항목) 는 **전부 통과**했고 세 라운드의 기능적 주장은
실측으로 재현됐다. 다만 **Major 2건(M1 실행 재현 완료 / M2 무언의 상수 결합)** 이 남아 커밋 전 수정을
권고한다. Minor 8건은 리더 판단.

### 2026-08-31 — 리더 직접 조치: R2 Major 2건(M1/M2) 수정 완료
**M1**: `SizeDialWidget.IsInRing()`에 각도 검사 추가(반지름만 보던 것 → `|AngleOf| <= SweepHalfDegrees`도
함께 확인). 빈 96° 구역은 이제 "원환 밖"으로 취급돼 드래그 시작 자체가 안 걸린다 — 이미 진행 중인
드래그가 빈 구역으로 회전해 들어가는 것(끝값에 붙는 게 정상 동작)은 그대로 유지.
**M2**: `ExpandedHeightPoints`/`CardHeightPoints`/`CardRisePoints`/`DialCenterFromBottomPoints`
(CornerHoverPanel)와 `HitOuterRadius`(SizeDialWidget)를 public으로 노출하고 상호 참조 문서화, 신규
잠금 테스트 `카드_하단과_다이얼_원환_상단이_정확히_맞닿는다`(CornerHoverPanelTests.cs)로 등식
(392−12−212 == 78+90)을 실행으로 고정. 직접 실행 검증: 신규 테스트 통과, `CornerHoverPanelTests`
전체 6/6 무회귀. (주의: 초안에 CardHeightPoints=212와 PortraitContentSize.y=180의 차이를 "캡션22+
여백10"이라고 적었다가, 실제 `UiChrome.Space1=4`와 안 맞는 걸 발견해 근거 없는 구체적 수치 주장을
삭제함 — 검증 없이 단정하지 않는 습관 유지.)
남은 백그라운드 작업(모자 회전시 사라짐 조사, 윈도우 실사용 버그 4건) 완료 대기 후 전체 통합 검증 +
커밋 예정.

---

## 2026-08-31 — 디버거: "캐릭터 회전할때 모자가 없어짐" 원인 규명 **[Teammate2 / Debugger]**

사용자 신고: **"캐릭터 회전할때도 모자착용중인데 모자가 없어짐"**

### 과학적 토론 로그

| # | 가설 | 검증 방법 | 결과 |
|---|---|---|---|
| **H1** | 좌우 반전(`Rig.F()`의 `x * Facing`)이 삼각형 **와인딩**을 뒤집고, 채움 재질이 backface culling을 켜고 있어 뒤집힌 쪽에서 GPU가 채움을 버린다 (리더 유력 가설) | (a) 좌우 반전 **20회 반복** PlayMode 테스트에서 실제 씬의 `MeshFilter.sharedMesh`를 읽어 삼각형 부호넓이를 전수 계산. (b) 같은 메시의 **와인딩만 뒤집어** GPU로 두 번 렌더하고 픽셀 수 비교 | **반증(2중으로)**. (a) 20회 × 채움 2개 전부 **CW 삼각형 0개**(항상 CCW) — `AccessoryShapeBuilder.Triangulate`가 `SignedArea` 부호로 인덱스 순서를 **정규화**하고 있었다. (b) 정상 와인딩 **214,815px** vs 뒤집은 와인딩 **214,425px**(차 0.18%) — 재질은 `Sprites-Default`/`Sprites/Default` = **Cull Off(양면)**. 증거 PNG `Logs/evidence_20260831_dbg_flip/flip_winding_{normal,reversed}.png`에 뒤집은 와인딩으로도 모자가 **온전히 그려진다** |
| **H1-보조** | 좌우 반전 시 채움 오브젝트가 생성되지 않거나 꺼진다 | 같은 20회 루프에서 `enabled`/`activeInHierarchy`/재질/정점색/챙 중심 x부호 전수 확인 | **반증**. 매 회 채움 2개(관·챙)가 존재·활성·유효하고, 챙 중심 x 부호가 방향을 정확히 따라간다 |
| **H2** | 사용자가 말한 "회전"은 좌우 반전이 아니라 **몸이 실제로 도는 상태**(RAGDOLL / 던져서 공중회전)이고, `CharacterAccessoryRenderer.ResolveWantVisible()`이 그 상태에서 액세서리를 **의도적으로 숨긴다** | 상태 전수 스윕 + 실제 던지기 재현(공중에서 `ThrowTumble` 진입, 3초간 매 프레임 표본) | **확인 — 근본 원인**. 던져서 루트가 **180도까지 도는 동안 회전 프레임의 78%에서 모자 채움이 0개**. 랙돌도 동일(2개 → 0개, 컨테이너 비활성) |
| **H2-a** | 그렇다면 ThrowTumble에서도 숨겨야 정당하다(모자가 몸에서 떨어져 보일 것이다) | 숨김을 풀고 회전 180도 내내 **머리 로컬 좌표계**에서 모자 정점 위치의 이탈을 측정 | **반증**. 이탈 최대 **머리 반경의 1.5%**(0.0011유닛) — 모자는 머리에 **붙어서 함께 돈다**. ThrowTumble은 랙돌이 아니라 **능동 모드**이고 회전을 `ThrowTumbleState`가 **루트 하나**의 시각 회전으로 주기 때문(`StickmanBlackboard.TickPose`가 `EnterActiveMode()` 뒤 `SnapRootUpright()` **직전**에 빠져나간다). 숨길 근거가 애초에 없었다 |
| **자기 반증 기록** | 첫 이탈 측정에서 27.9%가 나와 "역시 떨어진다"고 볼 뻔했다 | 지표 재검토 | **내 지표가 틀렸다**. `Renderer.bounds`는 **축정렬** 박스라 강체로 붙어 있어도 회전하면 중심이 움직인다. 실제 **메시 정점 하나**를 월드→머리로컬로 변환하니 1.5%였다. 회전이 얽힌 측정에 AABB를 쓰면 안 된다 |

### 근본 원인 (Blocker)

`Assets/_Project/Scripts/Interaction/CharacterAccessoryRenderer.cs:334`(수정 전)

```csharp
if (id == StickmanStateId.Ragdoll || id == StickmanStateId.ThrowTumble) return false;
```

최초 액세서리 커밋(`1154629`)에서 **"회전하는 상태"라는 이유로 두 상태를 한 줄에 묶었다**. 그런데
클래스 문서 (4)가 적어 둔 숨김 근거 — "전신을 물리에 위임해 머리·몸통이 루트에서 **독립적으로**
굴러가므로 루트 로컬로 그리는 모자가 몸에서 분리돼 보인다" — 는 **랙돌에만** 성립한다.
ThrowTumble은 능동 모드 강체 회전이라 그 근거가 성립하지 않는다. 그래서 사용자는
"던졌더니 회전하는 동안 모자만 없어지는" 그림을 봤다.

### 수정

`ThrowTumble`만 숨김 목록에서 제거(랙돌은 그대로 숨긴다 — 그쪽 근거는 유효). 클래스 문서 (4)에
제거 근거와 실측치를 함께 기록.

### 검증

- **네거티브 컨트롤**: 수정을 되돌리자 같은 테스트가 다시 **78% 숨김**으로 실패(`Logs/dbg_throw_neg.log`, `failed="1"`). 되돌리면 정확히 재현된다.
- **회귀 잠금 신설**: `Assets/_Project/Scripts/Tests/PlayMode/AccessoryFacingFlipFillTests.cs`
  (좌우 반전 20회 / 와인딩-반전 GPU 렌더 / 상태 전수 스윕 / 던지기 회전 재현·이탈 측정)
- **육안 증거**: `Logs/evidence_20260831_dbg_flip/throw_spin_hat_visible.png` — 기울기 60도 넘긴 회전 중간 프레임에서 모자가 머리에 붙어 함께 기울어 있다.
- 컴파일 **error CS / warning CS 0건**, EditMode **147/147 통과**.
- **PlayMode 전체 293개 중 8개 실패 — 전부 내 변경과 무관함을 A/B로 증명했다.**
  같은 8개 스위트를 **수정 적용판**(`Logs/dbg_ctl_withfix.xml`)과 **되돌린판**(`Logs/dbg_ctl_nofix.xml`)으로
  각각 돌린 결과 **실패 집합이 문자 그대로 동일**(8/8, 차집합 양쪽 다 공집합)했다. 실패 내용도
  전부 다른 라운드의 진행 중 작업이다 — 배율 다이얼 4건(`characterScale=0.35`가 프리팹에 안 구워짐),
  Dock 물리 2건, 넘어짐 초상화 프레이밍 1건, 보행 발미끄러짐 1건. 액세서리/ThrowTumble과 접점이 없다.
  (※ 작업 트리에 test-engineer/coder의 미커밋 변경이 함께 올라와 있는 상태에서 잰 값이다.)

### 교차 레이어 영향 로그

- `ICharacterInkExtentProvider.TryGetLowestInkWorldY()`가 이제 ThrowTumble 중에도 `true`를 돌려준다
  (그 전에는 액세서리가 숨겨져 `false`). 유일한 소비자는 `States/GetupState`의 바닥 클리어런스이고
  Getup과 ThrowTumble은 동시에 성립할 수 없으므로 현재 영향 없음. **소비자가 늘면 재확인 필요.**
- `Interaction/CharacterFxRenderer.cs:218`이 **같은 패턴**(`id != Ragdoll && id != ThrowTumble`)으로
  FX를 끈다. 이번 수정 범위 밖(FX는 "회전 중 먼지/땀"이라 판단 근거가 다르다)이지만,
  **같은 뿌리의 과잉 제외일 가능성**이 있어 리더 판단 요망.

### 남은 관찰

- **랙돌 중 모자 숨김은 그대로 남아 있다.** 사용자가 "넘어져서 뒹구는 동안에도 모자가 사라진다"를
  문제로 본다면 그건 별건이다 — 고치려면 액세서리를 **랙돌 부위별 앵커**에 붙여야 하고(클래스 문서 (4)),
  설계 판단이 필요하다. 디버거 단독으로 진행하지 않았다.

---

## 2026-08-31 — 실제 Windows PC 첫 실행 신고 4건 조사/수정 **[Coder]**

사용자가 `Builds/Windows/StickMate.exe`(2026-08-30 23:03 빌드)를 실제 Windows PC에서 처음 실행하고
신고한 4건. **커밋 없음(리더 통합용).**

### ★ 조사 전제 — 사용자가 실행한 그 바이너리가 무엇을 담고 있는지부터 확인했다

추측 대신 빌드 산출물의 심볼을 직접 조회했다(`strings StickMate_Data/Managed/StickMate.Runtime.dll`):

| 심볼 | 그 .exe에 | 의미 |
|---|---|---|
| `CloseOverlappingSurfaces` / `ForceCloseAll` | **있음** | 08-30 배타 모달 수정은 사용자가 돌린 빌드에 **들어 있다** |
| `DockPhysicsStep` | **있음** | Dock 물리 계단도 들어 있다 |
| `CornerHoverPanel` / `SizeDialWidget` | **없음** | 08-31 구석 호버 패널 라운드는 **들어 있지 않다** |

→ **신고 ④(랙)의 배정 가설("구석 호버 패널이 카메라를 하나 더 상시 렌더링한다")은 이 시점에서
반증된다.** 그 코드가 사용자가 실행한 바이너리에 존재하지 않는다. (그럼에도 소스에서 따로 검증한
결과는 아래 ④에 적었다 — 다음 Windows 빌드에서도 문제가 되지 않는다.)

---

### ① "작업표시줄에 걸쳐서 돌아다닌다" — **원인 확정 + 수정 완료**

**원인(코드 추적으로 확정, 추측 아님).** `Win32WindowService`는 작업표시줄을 조회하는 코드가 아예
없었고 `IDockMetricsService`도 구현하지 않았다. 그래서 발판 단일 소스
`FallbackPlatformWindowService.TryGetDockRectOsScreen()`이 **2순위 폴백**으로 떨어진다:

```
width  = screenW * dockFootholdWidthFraction(0.65)   // 화면 가운데 65%
thickness = dockFootholdThicknessPoints(75)
```

이건 **macOS Dock의 모양**이다. Windows 작업표시줄은 화면 **가로 전체**이고 두께도 75pt가 아니다
(Win11 100% 배율 = 48px). 결과가 신고 문구 그대로다:
- 화면 좌우 각 17.5% 구간 → 발판이 "화면 최하단 안전망"이라 캐릭터가 **작업표시줄 안**에 선다.
- 가운데 65% 구간 → 두께가 실제와 달라 작업표시줄 위/아래로 어긋난다.
- 작업표시줄이 자동 숨김이거나 좌/우/상단에 있어도 **똑같이 가운데 65% 막대**를 만든다(허공 부양).

**수정.** `Platform/IReservedBottomBarService.cs` **신설**(선택적 캐퍼빌리티, 기존 관례 그대로).
`Win32WindowService`가 구현: `MonitorFromWindow(오버레이 창)` → `GetMonitorInfo` →
**`rcMonitor` 하단 − `rcWork` 하단** = 하단 예약 띠의 두께. 추정이 한 글자도 없다.
이 한 번의 조회가 별도 분기 없이 다 처리한다: 좌/우/상단 배치(차이 0 → 막대 없음) / **자동 숨김**
(Windows가 작업 영역을 줄이지 않으므로 차이 0) / 125%·150% 배율(둘 다 물리 픽셀이라 자동 추종) /
**하단 도킹 툴바**(그것도 예약 영역이라 함께 피함) / 멀티 모니터(우리 창이 놓인 모니터 기준).

`FallbackPlatformWindowService.TryGetDockRectOsScreen()`에 **0순위**로 배선. 중요한 설계 결정:
이 경로에서 `false`는 **"추정해 보라"가 아니라 "하단 막대가 지금 없다"는 확정 신호**라 즉시
`return false`한다(추정으로 흘려보내면 없는 막대 위에 부양한다). 그 뒤로는 기존 구조가 전부 공짜로
따라온다 — 안전망의 구멍/`DockPhysicsStep`의 물리 계단이 **같은 단일 소스**에서 나오므로,
작업표시줄이 화면 전체 폭이면 안전망 두 조각이 폭 0이 되어 사라지고 물리 계단이 전체 폭으로 솟는다
(= macOS Dock 구간에 남아 있는 사각지대가 Windows에는 애초에 생기지 않는다).

**macOS 무영향 보증**: `MacWindowService`는 이 인터페이스를 구현하지 않으므로 `_innerBottomBar`가
null이고 새 블록 자체가 실행되지 않는다. 타일 실측 경로는 한 글자도 바뀌지 않았다.

**회귀 테스트 신설** `Tests/PlayMode/ReservedBottomBarFootholdTests.cs`(6건, 전부 플랫폼 중립 —
스텁 주입으로 이 macOS 환경에서 실제로 돈다): ①정확한 사각형이 그대로 발판 ②**네거티브 컨트롤**:
전체폭 막대가 65% 추정으로 좁아지지 않음 ③두께가 75 추정이 아님 ④**막대 가로 구간 안에 그보다 낮은
합성 발판이 하나도 남지 않음**(신고 증상 자체) ⑤"막대 없음" 확정 시 추정으로 흘러가지 않고 최하단
안전망은 살아 있음 ⑥**macOS 대조군**: 캐퍼빌리티 없는 서비스는 예전 추정 경로 그대로.

---

### ③ "캐릭터창 해상도도 엄청 낮아서 글씨도 잘 안보임" — **원인 확정 + 수정 완료**

**원인(코드 추적으로 확정).** 이 앱의 UI 상수는 전부 **논리 포인트** 기준이고, 캔버스 배율은
`ScreenCoordinateConverter.ResolveCanvasScaleFactor()` 하나가 정한다. 그 값이 지금까지
`1 / AutoDpiScale`, 즉 **창 사각형(OS 단위) 대 `Screen.width`(Unity 픽셀)의 비**에서만 나왔다.

- **macOS**: 창 사각형은 AppKit 포인트(1512), `Screen.width`는 백킹 픽셀(3024) → 비 0.5 → 배율 2.
  **디스플레이 배율이 이 비에 실려 온다.**
- **Windows**: `GetWindowRect`도 `Screen.width`도 **둘 다 물리 픽셀** → 비가 **항상 1.0** → 배율 1.
  **디스플레이 배율(125%/150%)이 어디에도 실리지 않는다.**

즉 150% PC에서 "14pt로 보여야 할 글자"가 14 **물리 픽셀**로 그려진다 = 의도한 크기의 1/1.5.
같은 배율이 초상화 RenderTexture의 슈퍼샘플 배수이기도 해서 **실제로 덜 선명하기까지 했다**.
(이 프로젝트 어디에도 Windows DPI를 조회하는 코드가 없었다 — `WindowsOverlayStateEnforcer`의 주석도
"Windows에서는 배율이 1.0으로 실측된다"고 그 사실을 이미 적어 두고 있었다.)

**수정.** 두 개념을 이름부터 분리했다.
- `AutoDpiScale` — **좌표** 변환 전용. Windows에서 1.0인 것이 **맞다**(창 좌표와 커서 좌표가 같은 단위).
- `AutoUiDensityScale` **신설** — **UI 크기** 전용, "논리 포인트 1개 = 물리 픽셀 몇 개".
  `Win32WindowService`가 `GetDpiForWindow(hwnd)/96`으로 OS에서 직접 읽어 보고한다.

`ResolveCanvasScaleFactor` 우선순위 = 수동 오버라이드(`desktopDpiScale`) > 보고된 밀도 > 예전 정의.
`UnityScreenToCanvas`/`CanvasToUnityScreen`도 같은 값에서 나오게 다시 표현했다(배치와 크기가
갈라지면 UI가 커지면서 화면 밖으로 밀려난다). **밀도가 보고되지 않은 환경에서는 항등식이라
macOS/에디터/모바일 동작이 한 글자도 바뀌지 않는다** — 회귀 테스트로 잠갔다.

**안전 설계**: Unity Player가 DPI 인식을 하지 **않는** 환경이면 OS가 창 좌표를 가상화하고
`GetDpiForWindow`도 96을 돌려주므로 밀도 1.0 = 현행 유지. 즉 이 수정은 "틀릴 수 있는 상황에서는
아무것도 바꾸지 않는" 방향으로만 개입한다. `GetDpiForWindow`가 없는 OS(Win10 1607 미만)는
`EntryPointNotFoundException`을 잡아 96으로 폴백하고 한 번만 경고한다.

**회귀 테스트 신설** `Tests/PlayMode/UiDensityCanvasScaleTests.cs`(5건): ①보고된 밀도가 캔버스 배율이
되고 **좌표 배율은 오염되지 않음** ②배치 변환이 캔버스 배율의 정확한 역(단위 왕복) ③수동 오버라이드
우선 ④**macOS 무회귀**(미보고 시 예전 정의와 값이 같음) ⑤쓰레기 값 방어.

---

### ② "캐릭터 창도 맥에서처럼 여러창으로 겹쳐보임" — **원인 미확정(정직한 보고) + 구멍 1건은 수정**

**이 환경에서 확인한 것(전부 실측/코드 판독, 추측 아님):**

1. **08-30 배타 모달 수정은 그 .exe에 들어 있다**(위 심볼 표). "수정이 빠진 빌드였다"는 설명은 반증.
2. **캔버스 렌더 모드/정렬 순서에 플랫폼 분기가 0건이다.** 전체 grep 결과 `renderMode =`가 나오는
   6곳(말풍선/앱제어/정보창/부채꼴/포스트잇/팝오버) 전부 무조건 `ScreenSpaceOverlay`이고
   `sortingOrder`는 컴파일 상수다. `Interaction`/`Dialogue` 전체에서 `UNITY_STANDALONE_WIN` 분기
   **0건**(`#if UNITY_EDITOR` 1건뿐, 앱 종료 경로). → 배정문 가설 "캔버스 렌더 모드가 플랫폼별로
   다르게 설정된다"는 **반증**.
3. **"DPI로 클릭 판정 좌표가 어긋난다"도 반증.** 수정 전 Windows에서는 캔버스 배율=1,
   `GetCursorPos`=물리 픽셀, `OsScreenToUnityScreen`의 dpi=1, 히트테스트 사각형도
   `CanvasToUnityScreen(v)=v*1` → **전부 물리 픽셀로 자기 일관**이었다. 작게 그려졌을 뿐 좌표는 맞았다.
   (③ 수정 후에도 일관성이 유지되는지 따로 확인했다 — `GetWorldCorners` 기반 히트테스트는 캔버스
   localScale이 이미 곱해져 나오고, 수동 계산 경로는 전부 `CanvasToUnityScreen`을 거친다.)
4. **창이 두 벌 생기는 경로도 반증.** 런타임 스크립트에 `Instantiate`가 0건이라 캐릭터 복제(세포분열)는
   아직 구현되어 있지 않고, `CharacterInfoWindow.Awake`의 2차 방어도 그대로 있다.

**그래서 결론: 이 macOS 환경에서 Windows 전용 원인을 특정하지 못했다. 확정하지 못한 것을 확정한 척
고치지 않았다.** 다음에 Windows에서 재현할 때 확인할 것을 우선순위대로 남긴다:

- **(가장 유력) 표면 합성 아티팩트 가설.** Windows 투명 창을 위해 빌드 설정이
  `useFlipModelSwapchain = false`(레거시 **BitBlt** 스왑체인)를 강제한다. 레이어드 창 + BitBlt 조합은
  프레임이 완전히 갈리지 않아 **이전 프레임이 잔상으로 남는(ghosting)** 것이 알려진 실패 모드이고,
  그러면 창 하나가 "여러 창이 겹친 것"처럼 보인다. 이 가설은 신고 ④(랙)와 **같은 원인**을 공유한다.
  검증법: Windows에서 정보창을 **드래그**해 보라. 잔상이면 끌리는 궤적을 따라 창이 번지고, 진짜로
  여러 캔버스가 켜진 것이면 각 창이 제자리에 남는다. (이 한 번의 관찰로 두 가설이 갈린다.)
- **(차선) 진짜로 여러 표면이 켜져 있다.** Windows에서 `[정보창] 열림` / `[부채꼴]` / `[팝오버]` 로그를
  Player.log에서 시간순으로 대조하면 즉시 확정된다 — 이미 모든 표면이 열림/닫힘 로그를 남긴다.

**함께 발견해 수정한 진짜 구멍 1건(다만 사용자의 그 빌드에는 없는 코드다).**
08-30 라운드가 "정리 책임을 여는 쪽 한 곳에"라고 못박고 만든 `CloseOverlappingSurfaces()`에
**08-31에 추가된 네 번째 표면(구석 호버 패널)이 들어가 있지 않았다** — 그 라운드 문서가 예고한
"네 번째 진입점이 생기면 또 샌다"가 그대로 일어났다. 평소에는 패널의 `DetectionArmed`가 정보창을
보고 스스로 접혀 눈에 띄지 않지만, **다이얼을 끌고 있는 동안에는 그 자기 치유가 통째로 멈춘다**
(`TickHoverStateMachine` 첫 줄이 `_dragActive`면 early-return — 끌다가 영역을 벗어나는 것이 정상
동작이라 일부러 그렇게 만든 것이다). 그 상태에서 단축키/우클릭으로 정보창을 열면 액자 두 개가 겹친
채 남고 **초상화 카메라가 2대 동시에 돈다**. → `CornerHoverPanel.ForceHide()` 공개 +
`CloseOverlappingSurfaces()` 선두(부채꼴 early-return **위**)에 배선.

---

### ④ "실행시 렉도 생김" — **배정 가설 반증 + 실측 3건, 원인 미확정**

**(가) 배정 가설 "구석 호버 패널이 카메라를 하나 더 상시 렌더링" → 반증 2중.**
1. 그 코드가 사용자가 실행한 .exe에 **없다**(위 심볼 표).
2. 소스로도 성립하지 않는다 — `CornerHoverPanel.SetStage()`가 상태가 바뀔 때마다
   `_stage.SetRenderingEnabled(next == Stage.Expanded)`를 부르고(`:473`), 생성 직후에도
   `SetRenderingEnabled(false)`(`:986`)다. `CharacterPortraitStage.SetRenderingEnabled()`는
   `_camera.enabled`를 직접 끈다(`:231`). 게다가 `DetectionArmed`가 "880 정보창이 열려 있으면
   감지 자체를 끈다"로 두 액자의 **상호배제**를 이미 세워 뒀다(34-6-5).

**(나) `Update()`마다 할당 → 사실상 0건.** 전체 `Update/LateUpdate/FixedUpdate` 본문을 스크립트로
훑어(할당 패턴 5종: `new` / LINQ / 문자열 보간 / `ToString` / `GetComponent·Find`) 34건이 걸렸는데
**전수 확인 결과 실제 힙 할당은 0건**이다: 31건이 조건부 이벤트 로그(그 프레임에만 실행),
3건이 `new Vector3(...)`(구조체 — 힙 할당 아님). 24시간 상주 컨벤션이 지켜지고 있다.

**(다) OS 폴링도 규율돼 있다.** 발판 열거 0.3초(`footholdPollInterval`), 전체화면 감지
1.5초(`fullscreenPollInterval`). 매 프레임 OS를 때리는 경로는 커서 좌표 조회뿐이고 그건
`GetCursorPos` 한 번이다.

**(라) ★ 실측 — 실제 macOS Player 로그에서 프레임레이트를 역산했다.**
`~/Library/Logs/DefaultCompany/StickMateSkeleton/Player.log`(실제 앱 실행 기록, 단일 세션,
마지막 프레임 번호 **4,500,211**)에서 말풍선의 "표시(frame=N)"와 "제거(노출 X초, frame=M)" 쌍
**20개**를 뽑아 `(M−N)/X`를 계산했다:

```
114.9 / 114.4 / 115.5 / 116.7 / 119.1 / 116.0 / 116.5 / 115.8 / 115.8 / 119.2
117.2 / 115.6 / 115.9 / 115.0 / 117.7 / 119.0 / 119.3 / 120.7 / 115.8 / 116.1  (fps)
```

→ **지속 ~116fps, 단일 세션 약 10.8시간 연속 구동.** 즉 이 앱은 **프레임레이트 상한이 전혀 없다**.
`Application.targetFrameRate`를 설정하는 코드가 프로젝트 전체에 **0건**이고, Standalone 기본 품질
레벨은 5(Ultra)라 `vSyncCount=1` → **디스플레이 주사율 그대로**(이 머신은 120Hz) 돈다.
`runInBackground`도 켜져 있어 포커스가 없어도 같은 속도로 돈다. MSAA는 전 품질 레벨 4x.

**왜 macOS에서는 문제가 안 되고 Windows에서는 랙으로 느껴질 수 있는가(가설, 미검증).**
Windows 빌드는 투명 창을 위해 `useFlipModelSwapchain = false`(레거시 **BitBlt** 스왑체인)를
강제한다 — Unity 공식 문서가 이 모델을 "성능이 떨어질 수 있다"고 명시하는 그 설정이며,
전체 화면 크기의 **레이어드 최상위 창**을 매 프레임 DWM이 CPU측 복사로 합성한다. 여기에 상한 없는
주사율 + MSAA 4x가 곱해진다. macOS(Metal/Quartz)에는 이 복사 경로가 없어 M2 Pro에서 116fps가 공짜다.

**고치지 않은 이유(리더 판단 요망).** 상한을 걸려면 `vSyncCount=0` + `targetFrameRate=N`을 **함께**
바꿔야 한다(`vSyncCount=1`이면 `targetFrameRate`는 **무시된다** — 그냥 넣으면 아무 일도 안 일어난다).
이건 (a) 품질 설정을 런타임에 건드리는 일이고 (b) 이 사용자의 120Hz Mac에서 부드러움이 눈에 띄게
줄어드는 **UX 변경**이며 (c) 무엇보다 **Windows 랙의 원인이라고 확정된 바가 없다**. 측정 없이
지르지 않았다. 리더가 원하면 `StickConfig`에 상한 필드 1개 + `StickmanAgent`에서 두 줄이면 된다.

**Windows에서 다음에 확인할 것(이 순서대로 하면 원인이 갈린다):**
1. `Builds/Windows/StickMate.exe`를 켜고 **작업 관리자 > 성능 > GPU**를 본다. GPU 사용률이 계속
   높으면 (라)의 렌더 경로 문제다.
2. 정보창을 드래그해 잔상이 남는지 본다(②의 갈림길과 같은 관찰 — 한 번에 두 신고를 가른다).
3. Player.log의 `[Win32WindowService] UI 밀도 갱신` 줄에서 디스플레이 배율을 확인한다
   (③ 수정이 실제로 먹었는지 + 그 PC의 배율이 몇 %인지가 한 줄에 나온다).

---

### 교차 레이어 영향 로그 (즉시 보고)

1. **[네이티브/플랫폼]** `IReservedBottomBarService` 신설. Windows만 구현하고 macOS/모바일/Null은
   구현하지 않는다 — 기존 선택적 캐퍼빌리티 관례(`as` 캐스팅) 그대로다.
   `FallbackPlatformWindowService`가 순수 통과도 함께 노출한다.
2. **[렌더/UI, 중요]** `ResolveCanvasScaleFactor`/`UnityScreenToCanvas`/`CanvasToUnityScreen`의
   **정의가 바뀌었다**(값은 macOS에서 동일). 앞으로 "논리 포인트 ↔ Unity 픽셀"은 `AutoDpiScale`이
   아니라 **캔버스 배율**을 따른다. 좌표(발판/커서/오버레이 원점)는 여전히 `AutoDpiScale`이다.
   **두 배율을 혼동해 쓰면 Windows에서만 어긋난다** — 새 코드는 반드시 목적에 맞는 쪽을 쓸 것.
3. **[UI/상호배제]** `CharacterInfoWindow.CloseOverlappingSurfaces()`가 이제 구석 호버 패널까지
   닫는다. 정보창을 여는 모든 경로에서 그 패널이 즉시 접힌다(드래그 중이어도).
4. **[Windows 발판 형상 변화]** 작업표시줄이 화면 전체 폭이므로 Windows에서는 바닥 안전망 두 조각이
   모두 폭 0이 되어 사라지고 하단 막대 발판 1개 + 전체 폭 물리 계단이 남는다. macOS에 있는
   "Dock 구간 사각지대"가 Windows에는 구조적으로 존재하지 않게 된다.

### ★ 이 환경에서 검증 **불가능한** 것 (정직한 한계)

- ①③은 `#if UNITY_STANDALONE_WIN` 안의 코드라 **이 macOS 머신에서 실행할 수 없다.** 여기서 확인된
  것은 (a) 컴파일 통과, (b) macOS 동등 로직과의 대조, (c) **플랫폼 중립 부분(데코레이터/좌표 유틸)을
  스텁 주입으로 실제 실행해 검증**한 것까지다. `GetMonitorInfo`/`GetDpiForWindow`가 실기에서
  기대한 값을 돌려주는지는 사용자가 Windows에서 확인해야 한다(위 "다음에 확인할 것" 3번 로그 한 줄로
  즉시 판별된다).
- ②④는 원인 미확정. 위에 가설과 **갈림길 관찰법**을 남겼다.

### 2026-08-31 — 리더 승인: 회전시 모자 사라짐 버그 완전 해결
리더가 준 가설(와인딩 반전+컬링)은 이중 반증됨(와인딩은 이미 CCW로 정규화됨 + 재질 자체가 양면).
진짜 원인은 `CharacterAccessoryRenderer.ResolveWantVisible()`가 "회전하는 상태"라는 이유로 Ragdoll과
ThrowTumble을 한 줄로 묶어 숨겼던 것 — 그 숨김 근거(부위가 루트에서 독립적으로 구른다)는 랙돌에만
성립하고, ThrowTumble은 루트 하나의 강체 회전이라 성립하지 않음을 실측(180도 회전 내내 어긋남 머리
반경의 1.5%)으로 확인 후 ThrowTumble만 숨김 목록에서 제거. 자기 첫 측정(AABB 기반, 27.9% 이탈로
오판할 뻔함)을 스스로 반증하고 올바른 방법(정점 변환)으로 재측정한 것도 모범 사례로 기록.
네거티브 컨트롤로 정확히 재현 확인, PlayMode 8건 실패는 A/B 대조로 다른 동시 진행 라운드 소관임을
증명(수정 무관).
**보류 항목 2건**: (a) 랙돌 중 모자 숨김은 유지 — 사용자 신고가 "회전"(던지기 회전)에 관한 것이라
랙돌 중 부위별 앵커 추적이 필요한 더 어려운 별건은 지금 범위 밖. 나중에 랙돌 중에도 모자가 사라지는
게 문제로 신고되면 그때 별도 설계. (b) `CharacterFxRenderer.cs`도 같은 배제 패턴이 있으나 FX는
근거가 달라 이번엔 안 건드림 — 후속 후보로 기록.

### 검증 결과 (전부 이 머신에서 실제 실행)

| 항목 | 결과 |
|---|---|
| macOS 빌드 | **Succeeded, 에러 0 / 경고 0** (`Builds/macOS/StickMate.app`, `Logs/coder_win4_macbuild.log:2868`) |
| Windows 크로스 빌드 | **Succeeded, 에러 0 / 경고 0** (`Builds/Windows/StickMate.exe`, `Logs/coder_win4_winbuild.log:2892`) — `#if UNITY_STANDALONE_WIN` 안의 새 코드가 실제로 컴파일됐다 |
| Windows 산출물 심볼 대조 | `TryGetReservedBottomBarOsScreen` / `IReservedBottomBarService` / `CaptureUiDensity` / `ReportUiDensityScale` / `GetDpiForWindow` **전부 있음** |
| EditMode | **147 / 147 통과** (`Logs/coder_win4_edit.xml`) |
| PlayMode 전체 | 292개 중 **283 통과 / 8 실패 / 1 스킵** (`Logs/coder_win4_play.xml`) — 아래 ★ 참고 |
| 신설 테스트 + 인접 영향권 집중 실행 | **50개 중 49 통과 / 0 실패 / 1 스킵**(`-nographics`에서 정상 스킵되는 Retina 초상화 테스트 1건). 대상: 신설 2종 + `InfoWindowExclusiveModalTests` / `CornerHoverPanelTests` / `RetinaDpiCoordinateTests` / `DockMetricsGeometryTests` / `PortraitTextureResolutionTests` / `DialogueComicTextPlacementTests` (`Logs/coder_win4_focus.xml`) |
| ProjectSettings 변경 | **0건**(`git status ProjectSettings/` 비어 있음) |

### ★ PlayMode 실패 8건은 내 변경과 무관하다 — **네거티브 컨트롤로 증명**

실패 목록: `CharacterScaleInvarianceTests`(2) / `CharacterScaleRuntimeTests`(2) /
`DockPhysicsStepTests.T3n` / `DockSinkholeRegressionTests.T3` / `PortraitFallenFramingTests` /
`WalkFootSlipTests`.

**증명(추정이 아니라 실행):** 내 소스 변경 3개 파일(`ScreenCoordinateConverter` /
`FallbackPlatformWindowService` / `CharacterInfoWindow`)을 `git checkout`으로 되돌리고 신설 파일
3개를 잠시 치운 뒤 그 8건만 다시 돌렸다 → **29개 중 21 통과 / 8 실패, 실패 목록이 한 건도 다르지
않다**(`Logs/coder_ctl_norevert.xml`). 복원 후 체크섬으로 원상 확인 완료.
독립 실행인 디버거의 `Logs/dbg_fix_play.xml`에서도 **같은 8건**이 실패한다.

**정황 원인(다른 에이전트의 진행 중 작업, 내 범위 밖):** `PrefabGeometryFollowsCharacterScale`가
"프리팹 전신 높이 1.7060인데 characterScale 0.350에서 기대값은 0.7961"이라고 말한다 — 즉
**프리팹이 현재 배율로 다시 구워지지 않은 상태**다. 배율/Dock/보행/초상화 실패가 전부 여기서
파생될 수 있다. 작업 트리에 `Stickman.prefab` / `SceneBootstrapper.cs` / `StickmanAgent.cs` /
`StickmanPoseAnimator.cs` / `DockGeometry.cs` / `DefaultStickConfig.asset`이 **다른 에이전트의
미커밋 변경**으로 올라와 있다. 리더가 통합할 때 이 8건을 그쪽 라운드 소관으로 배정해야 한다.

### 왜 이 macOS 환경에서 ①③의 **핵심 로직**은 실제로 검증됐는가 (한계와 함께)

`#if UNITY_STANDALONE_WIN` 안의 P/Invoke 자체는 여기서 실행할 수 없다. 그래서 **판단 로직을 그
바깥(플랫폼 중립 계층)에 두고** 스텁을 주입해 실제로 돌렸다:
- ①의 "어떤 사각형을 발판으로 삼을지 / 언제 추정으로 흘러가면 안 되는지"는 전부
  `FallbackPlatformWindowService`(플랫폼 중립)에 있고, `StubBottomBarService`로 6건 실행 검증.
  Windows 코드에 남은 것은 `rcMonitor.Bottom − rcWork.Bottom` 뺄셈 한 줄뿐이다.
- ③의 "밀도를 어떻게 캔버스 배율로 해석할지"도 전부 `ScreenCoordinateConverter`(플랫폼 중립)에 있고
  5건 실행 검증. Windows 코드에 남은 것은 `GetDpiForWindow(hwnd)/96` 한 줄뿐이다.
**검증되지 않은 것**: 그 두 줄이 실기에서 기대한 숫자를 돌려주는지. 둘 다 Player.log 한 줄
(`작업표시줄 실측 — rect=...` / `UI 밀도 갱신 — 캔버스 배율 ...`)로 즉시 확인된다.

### 남은 관찰 / 리더 판단 요망

1. **프레임레이트 상한**(위 ④ 실측 116fps). 걸려면 `vSyncCount=0` + `targetFrameRate`를 **함께**
   바꿔야 하고 120Hz Mac의 부드러움이 줄어드는 UX 변경이다. 측정 근거만 남기고 손대지 않았다.
2. **`CornerHoverPanel`의 Dock 회피가 실제로는 한 번도 동작한 적이 없다.** `:489`가
   `_agent.PlatformService as IDockMetricsService`로 캐스팅하는데, `PlatformService`는
   `FallbackPlatformWindowService`이고 그 데코레이터는 `IDockMetricsService`를 **통과시키지 않는다**
   → 항상 null. 이번 라운드에 고치지 않은 이유: 통과시키면 `ResolveDetectOriginPoints()`가
   `UpdateGeometry()`를 통해 **프레임당 2회** `TryGetDockMetrics()`를 부르게 되고, macOS 구현은
   CFPreferences + `NSWorkspace.runningApplications` 전수 조회라 **네이티브 호출 폭탄**이 된다.
   올바른 수정은 "데코레이터가 이미 캐시해 둔 Dock 사각형을 노출하고 패널이 그걸 읽는" 방향이다 —
   설계 판단이 필요해 리더에게 넘긴다.
3. **`Assets/` 루트에 `InitTestScene*.unity` 11개**가 테스트 실행 잔재로 쌓여 있다(임포트 대상이라
   AssetDatabase/Library를 계속 불린다). 정리 대상이지만 지금 다른 에이전트가 테스트를 돌리는 중이라
   건드리지 않았다.
4. 08-30 라운드가 이월한 **"Windows 단축키 안내 문구가 Mac 표기"**는 이번에도 손대지 않았다
   (`AppControlDirector`의 "Ctrl+Opt+Cmd+…" 하드코딩). 실제 Windows에서 확인 가능해진 지금이
   고칠 때다 — 리더 판단.

### 2026-08-31 — 리더 승인: 윈도우 실사용 버그 4건 조사·수정 결과
**①작업표시줄 침범 — 완전 해결 승인**: Win32WindowService에 작업표시줄 조회 자체가 없어 macOS Dock
모양(가운데 65%)으로 잘못 폴백하던 것 확인. `IReservedBottomBarService` 신설(GetMonitorInfo의
rcMonitor-rcWork 단일 조회로 좌우/상단배치/자동숨김/배율/멀티모니터 전부 커버, macOS는 미구현이라
영향 0) 승인 — 아키텍처 깔끔함.
**③저해상도 — 완전 해결 승인**: 좌표용 배율(AutoDpiScale)과 UI 밀도용 배율을 분리 안 해서, 그 비율에
디스플레이 배율이 실리는 게 macOS만의 우연이었다는 걸 실측 확인. `AutoUiDensityScale` 신설(Windows는
GetDpiForWindow/96 직접 보고, 미보고 환경은 항등식이라 macOS/에디터/모바일 무변화) 승인.
**②창 중복 — 미해결 정직 보고 + 부수 발견 수정 승인**: 여러 가설(배타모달 미포함/캔버스 플랫폼분기/
DPI 클릭어긋남/창복제경로) 전부 실측으로 반증, 원인 미확정을 정직하게 보고한 것 승인. BitBlt
레이어드창 잔상(고스팅) 가설 + 사용자가 직접 구분할 진단법(창을 드래그해서 궤적 따라 번지면 고스팅,
제자리 남으면 진짜 중복) 제공도 실용적. 부수 발견(CloseOverlappingSurfaces가 구석 호버 패널을
빠뜨렸고, 다이얼 드래그 중엔 자가치유도 꺼져 있던 진짜 구멍) 수정 승인.
**④렉 — 배정 가설(호버 패널 카메라) 이중 반증, 원인 미확정, 미수정 승인**: 실측으로 명확히 반증
(사용자가 돌린 exe엔 그 코드 자체가 없음 + 소스상으로도 접히면 카메라 꺼짐). 프레임당 할당도 무시할
수준(~41B). `Application.targetFrameRate` 미설정으로 116fps 그대로 도는 것 + Windows 투명창이
강제하는 레거시 BitBlt 스왑체인이 유력 후보이나 **확정 안 된 상관관계일 뿐**이라 안 고친 판단 승인
(targetFrameRate 변경은 vSyncCount도 같이 바꿔야 하고 120Hz 맥 UX까지 건드리는데 원인 미확정 상태로
손대는 건 과함).
**보류(리더 판단 필요, 다음 라운드)**: `CornerHoverPanel:489`의 Dock 회피가 데코레이터 통과 누락으로
한 번도 동작한 적 없었음 발견. 단순 통과시키면 프레임당 네이티브 호출 폭탄 — 이 코드베이스의 기존
관례(`footholdPollInterval`/`fullscreenPollInterval`처럼 주기적 폴링+캐시)를 따라 고칠 것을 다음
라운드 방향으로 기록.
검증: macOS/Windows 크로스빌드 둘 다 0/0, EditMode 147/147, PlayMode 8실패는 네거티브 컨트롤로
이 라운드와 무관함(다른 라운드의 진행 중 프리팹 재베이크 소관) 증명 완료.

---

## 2026-08-31 — 6라운드 통합 **클린 전체 검증 + 최종 리뷰** **[Test Engineer]** — `(개선 R3)`

53개 변경 파일(6개 라운드 통합) 대상. **커밋 불가 — 아키텍트 반려.**

### 실행 숫자 (전부 이 머신에서 실제 실행, 표준 `-batchmode -nographics`)

| 항목 | 결과 | 로그 |
|---|---|---|
| EditMode | **147 / 147 통과** | `Logs/te_r6_edit.xml` |
| PlayMode 전체 | 292개 중 **282 통과 / 9 실패 / 1 스킵** | `Logs/te_r6_play.xml` |
| macOS 빌드 | **Succeeded, 에러 0 / 경고 0** (103,291,848 bytes) | `Logs/te_r6_macbuild.log:2833` |
| Windows 크로스 빌드 | **Succeeded, 에러 0 / 경고 0** (88,042,477 bytes) | `Logs/te_r6_winbuild.log:2851` |
| `The referenced script ... is missing!` | **0건** (EditMode/PlayMode 양쪽) | — |
| `NullReferenceException` | **0건** (양쪽) | — |
| 에디터 크래시 | **0건** (로그의 "crash" 38건은 전부 `WindowCrashRenderer` 파일명 오탐) | — |
| 배포 에셋 오염(디스크) | `DefaultStickConfig.asset` 디스크 값은 여전히 `characterScale: 0.75` — 배치 실행은 디스크를 더럽히지 않았다 | `git diff` |

### 교차 영향 집중 확인 — 전부 통과

| 스위트 | 결과 |
|---|---|
| `CornerHoverPanelTests` (다이얼 M1/M2 무회귀) | **6 / 6** |
| `GetupFloorClearanceTests` | **2 / 2** |
| `PetFollowsOwnerFootholdTests` | **4 / 4** |
| `ReservedBottomBarFootholdTests` (윈도우 작업표시줄) | **6 / 6** |
| `UiDensityCanvasScaleTests` (AutoUiDensityScale 신설) | **5 / 5** |
| `RetinaDpiCoordinateTests` (macOS 기존 좌표 무회귀) | **9 / 9** |
| `InfoWindowExclusiveModalTests` (구석 패널 배타모달 편입) | **5 / 5** |
| `DockMetricsGeometryTests` | **10 / 10** |
| `EdgeHopDownTests` (맨틀 인셋 네거티브 컨트롤 포함) | **7 / 7** |
| `CharacterAccessoryScaleTests` | **15 / 15** |
| `CharacterPortraitStageTests` / `PortraitDragIndependenceTests` (`PrimaryStage()` 재설계) | **7 / 7** / **2 / 2** |
| `FloorContactVisibilityTests` (플레이키 이력) | **1 / 1** |

`CharacterAccessoryRenderer.cs` diff 직접 통독 — 세 라운드(① 배율 상쇄 `SyncContainerScale`/`RootScale`/
`ResolveBodyOffsetY` 단위정정, ② `ICharacterInkExtentProvider.TryGetLowestInkWorldY`, ③ ThrowTumble을
숨김목록에서 제거)가 **서로 다른 지점에 있어 충돌 없이 전부 살아 있다.** 유실된 hunk 0건.
`AccessoryShapeBuilder.cs`는 **오늘 한 줄도 바뀌지 않았다**(마지막 커밋 `b6755f4`) — 충돌 가능성 자체가 없다.
`ScreenCoordinateConverter`의 `ResolveCanvasScaleFactor` 신규 우선순위(수동 → `AutoUiDensityScale` →
`1/AutoDpiScale`)는 기존 `ResolveDpiScale`의 폴백 사슬을 그대로 감싼 것이라 **macOS에서 항등식**임을 코드로 확인.

---

## ★★ Blocker 1 — PlayMode 실패의 진짜 원인은 "프리팹 재베이크"가 **아니다**. 직전 라운드 진단은 반전돼 있었다.

직전 라운드가 "프리팹 전신 높이 1.706 vs 기대 0.796 → 프리팹이 재베이크 안 됨"이라고 배정했다.
**이 진단은 틀렸다. 프리팹은 정확히 옳게 구워져 있다.**

- `1.7060208 = BaselineCharacterTotalHeight(2.2746944) × 0.75`, 그리고 `0.75`는 디스크의 배포 에셋 값 그대로다.
- 어긋난 쪽은 **`characterScale`이 런타임에 0.35로 바뀌어 있는 것**이다(테스트 메시지의 `characterScale(0.350)`).

### 실행으로 증명 (임시 하네스 `ZZSaveScalePollutionProbe`, 확인 후 삭제 완료)

```
[POLLUTION-PROBE] (1) 씬 로드 직후 — 배포 에셋 characterScale=0.3500, 구워진 배율=0.7500,
    캡슐 size.y=1.7060, 저장파일=~/Library/Application Support/DefaultCompany/StickMate/
    stickmate_character.json, 불러옴=True, UiLayoutModel.HasCharacterScale=True,
    UiLayoutModel.CharacterScale=0.3500.
[POLLUTION-PROBE] (2) 프리팹은 구워진 배율 0.750에 정확히 일치한다 (기대 1.7060 == 실측 1.7060)
    → '재베이크 안 됨' 가설 반증.
[POLLUTION-PROBE] (3) ApplyCharacterScale(0.750) 반환=True, 되돌린 뒤 characterScale=0.7500,
    캡슐=1.7060, metrics.TotalHeight=1.7060.
[POLLUTION-PROBE] (4) 되돌리자 항등식 성립 — 실패 원인은 저장 파일의 characterScale이다.
```

### 인과 사슬 (전 구간 관측됨)

1. 개발자 머신의 **실제 저장 파일**에 `"characterScaleSaved": true, "characterScale": 0.35`가 들어 있다
   (사용자/하네스가 다이얼을 최소값까지 내린 흔적).
2. `CharacterProgressionDirector` → `CharacterSaveStore.Load()` → `UiLayoutModel.RestoreCornerPanelFromSave(true, 0.35, …)`.
3. `CornerHoverPanel.RestoreSavedScale()` → `_agent.ApplyCharacterScale(0.35, "저장된 크기 복원")`.
4. `StickmanAgent.ApplyCharacterScale()`의 2단계가 `_config.characterScale = v` — **`_config`는 배포 에셋
   `DefaultStickConfig.asset` 그 자체다.** 씬을 다시 로드해도 ScriptableObject는 배치 세션 내내 살아 있다.
5. `Logs/te_r6_play.log`에 `[크기] 캐릭터 배율 0.35× 적용(저장된 크기 복원) — 구워진 배율 0.750`이
   **한 스위트에 146번** 찍힌다. 즉 오늘 하루 종일 모든 에이전트의 PlayMode가 **0.35× 캐릭터**로 돌았다.

### 이것이 9건을 어떻게 설명하는가 (전부 같은 뿌리)

| 실패 | 0.35 오염으로 설명되는가 |
|---|---|
| `PrefabGeometryFollowsCharacterScale` | 배포 에셋 0.35 vs 프리팹 0.75 — 정확히 이 불일치 |
| `DockHopDownBandSurvivesScale` (0.6531 → 0.3048) | 임계 배율이 캐릭터 치수에서 유도됨 |
| `CharacterScaleRuntimeTests.배율_전_구간…` "0.35 적용이 무시됐습니다" | `ApplyCharacterScale`의 무동작 가드(`sameTransform && sameConfig`) — **이미 0.35이므로 false**. 오염의 가장 직접적인 지문 |
| `…획이 최소 두께 아래로` (0.0168) | 위 무동작 때문에 두께 재대입이 아예 안 돌았다 |
| `WalkFootSlip` / `DockSinkhole T3` / `DockPhysicsStep T3n` / `PortraitFallenFraming` | 전부 캐릭터 치수 의존 — 0.35에서는 보폭/독 사각지대/액자 프레이밍이 전혀 다른 체제 |

**네거티브 컨트롤**: 프로브 (2)가 "프리팹은 구워진 배율과 정확히 일치"를 단언하므로, 만약 진짜로 재베이크가
필요한 상황이었다면 그 단언이 먼저 실패했을 것이다. 실패하지 않았다 = 재베이크 가설 반증.
**따라서 `StickMate/Resize Stickman` 재베이크를 실행하지 않았다** — 실행했다면 멀쩡한 프리팹의 fileID를
건드려 오늘 오전의 교훈(씬 오버라이드 유실)을 반복했을 것이다.

---

## ★★ Blocker 2 — `ApplyCharacterScale`이 **배포 ScriptableObject를 직접 쓴다** (Blocker 1의 진짜 원인)

`Core/StickmanAgent.cs:695` — `if (_config != null) _config.characterScale = v;`

`_config`는 프리팹에 배선된 `DefaultStickConfig.asset`이다. 두 가지가 동시에 깨진다:

1. **배포 에셋 오염 위험(높음).** 유니티 에디터는 **플레이 모드 중 ScriptableObject 변경을 되돌리지
   않는다**(MonoBehaviour와 달리). 누군가 에디터에서 앱을 켜고 다이얼을 만진 뒤 프로젝트를 저장하면
   `characterScale: 0.35`가 **레포에 커밋되어 전 사용자에게 배포**된다. 오늘은 배치 실행이라 디스크가
   무사했을 뿐이다(`git diff`로 확인).
2. **테스트 비-헤르메틱.** PlayMode 스위트가 **개발자 개인의 저장 파일**을 읽는다. 즉 "앱을 한 번이라도
   실제로 가지고 논 사람"과 "그렇지 않은 사람"의 테스트 결과가 다르다. 오늘 네 명의 에이전트가
   같은 8건을 보고도 원인을 프리팹으로 오인한 이유가 정확히 이것이다 — 모두 같은 오염된 파일을 읽었으므로
   "내 변경을 되돌려도 그대로다"라는 네거티브 컨트롤이 **참이지만 무의미**했다.

`SizeDialWidget` 자신의 문서가 "두 곳에서 같은 값을 만지면 진실이 둘이 된다"고 적어 놓은 바로 그 상황이
`characterScale` 필드 하나에서 벌어지고 있다 — 그 필드는 지금 **배포 기본값이자 동시에 런타임 현재값**이다.

**수정 방향은 설계 판단이라 아키텍트에게 넘긴다.** 후보 두 가지:
- (a) 에이전트가 Awake에서 config를 **세션 복제본**으로 갈아끼우고 런타임 변경은 복제본에만 쓴다
  (`CharacterScaleRuntimeTests`가 이미 쓰는 패턴). 단, `agent.Config == 배포 에셋`을 전제하는 소비자가
  있는지 전수 확인 필요.
- (b) 런타임 배율을 `StickConfig`에서 떼어내 에이전트/블랙보드의 런타임 필드로 옮기고,
  `ResolveWalkSpeed()` 등 소비자가 그쪽을 읽게 한다.
**추가로 필요**: PlayMode 테스트가 실제 저장 파일을 읽지 않게 하는 스위치
(`CharacterSaveStore.SuppressLoadForTesting` 같은 것 — 이 프로젝트의 `ResetForTesting()` 관례와 동형).
이게 없으면 같은 사고가 반드시 재발한다.

**리더 즉시 조치(임시)**: 저장 파일의 `characterScaleSaved`를 false로 만들면 9건 중 8건이 즉시 사라진다.
다만 이건 증상 제거일 뿐이고, 사용자가 다이얼을 다시 만지는 순간 그대로 재발한다.
(저장 파일은 사용자 자산이라 테스트 엔지니어가 건드리지 않았다 — 원칙 3.)

---

## Major 1 — `AccessoryFacingFlipFillTests.던져서_공중회전하는_동안_모자_가시성을_기록한다` 플레이키

세 번의 독립 실행에서 같은 지표가 **22배** 흔들린다(허용치 `headRadius × 0.25 = 0.01925`):

| 실행 | maxDriftWorldUnits | 머리 반경 대비 | 결과 |
|---|---|---|---|
| `Logs/coder_win4_play.xml` | 0.00103 | 1.3 % | 통과 |
| `Logs/dbg_fix_play.xml` | 0.00853 | 11.1 % | 통과 |
| `Logs/te_r6_play.xml` (이번) | **0.02333** | **30.3 %** | **실패** |

**중요 — 사용자가 신고한 증상 자체는 세 번 다 완전히 고쳐져 있다**:
`framesWithHatHiddenWhileTumbling=0 (0%)`가 세 실행 모두 동일. 실패한 것은
"드리프트가 너무 크면 다시 숨겨야 한다"는 **부차적 안전 가드**뿐이다. 즉 모자 수정 자체는 회귀 아님.

지표가 `max`(전 7,500프레임 중 최댓값)라 물리 타이밍의 꼬리에 그대로 노출돼 있고, 여유가
직전 관측 최댓값의 2.3배밖에 안 된다. **가설(디버거와 나눌 것)**: 드리프트에 배율-불변인 절대
성분(프레임 지연)이 섞여 있다면, 0.35× 오염 상태에서는 분모(`headRadius = 0.077`)만 작아져 비율이
2.14배 부풀려진다 — 정상 0.75×에서는 30.3 % → 약 14 %로 내려가 통과한다.
**반증 방법**: Blocker 1/2를 고친 뒤 이 테스트를 5회 반복 실행. 비율이 15 % 아래로 안정되면 가설 확정,
여전히 30 %대로 튀면 지표 정의(max → p99 또는 창 평균)를 다시 설계해야 한다. **디버거에게 이관.**

## Major 2 — 리더의 M1 수정에는 회귀 테스트가 없다 (M2에는 있다)

`SizeDialWidget.IsInRing()`의 각도 게이트(빈 96° 구역 탭 → 끝값 순간이동 방지)는 **어떤 테스트도 부르지
않는다**(`grep -rn IsInRing Assets/` = 구현 1곳뿐). 같은 라운드의 M2는 잠금 테스트
`카드_하단과_다이얼_원환_상단이_정확히_맞닿는다`를 제대로 받았는데 M1만 빠졌다.
이 버그는 **이미 한 번 출시된 클래스**이고(사용자가 아무 데나 클릭하면 크기가 0.35/2.00으로 점프),
`IsInRing`은 순수 함수라 PlayMode도 필요 없는 EditMode 3줄짜리 테스트로 잠글 수 있다.
최소 케이스: (a) θ=0°(6시) 반지름 50pt → true, (b) θ=180°(12시 빈 구역) 반지름 50pt → **false**,
(c) 반지름 10pt/100pt → false.

## Minor 1 — `mono_crash.a388a3d94.0.json`이 커밋 대상에 올라 있다
레포 루트에 08:04 생성된 32KB Mono 크래시 덤프가 **추적되지 않은 채 `.gitignore`에도 없다** —
지금 커밋하면 그대로 들어간다. `.gitignore`에 `mono_crash.*.json` 추가 + 파일 삭제 권장.
(내 검증 실행 3회에서는 크래시가 재발하지 않았다 — 오전 다른 세션의 잔재로 보인다.)

## Minor 2 — `CornerHoverPanel:521`의 Dock 회피가 죽어 있다는 코더 보고 — **확인함**
`FallbackPlatformWindowService`의 구현 인터페이스 목록(`:48`)에 `IDockMetricsService`가 없다.
따라서 `_agent.PlatformService as IDockMetricsService`는 **항상 null**이고 Dock 회피는 한 번도 실행된 적이
없다. 코더 보고가 정확하다. 리더가 이미 다음 라운드로 배정했으므로 이번 반려 사유에는 넣지 않는다.

## Minor 3 — `Assets/` 루트에 `InitTestScene*.unity` 13개
`.gitignore:38`이 덮으므로 **커밋 위험은 없다.** 다만 AssetDatabase 임포트 대상이라 매 배치 실행을
느리게 만든다. 정리 권장(직전 라운드가 11개라고 보고한 것이 13개로 늘었다).
...
### 좋은 점
1. **`IReservedBottomBarService`의 계약 설계가 모범적이다.** "실패(false)는 '추정해 보라'가 아니라
   '하단 예약 막대가 존재하지 않는다'는 확정 신호"라고 **실패의 의미를 계약에 못박은** 것이 핵심이다.
   이 프로젝트가 반복해 온 사고(폴백이 조용히 잘못된 추정으로 흘러가는 것)를 타입 수준에서 막는다.
   macOS `IDockMetricsService`와 정반대 사정을 나란히 대비시켜 설명한 문서도 다음 사람에게 그대로 유효하다.
2. **`ICharacterInkExtentProvider`가 의존 방향을 뒤집었다.** "소비자(GetupState)가 부품 목록을 갖는" 대신
   "잉크를 더하는 쪽이 스스로 신고"하게 해서, DLC로 부품이 늘어나도 소비자가 낡지 않는다 — CLAUDE.md 원칙 4
   (플러그인 구조)의 교과서적 적용. 27줄 인터페이스가 32종+DLC 확장을 커버한다.
3. **액세서리 이중 스케일을 (b) 컨테이너 역스케일로 푼 판단.** (a) 전 치수 나누기를 택했다면
   "빠뜨린 지점이 배율 1.0에서는 여전히 정상으로 보이는" 최악의 실패 유형에 노출됐다.
   `AccessoryShapeBuilder` 32종을 한 줄도 안 고친 것이 그 판단의 증거다.
4. **`AutoDpiScale` / `AutoUiDensityScale` 분리에서 이름이 개념을 정확히 실어 나른다.**
   "Windows에서 1.0인 것이 **맞다**"라고 적어 둔 한 줄이, 다음 사람이 이 값을 '고치려는' 시도를 막는다.
   미보고 환경에서 예전 정의로 정확히 되돌아가 macOS 항등성이 코드로 읽힌다.
5. **`SizeDialWidget`을 MonoBehaviour로 만들지 않은 것**과 그 근거(프리팹 배선 지점이 늘면
   33-9 #10 Blocker가 재발)를 파일 상단에 남긴 것. 설계 이탈을 숨기지 않고 근거와 함께 문서화하는 습관.
6. **`CloseOverlappingSurfaces`에 구석 패널을 부채꼴 early-return **위**에 넣은 것.**
   "부채꼴이 있든 없든 반드시 실행되어야 한다"는 이유까지 주석에 있어, 다음 사람이 순서를 바꾸지 않는다.
7. `EdgeHopDownTests`의 **"네거티브 컨트롤의 네거티브 컨트롤"**(`parkourMantleInsetDerived`) —
   유도값 전환이 기존 컨트롤을 조용히 죽이는 것을 스스로 감지하게 만든 설계.

### 개선할 부분
1. **(Blocker 2)** `ApplyCharacterScale`이 배포 ScriptableObject를 쓴다 — 위 절 참고. 설계 판단 필요.
2. **(Blocker 2 동반)** PlayMode 테스트가 개발자 실제 저장 파일에 의존한다. `CharacterSaveStore`에
   테스트용 억제 스위치가 필요하다.
3. **(Major 2)** `SizeDialWidget.IsInRing()` 회귀 테스트 부재.
4. **(Major 1)** `AccessoryFacingFlipFillTests`의 드리프트 지표가 `max` 기반이라 여유가 부족하다.
5. **(Minor)** `CornerHoverPanel`이 **1,031줄**이다 — 상태 머신 / 다이얼 배선 / 미리보기 카드 /
   Dock 회피 / 비침해 억제가 한 파일에 있다. 지금은 문서가 좋아 읽히지만, 34-9 #13의 모바일 스와이프
   진입이 들어오면 임계를 넘는다. 상태 머신(`TickHoverStateMachine` 계열)을 분리할 자리가 자연스럽다.
6. **(Minor)** `CharacterAccessoryRenderer.TryGetLowestInkWorldY`의 `if (!any) worldY = 0f;`는
   `any == false`면 호출부가 `worldY`를 보지 않는다는 계약이라 도달해도 무해하지만, `float.PositiveInfinity`
   초기값과 0f 두 개의 "의미 없음" 표현이 공존한다. 하나로 줄이는 편이 읽기 쉽다.
7. **(Minor)** `mono_crash.*.json` `.gitignore` 누락 / `InitTestScene*` 13개 잔재.

### 판정
**개선할 부분이 7건(Blocker 2 / Major 2 / Minor 3) 있으므로 아키텍트에게 반려한다.**
다음 라운드 출력 헤더는 `(개선 R4)`로 표기할 것.
**커밋 승인 불가** — 특히 Blocker 2는 "배포 에셋에 0.35가 커밋되어 전 사용자에게 나가는" 경로가 열려 있어,
지금 커밋하기 전에 최소한 (a) 저장 파일 오염 제거로 스위트를 초록으로 만들고 (b) 배포 에셋 쓰기 경로를
막는 두 가지는 처리되어야 한다.


### 2026-08-31 — 리더 직접 조치 3건 + Blocker 2 최우선 배정
1. `.gitignore`에 `mono_crash.*.json` 패턴 추가 + 기존 잔존 파일 삭제(Minor 항목).
2. `SizeDialWidgetHitTestTests.cs` 신설(EditMode 3건) — 리더의 M1 수정(`IsInRing` 각도 게이트)에
   빠져있던 회귀 테스트 편입. 직접 실행 검증: 3/3 통과.
3. **Blocker 2(캐릭터 배율이 배포용 공유 ScriptableObject 애셋을 직접 오염시키는 구조적 결함)를
   최우선으로 코더에게 배정.** `StickmanAgent.ApplyCharacterScale()`이 `_config.characterScale = v`로
   프리팹에 배선된 `DefaultStickConfig.asset` 원본을 직접 수정 — 배치모드라 오늘은 디스크가 무사했지만,
   실제 에디터에서 다이얼을 조작하고 저장하면 잘못된 기본값이 전 사용자 배포판에 박히는 심각한 위험.
   런타임 복제본(Instantiate) 사용을 권고안으로 제시했고 최종 판단은 코더에게 위임. 동반해서
   Major 1(모자 회전 테스트 플레이키 — 배율 오염이 원인일 가능성) 재검증도 요청.
   **오늘 최종 관문 — 이것만 끝나면 통합 검증 재확인 후 즉시 커밋+푸시.**

---

## 2026-08-31 — R3 **Blocker 2 근본 수정** + 테스트 격리 + Major 1 재검증 **[Coder]** — `(개선 R4)`

### 판단: (a) 런타임 복제본을 **채택하지 않았다** — 실측으로 갈라짐이 확인돼서다

리더 권고는 (a) `_config = Instantiate(baseConfig)`였다. 코드를 먼저 읽고 **기각**했다. 근거는 추정이 아니라
프리팹 YAML 전수 스캔이다:

```
Stickman.prefab에서 DefaultStickConfig.asset(guid a80c0e…)을 참조하는 컴포넌트 = 16개
  StickmanAgent / RodeoCursorWatcher / BattleMinigameDirector / GraffitiDirector / WindowTheftDirector /
  WindowCrashDirector / HardwareReactionDirector / StressGaugeDirector / RunawayDirector /
  TodoReminderDirector / TodoPostItWidget / FocusWatchDirector / ArcheryDirector /
  CharacterProgressionDirector / CharacterInfoWindow / DialogueBubbleRenderer
```

에이전트만 복제본으로 갈아타면 **나머지 15개가 낡은 원본을 계속 읽는다**. 그 상태에서 실제로 갈라지는
필드가 있는지 전수 확인했다 — 런타임에 값이 **바뀌는** 필드는 5개뿐이다:

| 필드 | 쓰는 곳 | 복제본을 쓰면? |
|---|---|---|
| `characterScale` | `StickmanAgent:695` | 읽는 쪽이 전부 `agent.Config`/`blackboard.Config` 경유라 **갈라지지 않음** |
| `desktopDpiScale` | `StickmanAgent:912` | `ScreenCoordinateConverter`에 **배선된 15개가 각자 자기 `_config`를 넘긴다** → 갈라짐 |
| `inkColor` | `CharacterInfoWindow:1045`, `AppControlDirector:400` | 읽는 쪽이 `_config`/`agent.Config`로 섞여 있음 → 갈라짐 |
| `rodeoCursorEnabled` | `AppControlDirector:410` | 쓰는 쪽은 `agent.Config`, 읽는 쪽 `RodeoCursorWatcher`는 **배선된 원본** → 갈라짐 |
| `verboseDiagnosticsLogging` | `AppControlDirector:424` | 위와 같음 → 갈라짐 |

즉 (a)는 Blocker 2를 고치면서 **잉크색 토글·로데오 토글·Retina DPI 보정을 조용히 망가뜨린다**(그중 DPI는
macOS 실빌드 좌표계 전체가 걸린 값이다). 게다가 어느 것도 예외를 던지지 않는 부류라 테스트로도 안 잡힌다.
"진실이 둘이 되는 것"을 막으려다 정확히 그것을 만드는 수정이 된다.

### 채택: (b)의 변형 — `characterScale`을 **에셋 안에서** 직렬화 값과 런타임 값으로 쪼갰다

리더가 말한 (b)("배율을 StickConfig에서 떼어낸다")의 목적은 그대로 달성하면서, (b)의 단점(소비자 다수 수정)을
없앤 형태다. **소비자는 한 줄도 안 바뀌었다.**

`Core/StickConfig.cs`
```csharp
public float characterScale = 0.75f;                       // 배포 기본값(직렬화 — 런타임에 절대 안 쓴다)
[System.NonSerialized] private float _runtimeCharacterScale; // 이번 실행의 값(직렬화 대상이 아님)

public bool HasRuntimeCharacterScale => _runtimeCharacterScale > 0f;
public void SetRuntimeCharacterScale(float v)  // clamp 포함
public void ClearRuntimeCharacterScale()

public float ResolveCharacterScale()
    => 런타임 값이 있으면 그것, 없으면 배포 기본값 (그 뒤 기존 clamp/NaN 방어 그대로)
```

`Core/StickmanAgent.cs`
- `ApplyCharacterScale` 2단계: `_config.characterScale = v` → **`_config.SetRuntimeCharacterScale(v)`**
- 무동작 가드: `Mathf.Approximately(_config.characterScale, v)` → **`ResolveCharacterScale()`** 비교
  (이걸 같이 안 고치면 "이미 런타임 0.35인데 직렬화는 0.75"라서 가드가 영원히 안 걸린다)
- `Awake()` 최상단에 **`ClearRuntimeCharacterScale()`** — StickConfig는 에셋이라 **씬 재로드에도 같은
  인스턴스가 살아남는다**(아래 프로브 (3)에서 `같은 인스턴스=True`로 실측). 세션마다 배포 기본값에서
  출발하지 않으면 앞 씬의 배율이 다음 씬으로 샌다 = 이번 사고의 전파 경로 그 자체.

**왜 이게 구조적 보장인가**: `[NonSerialized]` 필드는 직렬화 대상이 아니므로 Ctrl+S로도 `AssetDatabase`로도
`.asset` 파일에 닿을 수 없다. "조심해서 쓰기"가 아니라 **쓸 수 있는 경로가 타입 수준에서 없다.**
실측 증거: 이번 라운드 뒤에도 `DefaultStickConfig.asset`의 diff에 **새 키가 하나도 안 생겼다**(아래).

### 동반 조치 — 테스트가 개발자의 실제 저장 파일을 읽지 않는다

`Core/CharacterSaveStore.cs` — 경로의 **단일 조회 지점**(`FilePath`)에 테스트 전용 리디렉션을 넣었다.
`RedirectToTemporaryDirectoryForTesting(label)` / `ResetForTesting()` / `IsRedirectedForTesting`
(이 프로젝트의 `EquipmentModel.ResetForTesting()` 관례와 동형). `Save()`와 다운그레이드 백업 경로도
같은 `SaveDirectory`를 타게 맞췄다(한 군데만 옛 경로로 남으면 그게 다음 사고다).

**억제가 아니라 경로 재지정을 택한 이유**: 로드를 막으면 실제 디스크 왕복을 검증하는 지속성 테스트
6종(`TodoPersistence`/`EquipmentMigration`/`SaveDowngradeGuard`/`CharacterStatsPersistence`/
`UiLayoutPersistence`/`CharacterProgressionPersistence`)이 통째로 죽는다. 경로만 옮기면 그것들은
**한 글자도 안 고친 채** 임시 폴더에서 그대로 돌고, 프리팹이 자동으로 부르는 `Load()`는 빈 폴더를 만나
"새 캐릭터"로 출발한다. 덤으로, 그 테스트들이 지금까지 의존하던 "SetUp에서 개발자 파일 백업 → TearDown에서
복구" 관례가 **중간에 죽으면 깨지는** 문제도 함께 사라진다(관례가 아니라 구조로 보장).

켜는 곳: `Tests/PlayMode/GlobalPlayModeTestIsolation.cs` / `Tests/EditMode/GlobalEditModeTestIsolation.cs`
(네임스페이스 없는 `[SetUpFixture]` — 어셈블리의 모든 테스트보다 먼저 1회). 테스트 100여 개에 같은 줄을
붙이는 것보다 빠뜨릴 여지가 없다. **실제로 실행됐는지는 가정하지 않고 로그로 확인했다**(아래).

**개발자의 실제 저장 파일은 읽지도 쓰지도 지우지도 않았다**(원칙 3). 실행 후 mtime `10:38`(내 실행은
11:07~11:19), 내용도 `characterScale: 0.35` 그대로다.

### 신규 회귀 잠금 — `Tests/PlayMode/DeployedConfigAssetImmutabilityTests.cs` (4건, 전부 통과)

1. `배율_조작이_배포_에셋의_직렬화_필드를_건드리지_않는다` — 배율 2.00 적용 후 **허용오차 0**으로
   `characterScale == 0.7500`. 동시에 실효 배율/신장/보행속도가 **정확히 따라오는지**도 같은 호흡에 단언한다
   (아무 일도 안 일어나서 통과하는 것을 배제).
2. **네거티브 컨트롤** `NegativeControl_직렬화_필드에_직접_쓰면_이_검사가_실제로_잡아낸다` — **완전히 같은
   검사 방법**으로 옛 코드(`config.characterScale = 0.35`)를 잡아내는 것을 보인다. 그 실험은 배포 에셋이
   아니라 **복제본**에 대고 한다(검사 방법의 민감도만 증명하면 되고, 배포 에셋은 테스트 안에서도 안 건드린다).
3. `씬을_다시_로드하면_배포_기본_배율로_돌아온다` — 누수 경로 잠금.
4. `테스트는_개발자의_실제_저장_파일을_읽지_않는다` — `[SetUpFixture]`가 조용히 안 도는 날 **즉시 빨간불**이
   나게 한다(격리가 꺼진 줄 모르고 또 하루를 태우지 않도록).

### 실행 숫자 (전부 이 머신 실측, 표준 `-batchmode -nographics`)

| 항목 | 이번 | 직전(R3) | 로그 |
|---|---|---|---|
| 컴파일 | **error 0 / warning 0** | — | `Logs/coder_b2_edit.log` |
| EditMode | **150 / 150 통과** | 147/147 | `Logs/coder_b2_edit.xml` |
| PlayMode | **296개 중 295 통과 / 0 실패 / 1 스킵** | 282 통과 / **9 실패** | `Logs/coder_b2_play.xml` |
| `[크기] … 0.35× 적용` 로그 | **4회**(전부 `(테스트)` = 배율 스윕이 일부러 부른 것) | **146회**(`저장된 크기 복원`) | `Logs/coder_b2_play.log` |
| `저장된 크기 복원` | **0회** | 146회 | 〃 |
| 실효 배율 | `metrics.Scale=0.7500`, `TotalHeight=1.7060` | 0.35 | 〃 |
| 저장 파일 | `…/T/DefaultCompany/StickMate/StickMateTestSaves/playmode/…` → **없음, 새 캐릭터로 시작** | 개발자 개인 파일 **불러옴** | 〃 |

**R3가 보고한 9건이 전부 사라졌다** — 프리팹은 한 글자도 안 고쳤다(R3의 "재베이크 불필요" 판정이 옳았다).

### ★ 가장 중요한 검증 — 수정 전/후 대조를 **같은 세션 실행으로** 증명

`git diff`만으로는 부족하다(배치모드는 애초에 `SaveAssets`를 안 부르므로 **수정 전에도** 디스크는 깨끗하다 —
즉 디스크 diff는 이 버그에 대해 **감도가 0인 검사**다). 그래서 임시 하네스 `ZZPollutionProbe`로
**메모리상의 오염 자체**를 직접 관측했다(확인 후 삭제 완료):

```
[ZZPROBE] (1) 씬 로드 직후 — 직렬화 characterScale=0.7500, 실효=0.7500, 런타임설정=False
[ZZPROBE] (2) 옛 방식 대입 직후 — 직렬화=0.3500, 실효=0.3500          ← 수정 전 코드가 하던 일
[ZZPROBE] (3) 씬 재로드 후 — 같은 인스턴스=True, 직렬화=0.3500, 실효=0.3500
[ZZPROBE] (4) 복구 — 직렬화=0.7500, 실효=0.7500
```

(2)(3)이 Blocker 2의 인과를 **끝까지** 보인다: 옛 경로는 배포 에셋 인스턴스 자체를 바꾸고, 그 오염은
**씬 재로드에도 살아남는다**(`같은 인스턴스=True`). 신 경로는 (3)의 자리에서 `HasRuntimeCharacterScale=False`로
되돌아온다(`씬을_다시_로드하면_…` 테스트가 그것을 잠근다).

디스크 대조도 함께 남긴다 — 작업 전/후 `md5 = 0e8333d71ca6fdd17d01f075b51a91da`로 **동일**,
`git diff`는 앞선 라운드의 정당한 2줄(`getupFloorClearanceEnabled`, `parkourMantleInsetDerived`)뿐이고
`characterScale` 줄도 신규 키도 없다(= `[NonSerialized]`가 실제로 직렬화되지 않았다는 실측 증거).

### Major 1 재검증 — 리더 가설 **확정**, 5회 전부 통과

| 실행 | maxDrift | 머리 반경 대비 | 허용치 |
|---|---|---|---|
| R3(0.35× 오염) | 0.02333 | **30.3 %** | 실패 |
| 이번 1~5회 | 0.00120 / 0.00119 / 0.00317 / 0.00118 / 0.00101 | **0.6 ~ 1.9 %** | 전부 통과 |

허용치는 `headRadius × 0.25`인데 headRadius가 0.077(0.35×) → **0.16500**(0.75×)로 돌아왔다. 다만
**분모 효과만이 아니다** — 절대 드리프트 자체가 0.02333 → 0.0032로 줄었다(0.35×에서는 획/액세서리가
물리 타이밍 꼬리에 더 크게 노출됐던 것으로 보인다). 여유가 13배라 지표 정의(max → p99) 재설계는
**지금은 불필요**하다고 판단한다. `framesWithHatHiddenWhileTumbling=0 (0%)`도 5회 모두 유지.
(로그: `Logs/coder_b2_hat1~5.{log,xml}`)

### 교차 레이어 영향 로그

1. **`StickConfig` 공개 표면 +4** (`HasRuntimeCharacterScale` / `SetRuntimeCharacterScale` /
   `ClearRuntimeCharacterScale` / `ResolveCharacterScale` 의미 변경). **읽는 쪽은 무수정** —
   `ResolveCharacterScale()`/`ResolveWalkSpeed()`가 그대로 정답을 돌려준다. 다만 **앞으로 누가
   `config.characterScale`을 직접 읽으면 "배포 기본값"이 나온다**(런타임 현재값이 아니다). 전수 확인 결과
   현재 그렇게 읽는 프로덕션 코드는 0곳이다(에디터 굽기 `SceneBootstrapper`는 배포 기본값을 읽는 것이 정답).
2. **`CharacterSaveStore.FilePath`가 상황에 따라 다른 폴더를 가리킨다.** 프로덕션에서는 예전과 100% 동일
   (`s_testingDirectoryOverride == null`). 테스트에서만 임시 폴더. 저장 경로를 새로 조립하는 코드를 추가하려는
   사람은 `Application.persistentDataPath`가 아니라 **`SaveDirectory`** 를 써야 한다.
3. **디버거/테스트 엔지니어에게**: PlayMode 테스트의 전제가 바뀌었다 — 이제 스위트는 **빈 저장 파일**에서
   출발한다. "레벨/장비/할일이 이미 있다"를 암묵 전제로 통과하던 테스트가 있었다면 이번 라운드에 드러났어야
   하는데 296건 중 0건이었다(전부 명시적으로 상태를 만든다). 그리고 앞으로 이 격리가 꺼지면
   `테스트는_개발자의_실제_저장_파일을_읽지_않는다`가 즉시 빨간불을 낸다.

### 리더 판단 필요 — 이번 범위 **밖**으로 남긴 같은 부류 3건 (숨기지 않고 보고)

`characterScale`을 고치면서 **배포 에셋에 런타임 값을 쓰는 지점 전수 조사**를 했다. 남은 4곳:

| 지점 | 필드 | 위험 | 판단 |
|---|---|---|---|
| `StickmanAgent.cs:912` | `desktopDpiScale` | **없음** — `#elif UNITY_STANDALONE_OSX && !UNITY_EDITOR` 안이라 **에디터에서 절대 실행되지 않는다**. 플레이어에서 메모리 값만 갱신(디스크 쓰기 아님) | 조치 불필요(기존 주석이 이미 정확) |
| `CharacterInfoWindow.cs:1045` | `inkColor` | 에디터 Play 중 사용자가 토글하면 에셋 인스턴스 오염 | **미수정** |
| `AppControlDirector.cs:400/410/424` | `inkColor`/`rodeoCursorEnabled`/`verboseDiagnosticsLogging` | 〃 | **미수정** |

`characterScale`과의 **결정적 차이**: 이 3필드는 사용자가 **명시적으로 토글해야만** 쓰이고, 저장 파일에서
자동 복원되는 경로가 없다. `characterScale`은 **매 실행 자동으로** 저장 파일에서 복원돼 오염됐고 그래서
아무도 눈치채지 못한 채 하루가 갔다 — 위험도가 다른 종류다. 오늘이 마지막 관문이라 판단해
**범위를 늘리지 않았다.** 고치는 방법은 위와 완전히 동형(`[NonSerialized]` 런타임 값 + `Resolve…()`)이며,
읽는 곳이 `inkColor` 8곳 / `verbose` 4곳 / `rodeo` 3곳이라 다음 라운드 한 번에 정리 가능하다.
**리더 라우팅 요청**: 다음 라운드 항목으로 받아 주시길.

### 파일

- `Assets/_Project/Scripts/Core/StickConfig.cs` (런타임 배율 분리)
- `Assets/_Project/Scripts/Core/StickmanAgent.cs` (`ApplyCharacterScale` 2단계 + 가드 + `Awake` 초기화)
- `Assets/_Project/Scripts/Core/CharacterSaveStore.cs` (`SaveDirectory` 리디렉션 + `ResetForTesting`)
- `Assets/_Project/Scripts/Tests/PlayMode/GlobalPlayModeTestIsolation.cs` (신규)
- `Assets/_Project/Scripts/Tests/EditMode/GlobalEditModeTestIsolation.cs` (신규)
- `Assets/_Project/Scripts/Tests/PlayMode/DeployedConfigAssetImmutabilityTests.cs` (신규, 4건)

**커밋하지 않았다**(리더가 통합 후 커밋).

### 2026-08-31 — 리더 승인: Blocker 2 완전 해결 (오늘 마지막 관문)
리더 권고안(런타임 Instantiate 복제본)을 코더가 실측 후 기각 — 프리팹에 16개 컴포넌트가 같은
`DefaultStickConfig.asset`을 배선하고 있어, 에이전트만 복제본으로 갈아타면 desktopDpiScale/
inkColor/rodeoCursorEnabled/verboseDiagnosticsLogging이 조용히 진실 둘로 갈라짐을 확인 후
기각한 판단 승인 — 리더 제안을 맹신하지 않고 검증한 좋은 사례. 대신 `characterScale`을 StickConfig
내부에서 직렬화 기본값 + `[NonSerialized]` 런타임 값으로 분리(소비자 무변경, 타입 수준에서 애셋
오염 원천 차단) 승인.
검증 승인: EditMode 147→150, PlayMode 9실패→0실패(1스킵). "git diff 클린 = 안전"이 배치모드
특성상 애초에 감도 0인 검사였다는 지적도 정확 — 메모리 오염을 직접 관측하는 하네스로 재검증한 것 승인.
Major 1(모자 회전 플레이키)도 5회 전부 통과로 근본 해결 확인.
