...
1. **실제 렌더 픽셀 스크린샷은 못 남겼다.** RT를 PNG로 뜨려고 `-batchmode`(그래픽 켬)에서
   `WaitForEndOfFrame`을 쓴 임시 캡처 테스트가 **영구 정지**했다 — Unity는 배치 모드에서
   `WaitForEndOfFrame`을 아예 발생시키지 않는다(문서화된 동작). 임시 파일은 삭제했다.
   **다음에 RT를 캡처할 때는 `WaitForEndOfFrame` 대신 `camera.Render()` 직접 호출 + 즉시 `ReadPixels`를 쓸 것.**
   대신 이번 결론은 포즈 트랜스폼의 절대 수치(78.00도 / 0.51유닛)로 확정했고, 그 수치가 곧
   `Rebuild()`의 `Fallen` 분기 상수(`-78f`, `0.30 * TotalHeight`)와 정확히 일치한다.
2. 전체 회귀(EditMode 63 / PlayMode 182)는 위 1번의 정지 프로세스가 프로젝트 락을 쥐고 있어 미실행.
   내 신규 2종 + 컴파일 무결성은 확인됨(수정 후 실행 EXIT=0, 2/2 통과).
