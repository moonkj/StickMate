...
namespace StickMate.States
{
    ...
    public static class FocusAmbientGestures
    {
        ...
        /// τ = 0.083초)가 2박을 삼킨다는 것이 이 개정의 근거였다 — 1.00초에서는 2박 진폭이 <b>1.24pt</b>까지
        /// 줄어 L1 상시 흔들림(왕복폭 2.07pt)보다 작아진다고 계산했고, 1.20초에서는 <b>2.88pt</b>(L1의
        /// 1.39배)로 회복된다는 것이 근거였다.</para>
        /// <para><b>★ 2026-09-07 test-engineer 실측 정정 — 위 1.24pt/2.88pt/1.39배는 전부 "2박이 1박보다
        /// 작다(81%)"는 잘못된 전제 위에서 나왔다.</b> 실측(C#·Python 독립 재현, 이번 라운드 세 번째
        /// 독립 재현으로 재확인)으로는 2박이 1박보다 작아지지 않는다(≈100.4%) — 되올림이 baseLean까지
        /// 못 내려온 채 다음 램프가 시작돼 골이 얕기 때문이다. 즉 <b>1.00초였어도 2박이 L1 아래로
        /// 붕괴하지는 않았을 가능성이 높다</b>(정확한 pt 재도출은 design-motion 소관이라 여기서 새
        /// 숫자로 덮어쓰지 않는다). 다만 이것이 <c>NodSeconds = 1.20f</c> 자체를 되돌릴 근거는 아니다 —
        /// 기능은 이미 정상 동작이 확인됐고(test-engineer), 1.20초가 틀린 값이라는 뜻이 아니라 <b>그
        /// 값을 고른 계산 과정의 근거 서술이 부정확했다</b>는 뜻이다. 이 근거를 다시 세울지, 아니면
        /// 별도 근거로 1.20초를 재확인할지는 design-motion 판단.</para></summary>
