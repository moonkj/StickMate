...
namespace StickMate.Tests.PlayMode
{
    ...
    public sealed class InfoWindowClippedHitTestTests
    {
        ...
        public IEnumerator ClippedCardEquipButtonIsNotClickableAndComesBackWhenVisibleAgain()
        {
            ...
            // ★ 그 자리는 줄어든 창 <b>바깥</b>이기도 하다 — 33-7-9의 "창 밖 클릭" 탈출구가 그 클릭으로
            //   창을 닫는다(보이는 창 사각형이 판단 기준이다). 착용이 바뀌지 않았다는 위 단언은 그대로
            //   유효하고, ③을 이어가려면 창을 다시 열어야 한다. 화면 크기에 따라 그 자리가 여전히 창
            //   안일 수도 있으므로 닫혔을 때만 다시 연다.
            if (!_window.IsOpen)
            {
