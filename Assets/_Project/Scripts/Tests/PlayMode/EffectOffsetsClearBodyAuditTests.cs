...
namespace StickMate.Tests.PlayMode
{
    ...
    ///
    /// ============================================================================
    /// ★★ 2026-09-07 정정 — 바로 아래 "반짝임(FX) — 결함 아님" 판정은 <b>틀렸다</b>(2차 실기 신고로 반증)
    /// ============================================================================
    /// 이 판정이 남아 있던 원문은 이랬다: <i>"발생 높이가 머리 중심 위 2.0R로 정수리(1.0R)보다도
    /// 훨씬 위라, 가로 폭(±0.9R)이 아무리 넓어도 그 고도에는 애초에 몸/머리 실루엣이 없다"</i>.
    /// 이 문장은 두 가지를 놓쳤다 — (1) 기준을 <b>맨머리 정수리</b>로만 잡아 <b>모자</b>(정수리보다
    /// 훨씬 높이 올라가고, HEAD 슬롯 6종 실측 최고점 2.5425R)를 비교 대상에서 빠뜨렸다. (2) "2.0R"는
    /// 별의 <b>중심</b>일 뿐인데 별 자신이 아래로 1.00R(SparkleArmInR)만큼 더 뻗는다는 사실을 빼서,
    /// 실제 하단(1.0R)을 정수리와 같은 높이로 만들어 놓고도 "훨씬 위"라고 오독했다. 지금은
    /// <c>CharacterFxRenderer.SparkleHeightInR</c>가 4.1R로 고쳐졌고(실측 최댓값의 1.2배 여유),
    /// sortingOrder도 <c>SortAerial</c>(모자보다 아래)에서 <c>SortSparkle</c>(모자보다 확실히 위)로
    /// 갈라섰다. 이 결함의 전용 회귀 잠금은 Tests/PlayMode/SparkleHatOcclusionRegressionTests.cs다 —
    /// <b>이 파일은 더 이상 반짝임을 다루지 않는다.</b>
    ///
    /// ============================================================================
    ...
    /// 성격 자체는 동일하다. 반짝임(위 정정 참고)은 <b>같은 계열의 세 번째 사례</b>이지만 원인은
    /// "R 배수가 몸통 물리 반폭보다 작다"가 아니라 "기준 대상(모자)을 빠뜨렸다 + 자기 크기를
    /// 안 뺐다 + sortingOrder를 안 봤다"로, 앞의 셋과는 실패 지점이 다르다.
    /// </summary>
