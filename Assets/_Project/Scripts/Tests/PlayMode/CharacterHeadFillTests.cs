...
namespace StickMate.Tests.PlayMode
{
    ...
    public sealed class CharacterHeadFillTests
    {
        ...
        //   ★★ 2026-09-02 정정(M6) — 예전 이 자리의 근거는 "하한이 채움과 링에 **똑같이** 걸리는데
        //   채움의 경로 반경이 더 작아서(R/2.2 &lt; R) 무조건 안전하다"였다. 그 전제는 이제 거짓이다:
        //   링은 채운 원반의 **경계선**이라 1.00pt를 쓰고 채움(HeadFill)은 잉크 덩어리라 2.00pt다.
        //   안전 조건이 "무조건"에서 "낱선 하한 ≤ 0.48 × 배율"로 바뀌었고(유도는 EditMode
        //   HeadFillGeometryTests), 다이얼 최소 배율에서 그 임계는 0.168유닛 = 헤드리스 실측(0.100)의
        //   1.68배 · 기준 디스플레이(0.0567)의 2.96배다. 그래서 여기서 재는 절대 조건은 여전히
