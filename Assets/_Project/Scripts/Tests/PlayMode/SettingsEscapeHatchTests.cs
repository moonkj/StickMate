...
namespace StickMate.Tests.PlayMode
{
    ...
    /// [행동 명령] 창의 <c>[✕ 앱 종료]</c>를 <b>빼면 안 됐다</b> — 이 앱에는 Dock 아이콘도 메뉴바
    /// 아이콘도 없어서, 순서를 뒤집으면 마우스만 쓰는 사용자의 종료 수단이 0이 되는 순간이 생긴다.
    ///
    /// <para>★★ <b>2026-09-03 — 그 순서가 지켜졌고, 이제 그 칩은 없다.</b> 이 게이트가 초록인 상태에서
    /// 사용자 지시로 <b>톱니 부채꼴의 [앱 종료]</b>가 신설됐고(UX_FLOW 53 · 2026-09-06에 위성 → 호 슬롯),
    /// 같은 라운드에 행동 명령창
    /// 푸터의 종료 칩이 삭제됐다 — 36-1이 <b>(라) 앱 수준 제어</b>로 분류해 놓고 <b>(가) 행동 명령창</b>에
    /// 얹어 두었던 오분류의 정리다. 즉 위 두 번째 항목이 지적한 *"앱 종료가 여섯 번째 놀이 항목으로
    /// 읽힌다"*는 결함은 <b>이 게이트가 아니라 그 삭제로 닫혔다</b>.</para>
    ///
    /// <para>★ <b>이 게이트의 역할은 끝나지 않았다.</b> 마우스 종료 경로는 여전히 둘(부채꼴 [앱 종료] ·
    /// 이 창)이고, 그중 하나가 스크롤 밖으로 숨는 순간 남는 것은 하나뿐이 된다.
    /// <b>이 파일을 「해결됐으니 지워도 된다」로 읽지 마라.</b></para>
    ///
