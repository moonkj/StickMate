...
namespace StickMate.Tests.EditMode
{
    ...
    public sealed class DialogueLanguageBudgetTests
    {
        ...
        ///
        /// <para>실측(2026-09-03 재측정): 골든 <b>42줄 중 32줄(76%)</b>이 달라진다.
        /// 나머지 <b>10줄</b>은 <see cref="DialogueBudget.MinSeconds"/> 하한에 걸려 있어 1틱으로는
        /// 안 움직인다(하한에 붙은 줄 수 = 안 움직이는 줄 수, 정확히 일치한다) — 그래서 기대값은
        /// "전부"가 아니라 <b>과반</b>이고, 단언도 숫자가 아니라 <c>golden.Count / 2</c>로 쓴다.
        /// 말뭉치가 늘어도 게이트가 썩지 않게 하려는 것이다.</para>
        ///
        /// <para>★ <b>이 비율은 기대값이 아니라 감도 기록이다</b> — 검사가 둔해지는 것을 알아채는 자리다.
        /// 이력: <b>28/33(85%)</b>(<c>docs/localization/verify/gate.out.txt</c>) →
        /// <b>31/42(74%)</b>(붙잡힘 반응 9줄이 들어왔고 그중 6줄이 하한에 붙었다) →
        /// <b>32/42(76%)</b>(2026-09-03 리더 판정으로 붙잡힘 공통 줄 하나가 4자 → 5자가 되어
        /// 하한에서 떨어져 나왔다. 하한 줄 11 → 10).</para>
        ///
        /// <para>★ <b>짧은 대사를 늘릴수록 이 검사는 둔해진다</b>(4자 이하는 계수를 아무리 흔들어도
        /// 안 움직인다). 이 비율이 과반에 가까워지면 하한 <b>위</b>의 표본을 늘려라 —
        /// 그렇지 않으면 <c>golden.Count / 2</c>가 어느 날 조용히 통과 못 하는 게이트가 된다.</para>
        /// </summary>
        [Test]
