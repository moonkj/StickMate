...
namespace StickMate.Tests.EditMode
{
    ...
    /// 38개의 색면 생존율이 30% 미만이던 이유가 이것이다.
    ///
    /// <para>★ 개수는 그 뒤로 올라갔다. <b>2026-09-03 실측 66개</b>(전체 82개 중.
    /// R12 HEAD 이식이 야구모자에 <c>HatBand</c> 하나를 더했다). 세는 법은
    /// 프로덕션 덤프 <c>Tools/ShapeDump/build.sh</c>의 <c>@SHAPE</c> 줄에서 <c>filled=1</c>을
    /// 집계하는 것이다.</para>
    ///
    /// <para>★ 이 숫자는 <b>단언이 아니라 주석</b>이다 — 틀려도 빨개지지 않고 <b>조용히 썩는다</b>.
    /// 옛 주석은 "지금은 60개(2026-09-02 털모자 단이 낱선이 됐다)"였는데 실측 65와 <b>5나 갈라져</b>
    /// 있었다. 채움을 늘리거나 줄이는 라운드는 <b>덤프로 다시 세고</b> 이 줄을 고쳐라.
    /// 아래 검사들은 개수를 세지 않으므로 이 줄이 틀려도 러너는 알려 주지 않는다.</para>
    ///
    /// ============================================================================
