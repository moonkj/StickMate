...
namespace StickMate.Tests.EditMode
{
    ...
    public sealed class PackPaletteGateTests
    {
        ...
        private static readonly (string Name, float Hue, int Primary, int Secondary)[] FrozenPacks =
        {
            ...
            // ★★ 2026-09-08 — 신규 유료 코호트 2개(<c>pack.mine</c> 코호트 7 · <c>pack.arcane</c> 코호트 8)를
            //   대장에 <b>실제로 실린 팩</b>으로 등재한다. 값의 출처는 design-art 확정값
            //   (<c>docs/EQUIPMENT_PALETTE.md</c> §13-4 — 광부 33° 번트 앰버 · 대마법사 138° 룬 에메랄드)이고,
            //   등재하지 않으면 아래 <see cref="팩_코호트_에셋의_색이_동결_대장_안에_있다"/>가 빨개진다
            //   (그 존재 단언이 «에셋 색 ⊆ 대장»을 요구한다 — 그게 이 대장이 살아 있다는 증거다).
            //
            //   ★ 보조색 2건은 사이버(#518C84 -> #518D85)와 <b>같은 함정</b>에 걸려 있어 같은 처방으로 옮겼다:
            //     광부 보조색   #8C7251 -> #8D7251  (V 140/255 = 0.5490 -> 141/255 = 0.5529, ΔE 0.4742)
            //     대마법사 보조 #518C63 -> #518D63  (V 140/255 = 0.5490 -> 141/255 = 0.5529, ΔE 0.7218)
            //   옛 값은 <c>WornColor</c> 명도 하한(0.55) <b>아래</b>라 몸에서 끌어올려졌고, 그 어긋남이
            //   <c>SameByte</c> 허용(0.004 = 1/255) 안에 숨어 「WornColor 항등」 검사를 통과했다.
            //   ★ 채도도 함께 봐야 한다 — 명도만 올리면 S = 59/141 = 0.4184 로 채도 하한(0.42) 아래로
            //   빠져 이번엔 <b>채도</b>가 끌어올려진다. 그래서 최소변(최대 채널 +1)에서 delta 를 60으로
            //   맞춰 S = 60/141 = 0.4255 를 함께 세웠다. 색상각은 오히려 정확해졌다
            //   (33.56° -> 33.00° · 138.31° -> 138.00°). design/art 사양서·에셋도 같은 값이다 — <b>세 곳이 한 값이다</b>.
            //
            //   ★★★ 2026-09-09 M-2 — 대마법사 색상각 <b>138° -> 150°</b>(리더 채택). 사용자가 직접 만든 카드
            //   비트맵 4장의 초록이 실측 <b>148~160°</b>인데 카탈로그가 138°라 «장비창과 착용이 다르다»는
            //   신고의 색 성분이 여기였다(<c>docs/EQUIPMENT_SHAPE_SPEC_PACK_DETAIL_R3.md</c> §2-1·§7 M-2).
            //   비트맵 대표 초록과의 ΔE2000 <b>8.01 -> 3.72</b>.
            //
            //   값은 <b>고르지 않았다</b> — §13-4가 못박은 한 규칙(<c>design/art/verify/pack78.pick</c>:
            //   주색 = 상자∩자립대역∩WornColor항등에서 <b>채도 최대</b> · 보조색 = <b>채도 최소</b>)에
            //   색상각만 150을 넣어 다시 돌렸다. 그 유도기는 H138에서 현행 <c>#00992E</c>/<c>#518C63</c>을
            //   그대로 재현하므로(음성 대조 겸 양성 대조) H150 출력도 같은 자격을 갖는다:
            //     주색   #00992E -> #00994C  (H 149.80° · S 1.000 · V 0.600 — <b>명도 불변</b>, 채도 불변)
            //     보조색 #518D63 -> #518D6F  (H 150.00° · S 0.4255 · V 0.5529 — 아래 V/S 정정을 같은 식으로 적용)
            //   ★ 보조색도 <b>반드시</b> 함께 움직인다 — 바로 아래
            //   <see cref="동결_대장의_색이_선언된_색상각에_있다"/>가 주색뿐 아니라 <b>보조색도</b> 선언
            //   색상각 ±1°를 요구하기 때문이다. R3 사양서 §6의 후보 탐색(<c>pack_bitmap_r3_gate.py</c> §6)은
            //   주색만 옮기고 보조색을 138°에 둔 채 판정해서 이 제약을 보지 못했다 — 그 표의 «H150 최적
            //   #22995D»(S 0.78)는 그 자체로는 자 5개를 통과하지만 <b>채도 최대라는 유도 규칙을 깬다</b>.
            //   여덟 팩 중 하나만 규칙 밖에 두는 것이 ΔE2000 3.72 -> 1.67 보다 비싸다고 판단했다.
            //   ★ 색상각 최소 간격은 25.0° -> <b>22.0°</b>(150 -> 172 사이버)로 좁아진다. 간격 자체를 재는
            //   자는 없고(§13-9가 «최소 간격 N도»를 실측 반증으로 기각했다) 실제 자인 ΔE는 전부 통과한다.
            //   ★ 함께 움직인 곳: <c>Resources/Items/pack_arcane_*.asset</c> 13곳 ·
            //   <c>PackManifest_arcane.asset</c>(색 2 + <c>paletteHueDegrees</c> 1 — R3 사양서 §7이 <b>빠뜨린</b> 자리다) ·
            //   <c>Golden/ItemCatalogGolden.txt</c> 21줄 · <c>docs/EQUIPMENT_PALETTE.md</c> §13-4 ·
            //   <c>design/equipment/verify/pack_bitmap_r3_gate.py</c>의 FROZEN 사본.
            ("광부", 33f, 0xC96F00, 0x8D7251),
            ("대마법사", 150f, 0x00994C, 0x518D6F),
        };
