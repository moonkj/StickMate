...
namespace StickMate.Tests.EditMode
{
    ...
    public sealed class AmbientChatterEligibilityTests
    {
        ...
        /// <summary>
        /// ★ 강제 발화 펄스는 <b>확률과 쿨다운</b>을 건너뛴다. 그러나 <b>자격</b>은 건너뛰지 않는다 —
        /// 그쪽은 "말할지 말지"가 아니라 "그 문장이 참인가"의 문제이고, 원칙 1에는 예외가 없다.
        ///
        /// <para>★★ <b>2026-09-07 — 이 펄스를 세우는 프로덕션 코드가 없어졌다.</b> 원문은
        /// <i>"지금 말풍선을 보고 싶다는 사용자 명령(Ctrl+Opt+Cmd+B / 행동 명령창 [말 걸기])"</i>이었는데,
        /// 그 두 진입점이 <b>사용자 지시로 함께 폐지</b>됐다
        /// (<c>Interaction/AppControlDirector.cs</c>의 「말 걸기 폐지」 절).
        /// ⇒ 이 테스트는 지금 <b>프로덕션 생산자가 0인 분기</b>를 재고 있다. 분기 자체는
        /// <c>AmbientChatter.TryRollChatter</c>에 남아 있고(다른 담당의 파일이라 이번 라운드가
        /// 걷지 않았다), <b>그 분기를 지울지 말지는 리더 배정 대상</b>이다. 이 테스트를 지우는
        /// 것은 그 결정 <b>다음</b>이다 — 먼저 지우면 분기가 무방비로 남는다.</para>
        /// </summary>
        [Test]
