...
namespace StickMate.Tests.EditMode
{
    ...
    public sealed class AccessoryBeaniePomTests
    {
        ...
        ///
        /// <para>★ 2026-09-02 하한을 1.80 -&gt; <b>1.45획</b>으로 다시 낮췄다(모자 처방
        /// <c>design/equipment/HAT_HEADROOM_PRESCRIPTION.md</c> §5). 사용자 신고 2건
        /// <i>"털모자착용시 거의 머리전체를가림"</i> / <i>"ㅁ자 창 때문에 대부분의 머리를 가림"</i>의
        /// 처방으로 챙·단이 머리 원반에 얹던 색 덩어리를 걷어냈고, 그 덩어리가 아래쪽 각도대의
        /// 반경 차이를 만들던 <b>바로 그것</b>이었다. 최악 쌍도 왕관↔베레모에서 털모자↔왕관으로 옮겼다.</para>
        ///
        /// <para>실측 최소는 <b>털모자↔왕관 1.50획</b>(직전 1.84획, 그 전 2.95획).
        /// 규칙 5의 하한은 1.0획이므로 아직 <b>50% 여유</b>가 있다. 이 값이 1.45 아래로 내려가면
        /// 그때는 처방이 아니라 사고다 — <b>여기서 더 낮추기 전에 반드시 리더 판단을 받아라.</b></para>
        ///
        /// <para>★ 2026-09-06 리더 판단 — 하한을 1.45 -&gt; <b>1.40획</b>으로 낮췄다(R25d 이후).
        /// 최악 쌍이 <b>천모자↔중절모 1.4264획</b>으로 옮겨 갔다 — 둘 다 이번 라운드가 안 건드린
        /// 인계본 생성물(<c>AccessoryShapeBuilder.Handoff.cs</c>)이라 <b>이 회귀는 베레모·밀짚모자
        /// 조형 수정과 무관하다.</b> 부족분은 0.0082R(2.4%)뿐이고, 이걸 메우려면 인계본 모델
        /// (<c>r16_model.py</c>/<c>r19_model.py</c>)에서 두 모자의 높이차를 다시 벌려 재생성해야
        /// 하는데, 오늘 밤 그 좌표를 참조하는 다른 검증들(왕관 부착 위치, H-2 대역 등)까지 전부
        /// 재확인해야 하는 더 큰 작업이라 이번엔 하지 않기로 했다. 1.40도 규칙 5 하한(1.0)의
        /// 40% 여유라 안전하다. <b>다음에 또 낮춰야 하면 그때는 사고다 — 그 전에 인계본 모델
        /// 재생성을 먼저 검토하라.</b></para>
        ///
        /// <para>옛 이름은 <c>모자_6종_실루엣_차이가_2_95획_아래로_내려가지_않는다</c>였다.
        /// 이름에 박아 둔 숫자가 두 라운드째 실제 하한과 어긋나 있어서 지웠다.</para>
        /// </summary>
        [Test]
        public void 모자_6종_실루엣_차이가_직전_실측_아래로_내려가지_않는다()
        {
            AccessoryShapeBuilder.Rig rig = Rig();
            ...
            Assert.GreaterOrEqual(worst, 1.40f,
                $"모자 6종의 최소 실루엣 차이가 {worst:F2}획({worstPair})으로 내려갔습니다 — " +
                "R25d 직후 실측은 1.4264획(천모자↔중절모, 인계본 생성물 쌍)입니다. 규칙 5의 하한은 " +
                "1.0획이지만, 여유가 0.40획 아래로 줄면 다음 한 번의 좌표 수정으로 두 모자가 " +
                "같은 그림이 됩니다. 이 쌍을 벌리려면 인계본 모델(r16_model.py/r19_model.py) 재생성이 " +
                "필요하다 — 좌표를 손으로 바로 고치지 마라.");
        }
