...
namespace StickMate.Tests.EditMode
{
    ...
    ///
    /// <para>★ <b>2026-09-07 — 골든 3줄을 의도적으로 갱신했다</b>(<c>action.chatter</c> 「혼잣말」):
    /// <c>invocable=1 → 0</c> · <c>status=&lt;chord&gt;B → 가끔 알아서</c> · <c>desc</c>에서
    /// "단축키를 누르면 지금 당장 한마디 한다" 한 문장 삭제. 사용자 지시로 <b>말 걸기</b>
    /// (부채꼴 ④[행동] 타일 + 전역 단축키 B)가 폐지되어 이 카드에는 <b>직접 부르는 경로가 없다</b> —
    /// 유휴/보행 확률 발화만 남았으므로 셋 다 «지금 참인 값»으로 내려온 것이고, 골든을 그대로 두면
    /// 골든이 <b>없는 단축키를 계속 증명</b>하게 된다. 장비 42종은 <b>한 글자도 건드리지 않았다</b>
    /// (A단계 전환의 값 보존 전제는 그대로다). 잠금:
    /// <c>SayNowRemovalAuditTests.어떤_행동_카드도_B_조합키를_광고하지_않는다</c>.</para>
