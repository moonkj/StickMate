...
namespace StickMate.Dialogue
{
    ...
    /// </list>
    ///
    /// <para><b>어조 — 리더 판정으로 사용자의 낱말을 쓴다(2026-09-03 확정, 판정 대기 종료).</b>
    /// <c>design-narrative</c>는 실측 33/33이 전부 반말이고 그중 <c>나 안 해!</c>·<c>흥... 그럼 한 입만이다</c>처럼
    /// <b>대드는 결</b>이라는 이유로 요청·애원을 반말 명령형으로 바꾸자고 했고, 그 판단 자체는 틀리지 않았다.
    /// <b>두 안이 다 성립하는 자리라서 사용자가 실제로 쓴 낱말</b>(<c>놔줘 놔줘</c>)을 택한 것이다. 근거 셋:</para>
    /// <list type="number">
    ///   <item>사용자 요청 원문의 낱말 그대로다 — <b>"놔줘 놔줘 라는 멘트 같은거"</b>.</item>
    ///   <item><b>9줄 중 1줄</b>이고, 대드는 어조는 나머지가 이미 갖고 있다
    ///     (<see cref="AnyLines"/>의 <c>야!</c>·<c>안 놔?</c>·<c>어딜 잡아</c>).</item>
    ///   <item>붙잡혀 들려 있는 작은 캐릭터에게는 애원 쪽이 오히려 산다.</item>
    /// </list>
    /// <para>★ <b>두 벌을 함께 두지 않는다</b> — 채택되지 않은 명령형 변형은 코드에 남기지 않았다.
    /// 되돌리려면 이 판정을 먼저 뒤집고 <see cref="AnyLines"/>의 그 한 줄만 교체한다.
    /// 글자수가 4에서 5로 늘어 가독예산이 하한(<c>DialogueBudget.MinSeconds</c>)에서 떨어져 나왔다 —
    /// 골든(<c>Tests/EditMode/Golden/DialogueBudgetKoGolden.txt</c>)의 그 한 줄이 함께 바뀌는 이유다.</para>
    ///
    ...
    public static class GrabReactionLines
    {
        ...
        {
            "야!",
            // ★ 사용자 요청 원문의 낱말. 리더 판정으로 확정됐다(근거는 클래스 문서 <어조> 절).
            "놔줘 놔줘",
            "안 놔?",
            "어딜 잡아",
        };
