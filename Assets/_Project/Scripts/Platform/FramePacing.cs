...
namespace StickMate.Platform
{
    ...
    internal static class FramePacing
    {
        ...
        // ============================================================================
        //
        // ★ 2026-08-31 당시 확정된 인과(코드 검증) — ★★ 2026-09-05 정정(perf-doc 신고): 아래 3·4번과
        //   그 다음 문단의 "플랫폼 비대칭"은 **2026-08-31 당시** 서술이며 현행 코드와 다르다. 2026-09-01부터
        //   FramePacingPolicy.BuildPlan은 보는 사람이 있는 등급(Calm/Still)에서 **플랫폼과 무관하게
        //   renderFrameInterval만** 나눈다(Calm 1/2, Still 1/StillDivisor). targetFrameRate/vSyncCount는
        //   손대지 않으므로 Windows에서도 게임 루프·커서 폴링은 60Hz다 — 즉 지금 Calm은 "표시만 30fps"이지
        //   "루프 30Hz"가 아니다. 5번(복귀 지연)은 여전히 참이고 아래 처방 (A)(B)는 그것을 위한 것이다.
        //   당시 인과는 역사로 남긴다(왜 이 홀드가 생겼는지가 여기 있다):
        //   1. 사용자가 정보창을 열고 읽는다 -> 마우스 무입력 2초 경과.
        //   2. 그 사이 캐릭터가 자율 배회의 Idle 구간(실측 2~6초)에 들어간다 -> **Calm 등급**.
        //   3. (당시) Windows에서 Calm이 `Application.targetFrameRate`를 60에서 30으로 내렸다
        //      (baseVSyncCount=0이라 targetFrameRate 쪽이 나뉘었다) — 즉 당시에는 게임 루프 자체가 30Hz였다.
        //      ※ 현행: 위 정정대로 renderFrameInterval만 나뉜다(루프 60Hz 유지).
        //   4. (당시) CharacterInfoWindow의 타이틀바 드래그는 `Update()`마다 OS 커서를 한 번 폴링해
        //      패널 위치를 갱신하므로 커서 표본 주기도 30Hz가 되어 창이 커서를 계단식으로 따라왔다.
        //      ※ 현행: 루프가 60Hz라 이 축은 사라졌다.
        //   5. 사용자가 다시 마우스를 움직여도 등급 복귀는 다음 관측 폴링(최대 0.2초)에 가서야
        //      일어난다 -> **모든 상호작용의 첫 0.2초가 절반 프레임레이트로 시작**한다.
        //
        // (당시) macOS에서는 같은 등급이 renderFrameInterval만 건드리므로 게임 루프는 60Hz 그대로였고
        // (= 커서 추적 정확도는 유지되고 표시만 30fps), 사용자가 "윈도우에서는"이라고 말한 것과
        // 플랫폼 비대칭이 정확히 일치했다. ※ 현행: 그 비대칭은 2026-09-01에 없어졌다(양 플랫폼 동일 경로).
        //
