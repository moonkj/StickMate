namespace StickMate.Platform
{
    ...
    /// 직접 알 필요가 없다).
    ///
    /// 왜 "필요한 것만" 있는가: 이 채널의 목적은 범용 키보드 입력이 아니라 **앱 제어용 단축키**다.
    /// 전체 키맵을 노출하면 조회 전용이라 해도 사실상 키로거의 형태가 되어, CLAUDE.md 비침해 원칙과
    /// 어긋난다. 여기 열거된 것 외에는 <b>물어볼 수단 자체가 없다</b> — 조합키 3개 + 동작키 17개.
    ///
    /// <para>★ 2026-09-02 정정 — 이 자리에 원래 "동작키 <b>4개</b>"라고 적혀 있었다. 기능이 늘면서
    /// 동작키는 4 → 17이 됐는데 숫자만 그대로 남아 4.25배 틀린 채로 굳었다. 이 문장은
    /// <b>프라이버시 논거</b>이고 심사·문의 때 그대로 제출된다.</para>
    ///
    /// <para>그래서 "개수가 적다"에 기대지 않고 실제로 참인 것을 적는다: <b>동작키는 조합키 3개가
    /// 모두 눌린 동안에만 조회된다.</b> Interaction/AppControlDirector.cs의 TickHotkeys가
    /// <c>chord &amp;&amp; IsKeyDown(...)</c> 꼴이고 C# 단락 평가라, 조합이 아니면 네이티브 호출 자체가
    /// 일어나지 않는다. 동작키가 몇 개가 되든 이 게이트는 늘지 않는다.
    /// 숫자가 다시 썩는 것은 Tests/EditMode/GlobalKeyPrivacyClaimTests가 enum과 대조해 막는다.</para>
    /// </summary>
    public enum GlobalKey
    {
        ...
        /// <c>ControlAction.UserHide</c>로 배선하고, 부팅 배너의 <b>사용자</b> 단축키 목록에도 실린다.
        /// 글자 한 자는 <c>StickmanAgent.UserHideHotkeyLetter</c>가 소유하고, 키코드는 양 플랫폼에
        /// 이미 매핑돼 있다(<c>MacWindowService</c>의 <c>kVK_ANSI_K</c> / <c>Win32WindowService</c>의 'K').
        ///
        /// <para>★ <b>이 자리를 다른 기능에 내주지 마라. 숨김 상태의 유일한 탈출구다.</b> 숨는 동안에는
        /// 톱니(<c>IsSuspended</c>)도 부채꼴·창·팝오버(<c>ArePanelsSuppressed</c>)도 스스로 내려가
        /// <b>마우스 경로가 0</b>이 되고, 이 키가 죽으면 사용자에게 남는 수단은 강제 종료뿐이다.
        /// Q(종료)와 같은 급의 계약이라 개발 게이트 뒤에도 두지 않는다 —
        /// Tests/EditMode/ManualHideAxisSeparationAuditTests가 배선·게이트·배너 세 가지를 잠근다.</para>
        ///
        /// <para>★ 2026-09-02 정정 — 여기 원래 <i>"비어 있다(예약) … 눌러도 아무 일도 일어나지 않고,
        /// 배너의 단축키 목록에도 나오지 않는다"</i>라고 적혀 있었고 <b>두 문장 다 거짓</b>이었다.
        /// 예약이던 구간은 격파 놀이 삭제(바인딩만 제거)와 숨김 배선 사이 <b>같은 날 약 9시간</b>뿐인데
        /// 주석만 남았다. 하필 <b>탈출구</b>를 "아무 일도 없다"고 설명하는 문장이라, 읽은 사람이
        /// 지워도 되는 값으로 오해한다.</para></summary>
        K,
    ...
    ///
    /// **비침해 원칙 유지**: 조회만 하고 어떤 입력도 주입하지 않으며, 위 GlobalKey에 열거된 20개 키
    /// 외에는 애초에 물어볼 수단이 없다(조합키 3 + 동작키 17). 그중 동작키는 조합키 3개가 모두 눌린
    /// 동안에만 조회되므로 — 그리고 그중 5개는 개발 게이트가 열렸을 때만 조회되므로 —
