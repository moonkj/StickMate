    ...
    public static class StallAttribution
    {
        ...
        private static void EmitEnumerationBaseline()
        {
            ...
            if (_enabled && (_baseCount > 0 || _baseFsCount > 0))
            {
                ...
                //   RecordNativeWindowListQuery 문서에 있다 — **설명은 소스에, 숫자만 로그로**가 이
                //   파일의 규율이다(2026-09-02 로그 감량 라운드). 요약하면:
                //     · 발판열거 − OS창목록 = 우리 후처리(가려짐 솔버·Dock 실측·안전망 합성)
                //     · 벽시계 >> CPU -> 블로킹/스케줄링 대기 / CPU가 함께 큼 -> 우리 관리 코드
            }
