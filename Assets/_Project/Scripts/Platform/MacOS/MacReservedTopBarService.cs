...
namespace StickMate.Platform.MacOS
{
    ...
    /// </code>
    /// ★ <b>이 두 줄의 값이 무엇으로 잰 것인지</b>(그리고 포인트인지 픽셀인지)는
    /// <see cref="MacReservedScreenEdgeService"/> 클래스 문서에 <b>재현 절차와 함께</b> 한 곳에만 적는다.
    /// 여기에 다시 적으면 두 벌이 되고, 이 저장소의 규칙대로 다음 라운드에 한쪽만 고쳐진다.
    ...
    ///  <item>★ <b>기동 직후 약 2초</b>: 라이브러리가 아직 Unity 창을 안 붙잡아 조회 자체가 실패한다 →
    ///        false → 인셋 0. <b>이것은 "메뉴 막대가 없다"가 아니라 "아직 못 쟀다"이고</b>, 이 계약은
    ///        둘을 <b>구분해서 말할 수 없다</b>(<c>bool</c> + <c>float</c> 하나뿐이다).
    ///        구분이 필요한 소비 측 — 특히 <c>Start()</c>에서 한 번 읽고 굳히는 곳 — 은
    ///        <see cref="ReservedTopBarProbe"/> 문서의 "일회성으로 읽지 마라" 절을 먼저 읽어라.
    ///        실제 사고 1건이 여기서 나왔다(2026-09-03 "OS 예약 띠 0.0pt" 오진).</item>
    /// </list>
    ///
    /// ============================================================================
