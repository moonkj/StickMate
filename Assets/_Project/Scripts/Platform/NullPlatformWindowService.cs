...
namespace StickMate.Platform
{
    ...
    public sealed class NullPlatformWindowService : IPlatformWindowService, ICursorPositionService, ILocalClickCaptureService, IDesktopIconLayoutService
    {
        ...
        //
        // ============================================================================
        // ★★★ 2026-08-29 (3) 40pt -> 6pt — 사용자 3차 신고 "아직도 바닥과 독위치를 정확히 파악못하는거 같음"
        //     (Dock 계산부는 무죄다. 직전 라운드에 실측으로 오차 0.5pt까지 맞춰뒀다 — 커밋 12d7672.)
        // ============================================================================
        // 40pt였던 근거는 다음 실측이었다: "루트(=발 원점)는 발판 상단에 정확히 놓이지만 렌더러 바운즈의
        // 아래끝은 루트보다 0.55월드유닛(=22pt) 더 내려간다(루트 -11.60일 때 bounds.min.y=-12.15)".
        //
        // ★ 그 0.55유닛은 캐릭터의 실제 모습이 아니라 **LineRenderer.bounds의 계측 아티팩트**였다.
        //   이번 라운드에 렌더러별 바운즈를 전부 찍어보고 드러났다(Logs/coder_floor_probe2.log):
        //   눈(LeftEye, 반지름 0.02유닛짜리 점)의 bounds.size.y가 1.027, 머리 링(지름 0.33)이 1.330,
        //   즉 **모든 LineRenderer의 바운즈가 Y로 정확히 +1.0유닛(위아래 0.5씩) 부풀려져 있다**
        //   (뷰 정렬 빌보드 라인에 대한 Unity의 보수적 바운즈). 실제 잉크 최저점은 LineRenderer의
        //   정점 좌표 ± 선 반폭으로 직접 계산해야 나온다.
        //
        // 그래서 상태별로 "그려지는 잉크의 최저점이 발판 상단보다 얼마나 아래인가"를 다시 실측했다
        // (Logs/coder_floor_probe5.log, 정점 기하 기준. 자유 관찰 45초 + 스폰 낙하 + RAGDOLL 강제 3회):
        //     Walk 1.48pt / Idle 1.82pt / Getup 2.62pt / Ragdoll 4.79pt / LandingCrouch 5.95pt(최악)
        // 즉 발끝이 잘리지 않기 위해 실제로 필요한 최소 높이는 **약 6pt**이지 40pt가 아니다. 40pt는
        // 부풀려진 바운즈(22pt)에 다시 여유를 얹은 값이라, 사용자 눈에는 Dock 바깥에서 캐릭터가 화면
        // 바닥보다 40pt(캐릭터 키의 절반이 넘는다 — 키는 약 70pt) 위에 떠 있는 것으로 보였다.
        //
        // 8pt를 고른 근거 = 실측 최악값 5.95pt + 약간의 여유(2.05pt, 약 35%). 즉 관측된 모든 자세에서
        // 잉크가 화면 밖으로 단 1pt도 나가지 않으면서, 서 있을 때 발 잉크는 화면 맨 아래에서 6.2pt
        // (8 - 1.82) 위 — 캐릭터 키(약 70pt)의 9%, 획 두께(약 2.5pt)의 2.5배라 육안으로는 바닥에 닿아
        // 있다(예전 40pt는 발 잉크가 38pt = 키의 54% 위에 떠 있었다 — 사용자가 본 그 "부양"이다).
        // RAGDOLL이 더 크게 벌어지는 극단은 화면 클램프(StickmanBlackboard)와 물리 바닥이 따로 막는다.
        // Dock 띠(하단 75pt) 안쪽이므로 "Dock 바깥에서는 화면 바닥으로 내려간다"도 그대로 성립한다.
        // 회귀 감시: Tests/PlayMode/FloorContactVisibilityTests.cs가 (1) 접지 시 발-화면바닥 간격 상한과
        // (2) 모든 상태에서 잉크가 화면 밖으로 안 나감을 동시에 강제한다(값을 되돌리면 (1)이 즉시 실패).
        public const float BottomSafetyNetInsetPoints = 8f;

        /// <summary>
        /// 안전망/더미 발판의 상단을 화면 진짜 바닥에서 위로 띄우는 비율(= 그 발판 띠의 두께 비율과 동일).
        /// 8pt / 982pt ≈ 0.0081. 이 값 하나가 (a) 이 클래스의 더미 발판, (b) FallbackPlatformWindowService의
