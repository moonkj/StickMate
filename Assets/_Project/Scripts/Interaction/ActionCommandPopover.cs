...
namespace StickMate.Interaction
{
    ...
    ///    이후 <b>성공해도 창은 남는다</b>. 창은 톱니 자리에 뜨고 연출은 캐릭터 자리에서 일어난다
    ///    (근거 전문: <see cref="OnCommandClicked"/>). 비켜야 할 때 <b>사용자가</b> 쓰는 탈출구는
    ///    오른쪽 위 [✕]다
    ///    (★ 2026-09-02 정정 ① — 예전에는 "창 바깥 아무 곳이나"였다. 지금은 밖 클릭으로 닫히지 않는다).
    ///
    ///    ★★ <b>2026-09-02 정정 ② — 이 창은 끌어 옮길 수 없다.</b> 여기에도 <see cref="PopoverPanel"/>
    ///    에도 드래그 구현이 <b>없다</b>. 게다가 앵커인 톱니를 끌면 <c>InfoGearIconWidget</c>이
    ///    <c>CollapseMenu(GearMenuCollapseMode.Drag, ...)</c>를 거쳐 <c>GearRadialMenuWidget.Collapse()</c>를
    ///    부르고, 그 함수의 첫 동작이 <c>ClosePopovers()</c>다 — <b>옮겨지는 게 아니라 닫힌다</b>.
    ///    자리를 바꾸려면 톱니를 원하는 곳에 놓고(그 자리는 저장된다) 창을 <b>다시 열어야</b> 한다.
    ///    창은 열릴 때 <c>ButtonScreenRect(GearMenuButton.Action)</c>을 앵커로 받아 그 자리에서 자란다.
    ///    이 문단은 종전 주석의 "창을 끌어 옮기면 된다"를 대체한다(그 문장은 코드에 근거가 없었다).
    ///
    /// ============================================================================
    ...
    public sealed class ActionCommandPopover : PopoverPanel
    {
        ...
        protected override UiWindowId WindowDragId => UiWindowId.ActionCommand;
        ...
        /// 저장한다). ★ 단 <b>창이 열려 있는 동안</b>은 아니다 — 톱니를 끌면 그 창은 옮겨지는 게 아니라
        /// <b>닫힌다</b>(클래스 문서 "정정 ②"). 톱니 자리를 바꿔 겹침을 피하려면 창을 다시 열어야 한다.
        /// 활쏘기는 발판 위 <b>가로</b> 밴드다 — 과녁 top이 정확히 신장 1H이다
