...
namespace StickMate.Interaction
{
    ...
    /// (★ 그 파랑 강조는 <b>2026-09-03에 브라스 <c>#C8A15A</c>로 다시 바뀌었다</b> —
    /// <see cref="Accent"/> 문서 참고. 이 문단은 그때의 기록이다.)
    ...
    public static class UiChrome
    {
        ...
        /// <summary>
        /// 착용 중인 카드의 테두리 rgba(93,161,245,<b>0.75</b>) — "지금 걸치고 있음"을 말하는 <b>파랑</b>이다.
        ///
        /// <para>★★ <b>이 파랑은 예약돼 있다. 다른 뜻으로 쓰지 마라</b>(TEAM.md 「색 예약」, 2026-09-03 확정).
        /// 특히 <b>등급</b>에 파랑을 쓰면 <c>persona-newcomer</c>가 실측으로 신고한 그 상태가 된다 —
        /// *"지금 뭘 쓰고 있는지 눈으로 못 찾겠다."*</para>
        ///
        /// <para>★ <b>2026-09-03 — 이 값은 더 이상 <see cref="Accent"/>와 같지 않다.</b> 예전에는
        /// 둘이 <b>완전히 같은 값</b>이었고("강조색 계열로"라고 적혀 있었다), 그래서 화면의 모든 강조
        /// 크롬과 「착용 중」이 <b>구분되지 않았다</b>. 강조색이 브라스로 바뀌면서 그 겹침이 풀렸고,
        /// 이제 이 파랑은 <b>「착용 중」 전용</b>이다(ΔE 90.52로 브라스에서 멀다).
        /// <b>이것을 다시 <c>Accent</c>로 대체하지 마라 — 그러면 예약이 사라진다.</b></para>
        ///
        /// <para>★ <b>2026-09-03 — α만 0.55 -> 0.75로 올렸다. hex는 한 자리도 안 바꿨으므로 위 예약은
        /// 그대로 유효하다</b>(UI_SURFACE_SPEC §15.6-a 검산 4). 올린 이유는 이 색 자체의 문제가 아니라
        /// <b>이웃이 밝아졌기 때문</b>이다: 같은 라운드에 카드 테두리의 「기본」 자리를 등급 램프가
        /// 승계했고(<see cref="RarityBorder"/>) 그 최대(전설)가 카드면 대비 <b>4.45</b>다.
        /// α0.55에서 이 파랑은 2.82라 <b>전설 카드보다 어두웠다</b> — 「지금 걸치고 있는 칸」이
        /// 아무 상태도 아닌 칸보다 흐려지는 역전이다. α0.75에서 <b>4.09</b>가 되고 등급 4색과
        /// 최소 ΔE <b>42.57</b>로 떨어져 있다.</para>
        /// </summary>
        // ★ 2026-09-03 α 0.75 → 1.00 (design-art §30-7-3, 간격 하한은 §30-7-2 규칙 L-1) — 등급 테두리
        //   승계(α0.55) 도입 후 착용 서열이 전설보다 어두워지는 역전이 실측됐다(착용 4.07 < 전설 4.44,
        //   design-art 자 / §15 표기로는 4.09 < 4.45 — 두 자가 0.02 안에서 맞는다). 램프 값·α는 안
        //   건드리고 착용 쪽만 불투명으로 올려 여유 2.3배(ΔL* +9.97, 하한 ΔL* 4.4의 2.3배) 확보. hex 불변.
        //   ★ 실기 미확인 — §30-12 K-7(Windows sRGB에서 파랑 α1.00이 황동 전설보다 실제로 밝은가).
        public static readonly Color CardBorderWorn = new Color(0.365f, 0.631f, 0.961f, 1.00f);

        /// <summary>커서가 올라간 카드의 테두리 rgba(255,255,255,<b>0.62</b>).
        ///
        /// <para>★ <b>2026-09-03 — 0.24 -> 0.62</b>(UI_SURFACE_SPEC §15.6-a 검산 3). 같은 이유다:
        /// 「기본」 자리를 등급 램프가 승계하면서 <b>아무 상태도 아닌 카드가 호버보다 밝아졌다</b>.
        /// α0.24의 실효색 <c>#52555A</c>는 카드면 대비 2.20으로 전설 기본(4.45)의 <b>0.49배</b>이고,
        /// 등급 4색과 최소 ΔE도 <b>7.42</b>로 변별 하한 7.8에 미달이라 <b>호버하면 테두리가 어두워지고
        /// 색도 안 갈리는</b> 상태였다. α0.62에서 실효색 <c>#A8AAAD</c> · 대비 <b>7.09</b>(등급 최대의
        /// 1.59배) · 등급과 최소 ΔE <b>25.87</b>이다. α0.70은 선택(15.00)에 붙어 과했다.</para>
        ///
        /// <para>★ <b>이 토큰의 호출부는 카드 테두리 한 곳뿐이다</b>(2026-09-03 전수 grep:
        /// <c>CharacterInfoWindow.Cards.cs</c>의 상태 3항식). 창 밖으로 새는 영향이 없다는 것을
        /// 올리기 전에 확인했다 — 확인하지 않으면 이 한 줄이 다른 화면을 조용히 밝힌다.</para></summary>
        public static readonly Color CardBorderHover = new Color(1f, 1f, 1f, 0.62f);


        /// <summary>
        /// ★ 강조색 — <b>브라스 <c>#C8A15A</c></b>. 이 팔레트의 강조색은 <b>하나뿐</b>이다.
        ///
        /// <para><b>2026-09-03 파랑(<c>#5DA1F5</c>) -> 브라스로 교체.</b> 리더 판정은 커밋 <c>eca8c58</c>에서
        /// 났는데 <b>코드에 안 들어와 있었다</b>(판정과 반영을 같은 칸에 적으면 생기는 일 — TEAM.md).
        /// 파랑이 남아 있는 동안 실제로 난 결함은 이것이다: 그 값이
        /// <see cref="CardBorderWorn"/>과 <b>완전히 같아서</b> 「지금 착용 중인 카드」가 다른 강조 크롬과
        /// 구분되지 않았다(<c>persona-newcomer</c> 신고). 두 색을 갈라 놓는 것이 이 교체의 실질이다.</para>
        ///
        /// <para><b>대비는 나빠지지 않았다</b>(실측): 카드 6.20 -> <b>6.84</b> / 패널 6.75 -> <b>7.45</b> /
        /// 보조면 6.34 -> <b>7.00</b>. 채워진 면 위 잉크(<see cref="OnAccentSolid"/>)도 7.17 -> <b>7.91</b>.</para>
        ///
        /// <para>★ <b>알고 받는 비용</b>(PALETTE_SPEC §12-5): 브라스는 등급 램프와 <b>같은 가족</b>이다 —
        /// 영웅 리본과 ΔE <b>12.86</b>(변별 하한 7.8은 넘고 식별 하한 48.6에는 한참 못 미친다).
        /// 그래서 <b>강조 칩과 영웅 리본을 한 자리에 붙여 놓지 않는다</b>. 붙일 수밖에 없는 자리에서는
        /// 등급을 <b>칸 수로만</b> 읽게 둔다 — 주 채널이 이미 칸 수이므로 추가 비용은 0이다.</para>
        /// </summary>
        public static readonly Color Accent = new Color(0.784f, 0.631f, 0.353f, 1f);

        /// <summary>강조색의 옅은 채움(선택 칩 배경 등) rgba(200,161,90,0.14).</summary>
        public static readonly Color AccentSurface = new Color(0.784f, 0.631f, 0.353f, 0.14f);

        /// <summary>강조색 테두리 rgba(200,161,90,0.55).</summary>
        public static readonly Color AccentBorder = new Color(0.784f, 0.631f, 0.353f, 0.55f);
        ...
        /// </summary>
        public static readonly Color WarmAccent = new Color(0.784f, 0.631f, 0.353f, 1f);
        ...
        /// <summary>옅은 <see cref="AccentSurface"/> 위에 얹는 글자(#8fc3ff).
        /// <para>★ <b>2026-09-03 — 「같은 색조의 밝은 값」이라고 적혀 있었는데 그 문장이 깨졌다.</b>
        /// 강조색이 브라스로 바뀌면서 이 값만 <b>파랑 계열로 남았다</b>. <b>대비는 멀쩡하다</b>(실측:
        /// 브라스 wash 위 7.03 / 7.75 / 7.21 — 파랑 시절 7.14 / 7.87 / 7.32과 사실상 동일. α 0.14짜리
        /// wash라 바탕이 거의 안 움직인다). 그래서 <b>값을 바꾸지 않았다</b> — 새 색을 여기서 발명하면
        /// 그 색만 어떤 표에도 근거가 없는 값이 된다.</para>
        /// <para><b>미결로 남긴다: 브라스 계열 밝은 값이 필요한지는 <c>design-art</c> 판정 사항이다.</b>
        /// 실기 캡처 항목이고, 이 주석을 지우기 전에 그 판정을 받아라.</para></summary>
        ...
        /// ★ 33-1에서 반전됐다 — 강조색이 <b>밝은</b> 색이 됐으므로 그 위 글자는 어두워야 한다(34-7 #3).
        /// 2026-09-03 강조색이 파랑에서 브라스로 바뀌어도 이 판단은 그대로다(브라스 위 <b>7.91:1</b>,
        /// 파랑 시절 7.17:1보다 오히려 낫다).</summary>
        ...
        //   그때 파랑에서 <b>청록으로 이동</b>한 것은 아래 두 번째 색 하나다: 당시 Accent(#5da1f5)와
        //   같은 색상대라 "강조"와 "카테고리"가 구분되지 않기 때문이다(당시 그 자리는 EYES/HAIR 몫이었다).
        // ★ <b>2026-09-03 강조색이 브라스로 바뀌었다 — 이 표를 따라 옮기지 않아도 된다.</b> 실측으로
        //   확인했다: 강조색과 가장 가까운 카테고리 틴트는 이제 주황(HEAD) ΔE <b>28.88</b>이고,
        //   파랑 시절의 최근접(라벤더 ΔE 28.89)과 <b>사실상 같다</b>. 분리 여유가 줄지 않았다.
        ...
        //
        // ★★ <b>2026-09-03 R9 보정판</b> — 영웅 <c>#DBBD7F -> #DEC081</c> · 전설 <c>#F9CB70 -> #FFD375</c>
        //    (일반·희귀는 그대로). design-art 판정 R9-3: <b>회색조 미달 2쌍 -> 0쌍</b>.
        //    이동량은 ΔE 1.15 / 3.02로 <b>같은 색으로 보이는 범위</b>이고(변별 하한 7.8의 1/7~1/2),
        //    다른 축은 전부 동일하거나 개선됐다 — 카드 바탕 대비 9.13->9.41 / 10.86->11.66,
        //    휘도 단조 4/4 유지, 일반↔전설 ΔE 48.6 초과 유지.
        //    <b>이 구멍은 §12-1이 남긴 유일한 미완이었다</b>(아래 참조). 되돌리지 마라.
        //
        // 색은 등급의 <b>주 채널이 아니다</b>. 실측(PALETTE_SPEC §12-0 / §12-1 / §27-10-1):
        //   · 인접 쌍 ΔE 15.16 / 18.48 / 18.02 — 변별 하한 7.8은 넘지만
        //   · <b>식별 하한 48.6을 넘는 쌍은 여섯 쌍 중 「일반↔전설」(51.54) 하나뿐</b>이다. 카드 한 장만
        //     보고 "이건 영웅이다"를 색으로 맞히는 것은 <b>정상 시각에서도 안 된다</b>.
        //   · 이색각 3유형(1·2·3형)에서는 <b>변별</b>이 안 무너진다 — 색상각 하나 + 단조 명도라 명도축이
        //     남기 때문. <b>완전색맹에서 무너지던 2쌍이 R9 보정판에서 닫혔다.</b>
        ...
        //   브라스 강조 칩(#C8A15A)과 <b>영웅</b> 리본(R9 <c>#DEC081</c>)은 ΔE <b>12.86</b>이라 변별은 되지만
        ...
        private static readonly Color[] _rarityRamp =
        {
            ...
            RarityHex(0xBCAC8B),   // 희귀   — 7.40:1
            RarityHex(0xDEC081),   // 영웅   — 9.41:1   ★ R9 보정판(구 #DBBD7F, 이동 ΔE 1.15)
            RarityHex(0xFFD375),   // 전설   — 11.66:1  ★ R9 보정판(구 #F9CB70, 이동 ΔE 3.02)
        };
        ...
        /// <see cref="MinNonTextContrast"/> 미달이다. <b>이 값이 두 방향의 최선</b>이다 —
        /// 채움 최악 <b>3.59</b>(하한 3.0 통과) / 카드 바탕 <b>1.58</b>.</para>
        ///
        /// <para>★★ <b>2026-09-03 정정 — 여기 「이 값만 양쪽이 선다」라고 적혀 있었고 그게 거짓이었다.</b>
        /// 카드 바탕 쪽 1.58은 <see cref="MinNonTextContrast"/> 3.0에 <b>미달</b>이다. 그런데
        /// 값을 바꿔 고칠 수 있는 결함이 아니다 — design-art가 부등식을 풀었고 <b>양쪽이 동시에 서는 색은
        /// 존재하지 않는다</b>(채움과 3.0을 지키려면 L ≤ 0.0703, 카드 바탕과 3.0을 지키려면 L ≥ 0.1406 —
        /// <b>공집합</b>). 트랙은 「보이는 구획선」이 아니라 <b>「홈」</b>으로 설계된 것이고, 지는 하한은
        /// 채움↔트랙 3.59 하나다. <b>틀린 것은 코드가 아니라 이 문장이었다. 트랙 색을 더 찾지 마라.</b>
        /// 그리고 트랙이 안 보여도 <b>총 폭</b>은 사라지지 않는다 — 채워진 칸들이 자기 간격으로 이미
        /// 눈금을 만든다(리본이 카드 폭 전체가 아니라 썸네일과 같은 폭에 정렬돼 있는 것이 그 보조 단서다).</para></summary>
        ...
        /// <summary>
        /// 등급이 카드 테두리의 <b>「기본」 자리를 승계</b>할 때 쓰는 색 — <see cref="RarityColor"/>의
        /// <b>같은 RGB에 α만 얹은</b> 것이다. <b>신규 hex 0개</b>.
        ///
        /// <para>★ <b>왜 「다섯 번째 상태」가 아닌가</b>(UI_SURFACE_SPEC §15.2): 카드 테두리에 사는
        /// 넷은 선택 &gt; 호버 &gt; 착용 중 &gt; <b>기본</b>인데 <b>「기본」은 의미가 아니라 「아무 상태도
        /// 아님」</b>이다. 실제 의미는 셋뿐이고 그 셋은 전부 <b>한 번에 한두 장</b>에만 붙는다 —
        /// 한 섹션 6장 중 통상 4~5장이 항상 「기본」이다. 등급은 <b>비어 있던 자리를 채운다</b>.
        /// <b>우선순위는 한 글자도 바뀌지 않는다</b>: 상태가 있으면 상태가 이긴다.</para>
        ///
        /// <para>★ <b>α는 왜 이 값인가</b>(§15.6-a 검산 1): 인접 등급이 카드면 위에서 변별 하한
        /// ΔE 7.8을 유지하는 <b>α 하한이 0.4640</b>이다(이분법). 채택값에서 인접 ΔE는
        /// <b>9.06 / 11.13 / 11.01</b>이고 하한 대비 여유 +16.2%다. α0.45는 7.59로 <b>미달</b>이므로
        /// 0.5 언저리로 내리지 마라. 반대로 올리면 24장 격자가 시끄러워진다 —
        /// <b>소음 이의가 나오면 되돌릴 손잡이는 이 상수 하나</b>다.</para>
        ///
        /// <para>★ <b>회색조에서는 이 색이 등급을 말하지 못한다.</b> α를 1.00까지 올려도 인접 최소
        /// ΔE 7.70으로 하한 미달이다(§15.8). 그 조건에서 사는 축은 <b>리본의 칸 수</b>뿐이므로
        /// <b>테두리가 생겼다고 리본을 지우지 마라</b> — 접근성 축이 통째로 사라진다.</para>
        ///
        /// <para>범위 밖 등급은 <see cref="RarityColor"/>가 이미 로그를 찍고 끝 색으로 물린다 —
        /// 같은 함수를 타므로 테두리도 자동으로 같은 색을 받는다(화면이 두 가지를 말하지 않는다).</para>
        /// </summary>
        public static Color RarityBorder(StickMate.Core.ItemRarity rarity)
        {
            Color c = RarityColor(rarity);
            c.a = RarityBorderAlpha;
            return c;
        }

        /// <summary>등급 테두리의 α. <b>등급마다 다르게 두지 않는다</b> — 알파가 갈리면 그 순간
        /// 밝기가 등급을 말하기 시작해서 램프의 휘도 단조와 두 벌이 된다.
        /// <para>하한 0.4640(§15.6-a 검산 1)보다 위여야 한다. 이 상수는 <c>RarityBorderSuccessionTests</c>가
        /// <b>값을 베끼지 않고</b> 네 등급의 α가 서로 같은지와 하한을 넘는지로 잠근다.</para></summary>
        private const float RarityBorderAlpha = 0.55f;
        ...
        // ---- 리본의 기하 토큰 (PALETTE_SPEC.md §12-3 · §12-4 / UX_FLOW.md 50-4-1) ----
        //
        // 색이 아니라 <b>개수</b>가 주 채널이므로, 칸의 개수와 칸 사이의 틈은 색과 같은 급의 토큰이다.
        // 카드도 보관함도 이 셋(칸 수 · 두께 · 틈)에서 자기 칸 폭을 <b>파생</b>시킨다 —
        // 폭을 각자 적으면 창 폭이 바뀔 때 한쪽만 따라온다(이 저장소가 폭 1042에서 실제로 당한 사고).

        /// <summary>등급 리본의 <b>총 칸 수</b> = 등급 단 수. <b>숫자를 적지 않는다</b> —
        /// 램프에 단이 하나 늘면 칸도 하나 늘어야 하고, 두 곳에 적으면 반드시 한쪽만 늘어난다.
        /// <para>이 값이 램프 길이에서 나오는 것이 중요하다: <see cref="RarityColor"/>가 범위 밖 등급을
        /// 끝 색으로 물리는 순간(<c>WarnUnknownRarity</c>) 칸 수도 같은 끝으로 물러나야 화면에서
        /// 「색과 칸 수가 서로 다른 등급을 말하는」 상태가 생기지 않는다.</para></summary>
        public static int RarityCellCount => _rarityRamp.Length;

        /// <summary>이 등급이 <b>몇 칸을 채우는가</b> — 일반 1 / 희귀 2 / 영웅 3 / 전설 4.
        ///
        /// <para>★ 이것이 등급의 <b>주 채널</b>이다(PALETTE_SPEC §12-4). 색은 보조이고 낱말이 확정을 맡는다.
        /// 근거는 실측이다: 브라스 램프는 인접 쌍 ΔE 15.16~18.48로 <b>변별</b>은 되지만 <b>식별</b> 하한
        /// 48.6을 넘는 쌍이 「일반↔전설」 하나뿐이라, <b>카드 한 장만 보고 색으로 등급을 맞히는 것은
        /// 정상 시각에서도 안 된다</b>. 개수는 색맹·흑백·저해상도 어느 축에도 안 걸린다.</para>
        ///
        /// <para><b>화면은 이 함수만 부른다.</b> <c>(int)rarity + 1</c>을 호출부가 직접 적으면 그 산술이
        /// 카드·보관함·(훗날) 상점에 세 벌로 흩어지고, 등급 단이 늘어난 날 한 벌만 고쳐진다.</para></summary>
        public static int RarityFilledCells(StickMate.Core.ItemRarity rarity)
        {
            int i = (int)rarity;
            if (i >= 0 && i < _rarityRamp.Length) return i + 1;

            WarnUnknownRarity(i);
            return i < 0 ? 1 : _rarityRamp.Length;
        }

        /// <summary>
        /// 등급 리본의 <b>두께</b>(pt). 카드도 보관함도 같은 값을 쓴다 — 같은 것은 같게 보여야 한다.
        ///
        /// <para>★★ <b>왜 4인가 — Windows 배율에서 잔차가 0인 가장 얇은 값이다.</b>
        /// <c>docs/UX_HANDOFF_REVIEW_BRASS_ARCHIVE.md</c> §8 C-7이 남긴 유일한 미측정 축이
        /// *"리본 높이 <b>2px</b>가 Windows 125/150/175%에서 사라지지 않는가"*였다. 2pt는 ×1.25에서
        /// 2.5px, ×1.75에서 3.5px로 <b>반 픽셀에 걸린다</b>. 배율이 각각 5/4 · 3/2 · 7/4이므로
        /// <b>4의 배수</b>만 세 배율 전부에서 정수 픽셀이 된다:
        /// <code>
        ///   2pt -> 2.5 / 3.0 / 3.5 px      (×1.25 / ×1.5 / ×1.75)   ← 두 곳에서 반 픽셀
        ///   3pt -> 3.75 / 4.5 / 5.25 px                              ← 세 곳 전부 어긋난다
        ///   4pt -> 5 / 6 / 7 px                                      ← ★ 잔차 0
        /// </code>
        /// 이것은 이 파일이 글자 크기를 전부 <b>짝수 pt</b>로 못 박은 것과 같은 병에 대한 같은 처방이다
        /// (그쪽은 ×1.5만 보면 됐으므로 2의 배수로 충분했다). PALETTE_SPEC §12-4의 초안값 3pt에서
        /// <b>+1pt</b> 한 것이고, 그 절의 세로 검산(카드 상단 여백 8pt)은 여전히 성립한다 —
        /// 위 2 + 리본 4 + 아래 2 = 8.</para>
        ///
        /// <para>★ <b>그래도 Windows 실기 미확인이다.</b> 이 머신에서 잴 수 없는 것이 두 가지 남는다:
        /// (가) 리본의 <b>y 위치</b>(카드 상단 여백 2pt)는 ×1.25/×1.75에서 반 픽셀에 걸린다 —
        /// 두께는 정수인데 <b>시작점</b>이 밀리면 위아래 경계 한 줄이 흐려진다.
        /// (나) 9-슬라이스 둥근 스프라이트(<see cref="RadiusDot"/>)를 4pt 높이에 눌러 담을 때의
        /// 코너 처리. <b>둘 다 실기 캡처 항목이고, 캡처 전에는 「확인됨」이라고 적지 마라.</b>
        /// macOS는 배율이 ×1/×2뿐이라 구조적으로 안전하다(4 -> 4 / 8px).</para>
        /// </summary>
        public const float RarityRibbonHeight = 4f;

        /// <summary>리본 칸 사이의 틈(pt). 이 틈으로 <see cref="RarityTrack"/>이 비쳐 칸이 갈라져 보인다.
        /// <para>PALETTE_SPEC §12-4 기하표의 <b>권고 조합</b>(카드 인셋 11 · 틈 2)에서 온 값이다.
        /// 그 조합에서 카드 칸 폭이 33.25pt가 되고, 이는 §12-3이 잰 하한(칸+틈 총 15pt)의 9.3배다.</para></summary>
        public const float RarityCellGap = 2f;

        /// <summary>총 폭 <paramref name="trackWidth"/>를 <see cref="RarityCellCount"/>칸으로 나눈 <b>한 칸 폭</b>.
        /// 카드(139pt -> 33.25)와 보관함(46pt -> 10.0)이 <b>같은 식</b>을 탄다 — 두 곳이 각자 나누면
        /// 칸 수가 늘어난 날 한쪽만 어긋난다.</summary>
        public static float RarityCellWidth(float trackWidth)
            => (trackWidth - RarityCellGap * (RarityCellCount - 1)) / RarityCellCount;
        ...
        //   두 면의 휘도는 0.2355 / 0.2351(브라스 전환 후 재실측)이고 P0-4가 세운 상한(흰 채움과
        //   카드 바탕의 중간)의 <b>절반 이하</b>다. 그 가드(InfoWindowSurfaceRegressionTests)는 그대로 통과한다.
        ...
        /// <summary>카드 하단 [해제](착용 중) 칩의 면. 같은 4.48:1 등급이되 <b>강조색</b>이라
        /// "지금 걸치고 있는 것"이라는 신호를 면 자체가 낸다.
        /// <para>★ 2026-09-03 강조색이 브라스가 되면서 이 <b>파생값</b>도 <c>#5087CC -> #9E814D</c>로
        /// 따라 움직였다. <b>규칙이 만들기 때문에 호출부는 아무것도 안 고쳤다</b> — 이 파일이 존재하는
        /// 이유가 그것이다. 대비 등급은 그대로다(면 4.48:1 / 잉크 5.18:1, 실측 재확인).
        /// [착용] 면과의 거리는 ΔE 38.82 -> <b>34.88</b>로 변별 하한 7.8의 4.5배를 유지한다 —
        /// 두 상태를 <b>밝기가 아니라 색상</b>으로 가른다는 설계가 살아 있다.</para></summary>
        ...
        //   면 <b>#5DA1F5</b>(<b>당시</b> 강조색, 활성과 똑같다) + 글자 <b>#AEB4BF</b> = <b>1.28 : 1</b>.
        //   (강조색은 2026-09-03 브라스로 바뀌었다. <b>결함의 성질은 색이 아니라 「비활성인데 면이
        //   활성과 같다」였으므로 이 절은 그대로 유효하다</b> — 처방도 값이 아니라 InkOnSurface다.)
