...
namespace StickMate.Core
{
    ...
    public sealed class AccessoryDefSO : ScriptableObject
    {
        ...
        ///
        /// <para>★★ <b>몸에 붙는 그림은 이 칸을 여전히 보지 않는다 — 다만 이유가 바뀌었다.</b>
        /// 2026-09-08에는 여기에 *"비트맵을 몸에 붙이면 캐릭터가 부위별로 다른 화법(선화 몸 + 사진 같은
        /// 모자)이 되어 스틱메이트라는 그림 자체가 갈라진다 — 리더 판단으로 <b>범위 밖</b>이다.
        /// 소비자를 늘리기 전에 그 판단부터 다시 받아라"*라고 적혀 있었다.
        /// <b>그 «다시 받음»이 2026-09-09에 일어났고, 사용자가 판단을 뒤집었다</b>
        /// (DLC 착용 벡터를 R2·R3·R4 세 번 재작업하고 *"전체적으로 전부 다 마음에 안듬"* →
        /// *"지금 StickMate도 거의 뼈대로 되어 있지 않냐, 거기에 부착하면 되지 않냐"* →
        /// 스타일 샘플을 보고 *"전체 기본 42종까지 이 스타일로 확장"* 확정).
        /// 판정과 근거는 <c>docs/GAME_ARCHITECTURE_REVIEW.md</c> §19 전문에 있다.</para>
        ///
        /// <para>그래서 <b>착용 비트맵은 이 칸이 아니라 자기 칸을 갖는다</b>:
        /// <see cref="wornSpriteOverride"/> · <see cref="wornSpriteBackOverride"/> ·
        /// <see cref="wornSpriteRectInR"/> · <see cref="wornSpriteInkBoxInR"/>.
        /// 칸을 나눠 둔 이유는 <b>같은 그림이 두 표면에서 4.8배 다른 크기로 그려지기</b> 때문이다
        /// (카드 72pt vs 몸 ≈15pt, §19-2 실측). 한 칸을 두 표면이 공유하면 한쪽에 맞춘 그림이
        /// 다른 쪽에서 무너지고, 그 무너짐은 «카드가 좀 달라 보인다»로만 나타나 아무도 신고하지 않는다.
        /// <b>이 문단을 근거로 착용 비트맵을 되돌리지 마라</b> — 되돌리는 쪽이 낡았다.</para>
        ///
        ...
        [Tooltip("카드/상점 진열 전용 비트맵 아이콘. 비우면(기본) 벡터 아이콘을 그대로 쓴다. " +
                 "★ 몸에 붙는 그림은 이 칸이 아니라 wornSpriteOverride 를 본다 — 카드 표면 전용이다.")]
        public Sprite cardIconOverride;

        // ============================================================================
        // ★★ 몸에 붙는 비트맵 (2026-09-09, 사용자 확정 «전체 기본 42종까지 이 스타일로 확장»)
        //     설계 정본: docs/GAME_ARCHITECTURE_REVIEW.md §19-6-a
        // ============================================================================
        // 네 칸 전부 <b>비어 있는 것이 기본이고, 비어 있으면 지금까지의 벡터 경로가 한 줄도 다르지 않게
        // 그대로 돈다</b>. 출하 42종은 전부 비어 있다 — 그래서 하위 호환이 「기본값이 곧 옛 동작」이라는
        // 구조적 사실로 보장되고, 저장 스키마(CharacterSaveStore.CurrentVersion)는 한 비트도 안 움직인다
        // (저장 파일은 «무엇을 입었는가»를 적지 «어떻게 그리는가»를 적지 않는다 — §19-6-e).
        //
        // ★ 「이전 것 보존」의 정의(§19-6-c): 벡터 좌표를 아카이브로 옮기거나 #if 로 감싸지 <b>않는다</b>.
        //   그 자리에 두고, 컴파일하고, 테스트를 계속 돌린다. 스프라이트 칸 하나를 비우면 그날로 돌아온다.
        //   갈라지는 지점은 도형 빌더가 아니라 <b>렌더러</b>다(CharacterAccessoryRenderer.Rebuild) —
        //   AccessoryShapeBuilder.Append 는 카드·초상도 부르므로 거기서 갈라면 그 두 표면이 조용히
        //   벡터를 잃는다.

        /// <summary>
        /// ★ <b>몸에 붙는 비트맵(앞층)</b>. 비면(기본) 이 아이템은 지금까지처럼 벡터로 그려진다.
        /// 값이 있으면 <b>그 아이템의 벡터 조각을 한 개도 굽지 않고</b> 스프라이트 한 장으로 대체한다.
        ///
        /// <para>층은 슬롯이 정한다(모자 10 / 눈 8 / 목 7 / 어깨 −1 / 머리 6) — 데이터에 안 넣는 이유는
        /// <b>자리끼리의 겹침 규칙</b>이라 팩 작성자가 정하면 남의 아이템이 내 모자 위로 올라오기
        /// 때문이다(<c>AccessoryShapeBuilder.TryWornAssetSlotOrder</c>가 그 표의 유일한 주인이다).</para>
        ///
        /// <para>값이 실리는 자리는 병렬 표(<c>ItemCatalog.WornSprite</c>) 하나다 —
        /// <see cref="cardIconOverride"/>와 <b>같은 이유</b>로 <see cref="ItemCatalogEntry"/>에 얹지 않는다.</para>
        /// </summary>
        [Header("몸에 붙는 비트맵 (비우면 벡터 그대로 — 기본값이 곧 옛 동작)")]
        [Tooltip("착용 표면 전용 비트맵(앞층). 비우면(기본) 벡터 wornShapes/코드 도형을 그대로 쓴다. " +
                 "★ 카드는 이 칸을 보지 않는다 — 카드는 cardIconOverride 다.")]
        public Sprite wornSpriteOverride;

        /// <summary>
        /// ★ <b>몸통·머리 <u>뒤</u>로 넘어가는 부분</b>의 비트맵. 비면 뒤층을 아예 그리지 않는다.
        ///
        /// <para><b>왜 칸이 두 개인가</b>(§19-7-c 전수 조사): 인계본 12종 중 <b>6종이 서로 다른
        /// <c>sortingOrder</c> 두 개</b>를 쓴다 — 야구모자·중절모·왕관은 (10, −1), 짧은망토·긴망토·배낭은
        /// (−1, 3). 한 장으로 받으면 <b>왕관 뒷테가 머리 앞에 그려진다</b>. 저작 지시서에
        /// «머리/몸통 뒤로 가는 부분은 별도 레이어로 내보낼 것»이 들어가야 하는 이유가 이것이다.</para>
        ///
        /// <para>앞층(<see cref="wornSpriteOverride"/>)이 비어 있으면 이 칸만 있어도 <b>아무 일도
        /// 일어나지 않는다</b> — 앞장이 주인이다. 「뒤만 있고 앞이 없는」 아이템은 존재하지 않으므로
        /// 그 조합은 오타로 보고 조용히 벡터로 되돌린다(빈칸이 곧 롤백이라는 계약을 깨지 않기 위해서).</para>
        /// </summary>
        [Tooltip("착용 비트맵 중 몸 뒤로 가는 부분(왕관 뒷테, 망토 뒤판 등). 비우면 뒤층을 안 그린다. " +
                 "앞층(wornSpriteOverride)이 비어 있으면 이 칸도 무시된다.")]
        public Sprite wornSpriteBackOverride;

        /// <summary>
        /// ★ 이 비트맵을 <b>몸의 어디에 얼마만큼</b> 놓는가. 단위는 전부 <b>머리 반경 R의 배수</b>다
        /// (월드 유닛 절대값이 아니다 — 그래야 캐릭터 배율 다이얼 0.35~1.00을 자동으로 따라간다).
        ///
        /// <list type="bullet">
        ///   <item><c>x</c>,<c>y</c> — <b>앵커 기준 중심 오프셋</b>. 앵커는 슬롯에서 유도한다:
        ///     HEAD/EYES/HAIR = 머리 중심, NECK/SHOULDERS = 어깨선. 데이터에 안 넣는 이유는
        ///     <see cref="wornSpriteOverride"/>의 층과 같다 — 자리끼리의 규칙이다.</item>
        ///   <item><c>width</c>,<c>height</c> — 그림 <b>캔버스 전체</b>의 크기(R 배수). 잉크의 크기가
        ///     아니다. 캔버스가 곧 좌표계라, 잉크를 캔버스 안에서 옮기면 이 값은 그대로여야 한다.</item>
        /// </list>
        ///
        /// <para><c>width</c>/<c>height</c>가 0 이하면(= 안 적음) 렌더러가 <b>머리 지름(2 R) 폭에
        /// 그림 비율 유지</b>로 되메운다 — 「모른다」를 「안 그린다」로 바꾸지 않는다.</para>
        /// </summary>
        [Tooltip("착용 비트맵 배치 사각형. 단위는 머리 반경 R의 배수. " +
                 "x,y = 앵커(머리 중심 또는 어깨선) 기준 중심 오프셋 / w,h = 캔버스 전체 크기. " +
                 "w 또는 h 가 0이면 머리 지름(2R) 폭으로 되메운다.")]
        public Rect wornSpriteRectInR;

        /// <summary>
        /// ★★ <b>알파 타이트 박스</b> — (minX, minY, maxX, maxY), 단위는 <see cref="wornSpriteRectInR"/>와
        /// <b>같은 좌표계</b>(앵커 기준, R 배수). <b>손으로 적는 값이 아니다</b>:
        /// 에디터 도구(<c>Assets/Editor/WornSpriteImport.cs</c>)가 PNG의 알파를 훑어 굽는다.
        ///
        /// <para><b>왜 필요한가</b>(§19-8-3): <c>Renderer.bounds</c>를 그대로 읽는 소비자가 둘 있다 —
        /// <c>StickmanAgent</c>의 시각 반폭(화면 여백 클램프)과
        /// <c>CharacterAccessoryRenderer.TryGetLowestInkWorldY</c>(GETUP 바닥 클리어런스).
        /// 스프라이트의 bounds 는 <b>투명 여백까지 포함한 캔버스 사각형 전체</b>라, 모자가 캔버스의
        /// 60%만 차지하면 반폭이 1.67배 과대가 되어 캐릭터가 화면 끝에서 헛되이 밀려나고,
        /// 바닥 클리어런스는 투명 여백만큼 캐릭터를 <b>공중에 띄운다</b>.</para>
        ///
        /// <para><c>maxX ≤ minX</c> 또는 <c>maxY ≤ minY</c>면(= 안 구움) 렌더러는
        /// <see cref="wornSpriteRectInR"/> 전체를 잉크로 본다 — 안전한 방향(여유를 더 주는 쪽)으로 틀린다.</para>
        /// </summary>
        [Tooltip("알파 타이트 박스 (minX, minY, maxX, maxY) — 단위는 wornSpriteRectInR 와 같은 R 배수. " +
                 "★ 손으로 적지 마라. StickMate/착용 비트맵 메뉴가 PNG 알파에서 구워 넣는다.")]
        public Vector4 wornSpriteInkBoxInR;
