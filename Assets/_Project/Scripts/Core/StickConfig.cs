...
namespace StickMate.Core
{
    ...
    public sealed class StickConfig : ScriptableObject
    {
        ...
        [Tooltip("밧줄 등반으로 오를 수 있는 최대 높이 — 신장 배수(H). stepUpMaxHeights(파쿠르 상한)에서 " +
                 ...
                 "두 독립된 값이 교차 확인해 준다.\n" +
                 "★★★ 2026-09-08 — <b>이 값은 더 이상 주 상한이 아니다.</b> 주 상한은 화면 비례 " +
                 "(ropeClimbMaxScreenFraction)로 옮겼고, 이 값은 <b>등반 소요시간 천장</b>으로 역할이 " +
                 "바뀌었다. 사용자 지시: '6.6배는 너무 작은거 같아 화면자체가 캐릭터보다 많이 큰데 " +
                 "6.6배면 너무 작은거 같은데 화면 해상도에 따라 계산해서 나눠야할거같은데'. 실측이 그 " +
                 "지적을 뒷받침했다 — 배포 기본 배율에서 화면은 13.0H이고 발밑(Dock)에서 화면 상단까지 " +
                 "도달범위가 12.1H인데, 6.6H는 그 <b>55%</b>밖에 안 덮었다. 화면이 커질수록(더 많은 H를 " +
                 "담을수록) 이 고정 배수의 커버율은 더 떨어진다 — 해상도 의존이 잘못된 방향으로 걸려 있었다.\n" +
                 "★ 13.6f 근거: 새 속도 1.36 H/s에서 <b>10.0초</b>가 되는 높이다(13.6 / 1.36 = 10.0). " +
                 "즉 '몇 H까지 오를 수 있나'가 아니라 '한 번의 등반이 최대 몇 초까지 이어져도 되나'를 " +
                 "정하는 값이다. 배포 기준 화면에서는 화면 비례 쪽(10.9H)이 먼저 걸려 이 값이 binding " +
                 "되지 않는다 — 훨씬 큰 모니터에서만 발동하는 안전핀이다. 속도를 바꾸면 이 값도 함께 " +
                 "바꿔야 의미가 유지된다(둘의 몫이 초 단위 천장이다).")]
        public float ropeClimbMaxHeights = 13.6f;

        [Tooltip("밧줄 등반 높이 상한 — <b>발밑에서 화면 클램프 상단까지의 도달범위</b>에 대한 비율. " +
                 "2026-09-08 사용자 지시('화면 해상도에 따라 계산해서 나눠야할거같은데')로 신설했고, " +
                 "ropeClimbMaxHeights(신장 배수 고정값)를 대신해 <b>주 상한</b>이 된다.\n" +
                 "★ 왜 신장 배수가 아니라 화면 비례인가: 신장 배수는 화면이 커질수록 커버율이 떨어진다. " +
                 "실측(배포 기본 배율) — 화면 13.0H · 도달범위 12.1H · 옛 상한 6.6H = 커버율 55%. " +
                 "같은 배율로 더 큰 모니터에 올리면 도달범위만 늘고 상한은 그대로라 커버율이 더 낮아진다. " +
                 "화면 비례로 잡으면 어느 해상도에서나 같은 비율이 유지된다.\n" +
                 "★ 0.90f 근거: 도달범위의 90%. 남긴 10%는 화면 최상단에 딱 붙은 창을 목표로 삼아 " +
                 "매달린 채 화면 밖으로 밀려나는 것을 막는 여유다(화면 클램프가 이미 상단을 자르지만, " +
                 "그 경계에 정확히 닿는 목표는 프레임 간 판정이 갈릴 수 있다 — 이 저장소가 겪은 " +
                 "'진입 직후 목표 벽 소실' 사고와 같은 형태). 배포 기준 화면에서 12.1H × 0.90 = 10.9H이고, " +
                 "그때까지 탈락하던 오른쪽 Finder 창(8.17H)이 이 상한으로 비로소 후보가 된다.\n" +
                 "★ 실질 상한은 min(도달범위 × 이 값, ropeClimbMaxHeights × H)이다 — 앞은 '화면 어디까지', " +
                 "뒤는 '몇 초까지'. 둘 중 먼저 걸리는 쪽이 이긴다.")]
        [Range(0f, 1f)]
        public float ropeClimbMaxScreenFraction = 0.90f;

        [Tooltip("★★ 목표 지향 등반 주기(초) — 2026-09-08 사용자 지시로 신설.\n" +
                 "사용자 원문: '그냥 바닥에 캐릭터가 위치할때 전체 화면을 한번 스캔한후 창들의 위치 및 " +
                 "창 상단의 좌표만 인식하면 확률로 밧줄 던지면 되는거 아니야?' / '목표창 아래로 가서 던짐'.\n" +
                 "★ 왜 신설했나: 기존 등반(밧줄·파쿠르)은 <b>발판 경계에 도달했을 때만</b> 평가됐고 " +
                 "화면 끝은 제외됐다. macOS Dock은 화면 가운데 띠라 내부 경계가 2개 생겨 동작했지만, " +
                 "Windows 작업표시줄은 <b>화면 전폭</b>이라 내부 경계가 0개이고 유일한 경계인 좌우 끝은 " +
                 "화면 끝이라 제외된다 — 즉 Windows에서는 <b>체인 전체가 한 번도 돌지 않았다</b>. " +
                 "사용자 신고 3건('윈도우에서는 밧줄 동작안함' x2, '윈도우에서는 파쿠르도 동작안하는데')이 " +
                 "전부 이 뿌리 하나였다. 확률·높이 상한을 두 라운드에 걸쳐 고쳤는데도 효과가 없었던 이유다.\n" +
                 "★ 이 주기마다 화면 전체 발판을 한 번 훑어 대역에 드는 벽을 고르고, ropeClimbChance로 " +
                 "추첨한다. 이기면 그 벽의 «내 쪽 세로 모서리» 앞까지 걸어가 던진다.\n" +
                 "★ 100f 근거: 체감 주기 = 이 값 / ropeClimbChance(0.85) ≈ 118초로, 사용자가 지정한 " +
                 "'2분에 한번정도'와 맞는다. 기존 경계 기반 경로는 그대로 남아 있으므로(한 줄도 안 " +
                 "건드렸다) macOS에서는 두 경로가 합쳐져 조금 더 잦아진다 — 그쪽 실측 120초/회는 " +
                 "경계 경로만의 값이었다. 너무 잦으면 이 값을 올릴 것(확률보다 이쪽이 직관적이다).")]
        [Min(1f)]
        public float climbSeekIntervalSeconds = 100f;
        ...
        [Tooltip("밧줄을 타고 오르는 속도 — 초당 신장 배수(H/초). WalkState.ResolveWalkSpeed가 배율에 " +
                 ...
                 "★★★ 2026-09-08 — 0.88 → 1.36으로 상향(사용자 승인). 같은 라운드에서 높이 상한을 " +
                 "화면 비례(ropeClimbMaxScreenFraction=0.90)로 바꾸면서 오를 수 있는 벽이 6.6H → 10.9H로 " +
                 "높아졌는데, 옛 속도로는 그 등반이 <b>12.4초</b>가 된다. 그 동안 캐릭터는 다른 행동을 " +
                 "못 하므로 상주 앱에서는 너무 길다는 판단이다. 1.36 H/s면 10.9H가 <b>8.0초</b>다.\n" +
                 "★ 위 0.88 역산(파쿠르 경계 연속성)이 <b>깨진다</b>는 것을 명시해 둔다 — 이제 하한 " +
                 "경계에서 밧줄이 파쿠르보다 약 1.55배 빠르다(1.36/0.88). 그 연속성보다 '등반이 너무 " +
                 "오래 끌지 않는다'를 우선한 것이고, 되돌릴 근거가 생기면 <b>속도를 되돌리는 대신 " +
                 "ropeClimbMaxScreenFraction을 낮추는 쪽</b>을 먼저 검토할 것(그러면 연속성과 소요시간을 " +
                 "동시에 살릴 수 있다). ropeClimbMaxHeights(소요시간 천장)도 이 값과 짝이라 함께 바꾼다.")]
        public float ropeClimbSpeedHeightsPerSecond = 1.36f;
        ...
        [Tooltip("자율 배회 중 밧줄 등반을 시도할 확률 — stepUpChance와 별도로 추첨한다(재사용하면 " +
                 ...
                 "확인했다(사용자 신고 '영원히 안될거 같음'이 문자 그대로 맞았다).\n" +
                 "★★★ 2026-09-08(3차) — 0.20 → 0.60으로 재상향. <b>사용자 지시</b>: '창이 저렇게 올라갈수 " +
                 "있는 조건이면 2분에 한번정도의 빈도로는 올라가야함'. 2차의 0.20은 몬테카를로 시뮬레이션 " +
                 "(rope_trigger_sim.py, §9-3, 6000표본)이 근거였는데 그 시뮬레이션은 §9-3 본문이 스스로 " +
                 "밝혀 둔 대로 <b>'벽은 한쪽 경계에 고정 배치… 지형 문제는 해결됐다고 가정한 이상적 " +
                 "케이스'</b>였다. 실제 데스크톱은 그 전제를 만족하지 않는다 — 리더 실기 측정(2026-09-08, " +
                 "macOS 배포 빌드, STICKMATE_QA_ROPE_CLIMB_CHANCE=1.0으로 <b>확률을 제거하고 지형만</b> 잰 " +
                 "결과) <b>벽 탐색 성공(=기회) 자체가 515초에 6회 = 평균 86초에 1회</b>였다. 즉 확률 1.0을 " +
                 "줘도 86초가 천장이고, 0.20을 곱하면 체감 주기가 7분대가 된다(시뮬레이션이 예측한 '3분 " +
                 "이내 94%'와 갈라진 지점은 트리거 체인이 아니라 <b>지형 가정</b>이었다).\n" +
                 "0.85의 근거는 <b>2단 실측</b>이다(모델 외삽이 아니다 — 이 값을 시뮬레이션으로 정한 것이 " +
                 "2차의 실패였다). ① c=1.00: 515초/6회 = 86초에 1회. ② c=0.60: 1201초/6회 = 200초에 1회. " +
                 "②에서 단순 반비례로 외삽하면 목표 120초에는 c=1.00이 필요하지만, 그건 추첨을 없애는 " +
                 "것이라 창이 빽빽한 데스크톱에서 경계마다 밧줄을 던지게 된다. c=0.85는 외삽 141초로 " +
                 "'2분 정도'에 들면서 15%는 그냥 지나쳐 그 도배를 막는다. 값을 stepUpChance와 같은 0.85로 " +
                 "맞춘 것은 우연이 아니라 <b>같은 뜻</b>이어서다 — '기회가 오면 대체로 잡지만 늘 그렇지는 " +
                 "않다'.\n" +
                 "★ 측정 정밀도의 한계를 숨기지 않는다: 6회 표본의 95% 신뢰구간은 ①이 48~430초, ②가 " +
                 "111~1002초로 넓다. 두 점으로 T(c)=t_e/c+t_c를 연립하면 t_c가 음수로 떨어져 모델이 " +
                 "성립하지 않는다(측정 사이에 창 배치가 달라졌을 가능성이 크다). 그래서 이 값은 " +
                 "'정밀하게 푼 해'가 아니라 <b>목표 구간에 드는 가장 방어 가능한 선택</b>이다. 더 조이려면 " +
                 "고정된 창 배치에서 30분 이상 재측정할 것.\n" +
                 "★ 이 값은 <b>유효한 로프 대역 벽이 도달 범위에 있을 때</b>의 빈도를 맞춘 것이다(사용자 " +
                 "지시의 '~조건이면'이 그 전제다). 창 배치에 그런 벽이 하나도 없으면 이 값이 얼마든 " +
                 "발동은 0이다 — 그건 확률이 아니라 지형 문제이고, 대역 상한은 ropeClimbMaxHeights가 쥔다.\n" +
                 "★ 되돌릴 근거가 생기면 이 값을 0으로 되돌리기보다 STICKMATE_QA_ROPE_CLIMB_CHANCE " +
                 "환경변수(Core/RopeClimbQaOverride.cs)로 먼저 실기 재검증할 것 — 그 훅은 이 값과 " +
                 "독립적으로 언제나 우선한다.")]
        [Range(0f, 1f)]
        public float ropeClimbChance = 0.85f;

        [Tooltip("밧줄 등반 전용 벽 탐지 밴드 폭 배수 — parkourDetectionRadius의 배수(손 등반/매달리기/" +
                 "뛰어내리기가 쓰는 기존 배수 4와는 별도 필드, GroundSensor.AdjacentFootholdSearchRadiusMultiplier " +
                 "는 한 줄도 바뀌지 않는다). GroundSensor.TryFindClimbableWallOverlapping이 '내 경계 ± " +
                 "이 값'을 겹침 검사 밴드로 쓴다.\n" +
                 "★★ 2026-09-07 3차 신설·2차 보강(docs/DESIGN_ROPE_CLIMB_ARCHITECTURE.md §9→§10, 근본재설계) " +
                 "— 리더 실기 관측: ropeClimbChance를 0.20으로 올리고 §9-4-B 좁은 보강까지 적용한 뒤에도 " +
                 "실제 데스크톱(macOS)에서 자연 발동이 전혀 없었다. 원인은 확률이 아니라 지형이었다.\n" +
                 "★ 1차 진단은 부정확했다(기록을 위해 남긴다) — 처음엔 '벽이 도보 한 걸음보다 멀리 있어서 " +
                 "좁은 탐색이 못 찾는다'로 보고 단순히 탐색 폭(한쪽 방향 슬랙)만 넓혔으나, 이 라운드가 " +
                 "라이브 세션에서 재확인한 결과 실제 Finder 창(OS x 976~1512)은 Dock 오른쪽 경계(1259) " +
                 "보다 '멀리' 있던 게 아니라 그 경계를 '가로질러 양쪽에 걸쳐' 있었다(왼쪽 모서리는 경계 " +
                 "안쪽, 오른쪽 모서리는 바깥쪽). 선행 모서리 하나만 보는 기존 공식은 폭을 아무리 넓혀도 " +
                 "이 형태를 원리상 못 잡는다 — 그래서 GroundSensor.TryFindClimbableWallOverlapping(겹침 " +
                 "검사)으로 교체했다. 이 필드는 이제 '한쪽으로 얼마나 멀리'가 아니라 '겹침을 인정하는 " +
                 "밴드가 얼마나 넓은가'를 뜻한다.\n" +
                 "★★ 2026-09-07 4차 정정(debugger 소견 1) — 위 문장은 한때 '경계 <b>좌우로</b> 넓은 " +
                 "밴드'라고 적혀 있었으나 <b>지금은 앞쪽만 열린다</b>. 그 낡은 서술이 남아 있으면 다음 " +
                 "사람이 대칭 밴드를 전제로 이 값을 조정하게 된다 — 이 저장소가 반복해서 앓은 형태다.\n" +
                 "★ 왜 뒤쪽을 닫았나(Major 결함이었다): 대칭 밴드는 <b>진행 방향 등 뒤에 있는 높은 창</b>도 " +
                 "후보로 채택했다. 배포 기본값에서 밴드 폭은 parkourDetectionRadius(0.5) × 이 값(16) = " +
                 "8.0유닛 = 실측 환산 327pt이고, 1512pt 화면의 <b>21.7%가 통째로 '뒤쪽 후보 구역'</b>이었다. " +
                 "그러면 RopeClimbState.Enter()는 진행 방향으로 밧줄을 던지는데 목표 벽은 등 뒤인 그림이 된다.\n" +
                 "★ 지금 형태: 앞쪽으로는 예전처럼 이 값만큼 열고, 뒤쪽은 '내 몸과 목표 경계 중 더 뒤에 " +
                 "있는 쪽'에서 닫는다(GroundSensor.TryFindClimbableWallOverlapping의 rearLimitX). " +
                 "즉 <b>벽의 어느 부분이라도 내 앞에 걸쳐 있으면 여전히 후보</b>고, 끝까지 전부 뒤에 있는 " +
                 "벽만 떨어진다 — 그래서 아래 '경계를 가로질러 걸친 창'(이 필드가 생긴 이유)은 그대로 잡힌다.\n" +
                 "★ 왜 손 등반과 분리된 필드/메서드인가: '밧줄을 던진다'는 컨셉 자체가 손보다 훨씬 " +
                 "느슨하게 닿는다는 뜻이라, 겹침 검사로 바꾸는 것이 6-C가 금지한 '목표를 찾아 이동하는' " +
                 "행동을 새로 여는 것이 아니다 — 여전히 '지금 서 있는 경계'에서만 평가한다(EdgeProbeReachWorld " +
                 "게이트는 그대로). 손 등반/매달리기/뛰어내리기는 이 필드도 겹침 검사도 전혀 쓰지 않으므로 " +
                 "(StickmanBlackboard.TryFindRopeClimbWallWide 전용 소비) 파쿠르 전체 회귀 위험이 없다.\n" +
                 "16f 근거(개산): 기존 배수 4의 4배 — 정밀 유도가 아니라 '밧줄이 손보다 대략 네 배 " +
                 "느슨하게 닿는다'는 감각값이다. 겹침 검사 자체는 창이 경계를 가로지르기만 하면 이 값이 " +
                 "작아도 잡히므로(실측 확인), 이 값은 주로 '경계 양쪽 다 벗어난, 완전히 분리된 창'까지 " +
                 "얼마나 관대하게 볼지를 결정한다. 6-C의 '원거리 목표 탐색이 아니다' 원칙은 유지된다 — " +
                 "화면 폭 전체(≈25유닛)에 비하면 여전히 좁은 확장이다.")]
        public float ropeClimbAdjacentSearchRadiusMultiplier = 16f;
        ...
        // ============================================================================
        // ★★ 코스튬 × 집중 세션 (2026-09-07, docs/UX_MOTION_COSTUME_FOCUS.md 9절 +
        //     docs/DESIGN_COSTUME_FOCUS_ARCHITECTURE.md 6·8절)
        // ============================================================================
        // 코스튬 연출은 «몰입기»(3구간 중 가운데)에만 산다. 여기 있는 것은 «켜고 끄기»와
        // «등장/퇴장 길이»뿐이다.
        //
        // ★ <b>키포즈 각도 · 프롭 좌표 · 스텝레이트 · 소구간 듀티는 여기 없고 앞으로도 없다.</b>
        //   각도는 서로 정합성을 갖는 하나의 실루엣이고(위 관망 자세 절과 같은 판단),
        //   스텝레이트는 인스펙터에서 4로 바꾸는 순간 «15의 약수» 조건이 조용히 깨진다
        //   (3·5fps만 Still 등급 분주 4로 나누어떨어진다 — design-motion 2-3절 표).
        //   <b>바꿀 수 있게 만들면 안 되는 값이다.</b>

        [Tooltip("코스튬 소환 오브젝트(프롭)와 코스튬 전용 모션을 켠다. " +
                 "★ 마스터 스위치 — 끄면 두 층이 통째로 꺼지고 거동이 100% 이 기능 도입 이전과 같아진다 " +
                 "(이 저장소의 네거티브 컨트롤 규약). 프롭이 안 서면 코스튬 포즈도 자동으로 꺼진다 " +
                 "— 「없는 것을 향한 자세」가 만들어질 자리를 구조적으로 없앤 것이다(P-GHOST).")]
        public bool costumeFocusMotionEnabled = true;

        [Tooltip("몰입기 진입 시 프롭이 바닥에서 솟아오르는 시간(초). 이 구간에만 루트 localScale 1개를 " +
                 "매 프레임 쓰고, 끝나면 프롭 생애 동안 쓰기가 0이 된다(요구 「매 프레임 재드로우 금지」의 실체).")]
        public float costumeFocusPropIntroSeconds = 0.35f;

        [Tooltip("한계 구간 진입 또는 세션 종료 시 프롭이 사라지는 시간(초).")]
        public float costumeFocusPropOutroSeconds = 0.22f;

        [Tooltip("몰입기 소구간(진입/절정/이완) 듀티 사이클의 주기 상한(초). 하한 4.0초는 코드 상수다 " +
                 "— 한 소구간에 주기가 3번은 들어가야 「리듬」으로 읽히고, 8초를 넘으면 정지 구간이 길어져 " +
                 "「멈춘 캐릭터」로 되돌아간다. ★ 이 값을 쓰는 곳은 코스튬 포즈 층(P4)이다.")]
        public float costumeFocusDutyCycleMaxSeconds = 8f;

        [Tooltip("★ 몰입기 동안 Idle이 끝났을 때 걷기로 갈 확률. 기본 0 = 프롭 곁을 안 떠난다. " +
                 "프롭은 몰입기 진입 프레임 좌표에 고정되므로 캐릭터가 걸어가면 프롭만 덩그러니 남는다 " +
                 "— 「걷기 0」과 「프롭 고정」은 한 묶음이다.\n" +
                 "★ 되돌릴 여지로 남긴 값이다(리더 판정 2026-09-07): 이 저장소는 「걷기 듀티가 떨어지면 " +
                 "덜 살아있어 보인다」를 실측한 전력이 있고, 집중모드는 정의상 사용자가 책상 앞에 있는 " +
                 "시간이다. 실기 관측 + 페르소나 판정 뒤에 값만 바꿔 뒤집을 수 있다.\n" +
                 "★ 올릴 때 주의: 배회 사다리는 부채꼴 0 → 몰입기 → 자리비움 0.15 → 집중 0.40 → 평소 0.75 " +
                 "순의 단조 내림차순이라야 「더 조용한 쪽이 이긴다」가 성립한다. " +
                 "awayWanderWalkChance(0.15)보다 크게 잡으면 그 성질이 깨진다.")]
        public float costumeImmersionWalkChance = 0f;
