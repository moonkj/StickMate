...
namespace StickMate.Core
{
    ...
    public sealed class StickConfig : ScriptableObject
    {
        ...
        [Tooltip("★ 발판 경계에서 바깥쪽으로 내딛는 거리(월드 유닛). 두 곳에 **같은 값**이 쓰인다: " +
                 "(a) '내려앉을 발판이 있는가'를 물어볼 때의 탐침 x, (b) 실제로 발을 뗄 때 몸을 옮기는 x. " +
                 "둘이 같아야 '예상 착지 지점'과 '실제 낙하 시작 지점'이 일치해 착지 예측이 빗나갈 수 없다.\n" +
                 "0으로 두면 안 된다: 서 있는 몸은 발판 상단선에 정확히 스냅돼 있어서, 모서리를 넘지 않은 " +
                 "채 낙하로 전이하면 FallState의 스윕 교차 판정이 **방금 떠난 그 발판**을 위에서 관통한 " +
                 "것으로 보고 제자리에 도로 착지시킨다(2026-08-29 실측: 낙하높이 0.00유닛).")]
        public float hopDownProbeOutward = 0.2f;

        [Tooltip("뛰어내리기로 결정한 뒤, 모서리까지 남은 거리가 이 값(월드 유닛) 이하가 되어야 실제로 " +
                 "발을 뗀다. 경계 판정 거리(wanderEdgeStopDistance=0.3)에서 곧바로 떼면 그만큼을 " +
                 "순간이동으로 건너뛰게 되므로(위 hopDownProbeOutward의 몸 이동), 모서리 코앞까지 " +
                 "걸어가 그 순간이동 폭을 눈에 띄지 않을 만큼 줄이는 것이 이 값의 역할이다. " +
                 "너무 작게 잡으면 프레임당 이동거리보다 작아져 창을 건너뛸 수 있는데, 그때는 그냥 걸어서 " +
                 "모서리를 넘어가 자연 낙하하므로(결과는 같다) 안전 문제는 없다.")]
