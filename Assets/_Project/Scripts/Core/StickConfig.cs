...
namespace StickMate.Core
{
    ...
    public sealed class StickConfig : ScriptableObject
    {
        ...
        [Tooltip("밧줄 등반으로 오를 수 있는 최대 높이 — 신장 배수(H). stepUpMaxHeights(파쿠르 상한)에서 " +
                 ...
                 "★★★ 2026-09-08 (2차 정정) — 13.6f로 두었던 것이 <b>사용자 신고 4번째의 직접 원인</b>이었다. " +
                 "실기 로그(Windows, 4K, 캐릭터 배율 0.35x)에서 신장이 0.796유닛이라 화면이 <b>30H</b>였고, " +
                 "13.6H = 10.83유닛은 도달범위(23.2유닛)의 <b>47%</b>밖에 못 덮었다. 그 결과 화면의 모든 창이 " +
                 "탈락했다 — msedge 21.3 / Explorer 23.2 / Notepad 18.3 / Outlook 15.7유닛, 전부 10.83 초과. " +
                 "[밧줄진단] 로그가 «대역=[0.84 ~ 10.83]유닛 / 벽없음 1회»로 이것을 그대로 찍어 주었다.\n" +
                 "★ 무엇이 잘못이었나: 이 값은 «소요시간 천장»을 <b>H 배수로</b> 표현한 것이었는데, H는 " +
                 "사용자가 바꾸는 값이다(캐릭터 배율). 배율을 줄이면 화면이 더 많은 H를 담게 되어 같은 " +
                 "배수가 화면의 더 작은 조각이 된다 — <b>사용자가 처음 지적한 바로 그 병</b>을 내가 다른 " +
                 "이름으로 다시 만든 것이다.\n" +
                 "★ 지금 형태: 소요시간은 <b>속도를 높이에서 유도해</b> 묶는다" +
                 "(ropeClimbTargetDurationSeconds). 그래서 이 값은 더 이상 소요시간과 무관한 " +
                 "<b>순수 안전핀</b>이고, 60f는 «어떤 배율·해상도에서도 걸리지 않는다»를 뜻한다" +
                 "(0.35x/4K에서 60H = 47.8유닛 > 도달범위 23.2유닛). 실질 상한은 언제나 " +
                 "ropeClimbMaxScreenFraction 쪽이다. <b>이 값을 낮춰 «소요시간을 줄이려» 하지 마라</b> — " +
                 "그것이 이번 결함의 재발 경로다. 시간은 ropeClimbTargetDurationSeconds로 줄인다.")]
        public float ropeClimbMaxHeights = 60f;
        ...
        [Tooltip("★★ 밧줄 등반 <b>목표 소요시간</b>(초) — 2026-09-08 신설.\n" +
                 "높이가 아니라 <b>시간</b>으로 등반 길이를 묶는다. 실제 속도는 " +
                 "max(ropeClimbSpeedHeightsPerSecond x 신장, 오를높이 / 이 값)이다 — 즉 <b>짧은 벽은 " +
                 "기존 속도 그대로</b>(자연스러운 손맛 보존)이고, <b>높은 벽만 이 시간에 맞춰 빨라진다</b>.\n" +
                 "★ 왜 이 형태인가: 높이 상한을 H 배수로 잡으면 캐릭터 배율(사용자 설정)에 따라 커버율이 " +
                 "달라진다. 실기에서 배율 0.35x·4K일 때 화면이 30H나 되어 «몇 H까지»라는 상한이 화면의 " +
                 "절반도 못 덮었고, 그래서 화면의 모든 창이 후보에서 탈락했다(사용자 신고 4회). 시간으로 " +
                 "묶으면 배율·해상도가 무엇이든 «한 번의 등반은 대략 이만큼 걸린다»가 보존된다.\n" +
                 "★ 7f 근거: 상주 앱에서 캐릭터가 다른 행동을 못 하는 구간의 상한이다. 4초는 화면 끝까지 " +
                 "오르는 그림이 순간이동처럼 보이고, 10초를 넘으면 «저거 언제 끝나나»가 된다. 활쏘기 " +
                 "연출(3발, 약 6초)과 같은 자릿수에 두었다 — 이 앱에서 «하나의 사건»이 차지하는 길이다.")]
        [Min(0.5f)]
        public float ropeClimbTargetDurationSeconds = 7f;
