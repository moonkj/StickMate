...
namespace StickMate.Core
{
    ...
    /// 파괴되는 순간(씬 재로드/라이벌 복제/테스트 리그) 값이 조용히 사라지거나 두 벌이 된다.
    ///
    /// 이 클래스는 <b>언제 XP가 들어오는지 전혀 모른다</b>. 패시브 적립과 보너스 훅(격파 승리 / 라이벌
    /// 대결 승리 / 활쏘기 명중)은 전부 Interaction/CharacterProgressionDirector.cs의 책임이고, 저장/로드는
    ...
    /// (기본 100 * level^1.15). 패시브 적립은 분당 <c>progressionPassiveXpPerMinute</c>(기본 1.5)이므로
    /// 시간당 90XP다. 실측 환산:
    ///
    ///   Lv1→2  100XP    누적    100 →  1.1시간
    ///   Lv2→3  222XP    누적    322 →  3.6시간
    ///   Lv3→4  352XP    누적    674 →  7.5시간
    ///   Lv4→5  487XP    누적  1,161 → 12.9시간
    ///   Lv5→6  630XP    누적  1,790 → 19.9시간
    ///   Lv6→7  780XP    누적  2,571 → 28.6시간
    ///   Lv7→8  937XP    누적  3,508 → 39.0시간
    ///
    /// 리더가 준 목표("초반 레벨업이 1~3시간 안에, 갈수록 서서히 느려짐")를 만족하고, 장비 해제 레벨
    /// 2/4/6/8은 하루 8시간 사용 기준으로 각각 1일차/1일차/2.5일차/5일차에 열린다("며칠 안에 하나씩").
    /// 보너스 XP(격파/대결/명중)는 이 시간을 앞당기지만 <b>주 경로가 아니다</b> — 관찰형 앱 철학상
    ...
    public static class CharacterProgressionModel
    {
        ...
        public const float FallbackXpCurveExponent = 1.15f;
