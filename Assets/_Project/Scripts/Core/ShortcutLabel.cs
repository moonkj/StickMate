namespace StickMate.Core
{
    ...
    public static class ShortcutLabel
    {
        ...
        /// <b>강제하지 않는다</b> — 강제하면 지금 출하 중인 단축키(2026-09-07 기준 C·G·K·Q·R)가
        /// 근거 없이 죽는다.
        ...
        ///
        /// <para>★★ <b>2026-09-07 — <c>B</c>가 이 목록에서 빠졌다</b>(9개 → 8개). 근거가 사라져서가
        /// 아니다 — <c>Win+Alt+B</c>(HDR 켜기/끄기)는 지금도 문서에 있다. <b>우리가 그 조합을 더 이상
        /// 쓰지 않는다</b>: 사용자 지시로 「말 걸기」가 폐지되면서 <c>AppControlDirector</c>의
        /// <c>GlobalKey.B</c> 조회가 사라졌다. 이 목록의 계약은 <i>"실기 세션에서 <b>우리가 폴링하는</b>
        /// 글자를 우선순위로 눌러 본다"</i>이므로(아래 테스트 「의심 후보는 전부 실제로 배선된
        /// 글자다」), 안 쓰는 글자를 남기면 세션 시간만 먹고 아무것도 가르지 않는다.
        /// <b>Windows 충돌 위험이 하나 줄었다</b> — 되살릴 때는 이 항목도 함께 되살려라.</para>
        ///
        /// <para><b>★ 이 목록으로 무엇을 사는가</b>: 실기 세션에서 폴링하는 글자를 다 눌러 보는 대신
        /// <b>출하분(C · G · K · Q · R)을 먼저</b> 누르고 <b>OS 쪽 부수 효과</b>(색 필터 반전 /
        /// 녹화 시작 / 마이크 음소거 / 빠른 지원 창)를 본다.
        /// (★ 2026-09-07 — 「이 6개」에서 <b>B(HDR 토글)</b>이 빠져 5개가 됐다: 말 걸기 폐지로
        /// 그 조합을 더 이상 읽지 않는다.) <b>우리 동작이 도는지는 관측 대상이
