...
namespace StickMate.Core
{
    ...
    public enum WanderAmbientMotion
    {
        ...
        /// <summary>G2 끄덕임 2박 — 자세를 유지한 채 상체만 앞으로 <b>두 번</b> 끄덕였다 돌아온다(1.20초).
        ///
        /// <para>★ <b>2026-09-07 개정 — 원래 이름은 <c>FocusRingCheck</c>(발밑 타이머 링 확인)였다.</b>
        /// 그 링(<c>Interaction/FocusWatchRenderer</c>)은 2026-09-06 사용자 지시로 <b>파일째 삭제</b>됐는데
        /// 이 어휘가 함께 손질되지 않아, <b>존재하지 않는 원을 향해 세션당 11.5번 상체를 숙이는 그림</b>이
        /// 그대로 출하돼 있었다(절대 불변 원칙 1 위반).</para>
        ///
        /// <para><b>왜 끄덕임인가</b>: 대체 동작은 <b>가리키는 좌표가 없어야</b> 한다. 어떤 렌더러가
        /// 삭제돼도 같은 방식으로 다시 거짓이 될 수 없기 때문이다. G3(화면 쪽 돌아보기)가 커서를 못 읽으면
        /// 추첨에서 <b>빠져야</b> 하는 것과 정확히 같은 잣대이며, 끄덕임은 애초에 조회할 대상이 없어서
        /// 그 폴백조차 필요 없다. 옛 동작의 «유지 0.28초»가 <b>무언가를 들여다보고 있다</b>의 문법이었으므로,
        /// 그 긴 체류를 <b>두 박의 방향 반전</b>으로 바꾼 것이 이 개정의 핵심이다.</para></summary>
        FocusNod,
